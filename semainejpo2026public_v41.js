(function () {
  // V37 : harmonisation 4 JS, encards enrichis, demi-journée, proposition automatique, jauges salle alignées.
...
function renderStats(slots) {
  ...
  const schoolDebug =
    (window.JPO_PUBLIC_DEBUG &&
      window.JPO_PUBLIC_DEBUG.schoolReservations &&
      window.JPO_PUBLIC_DEBUG.schoolReservations.debug)
      ? window.JPO_PUBLIC_DEBUG.schoolReservations.debug
      : [];
  ...
  (allSlots || []).forEach(slot => {
    ...
    const dedicatedSchool =
      target.includes("scolaire") ||
      target.includes("ecole") ||
      target.includes("école") ||
      target.includes("etablissement") ||
      target.includes("établissement") ||
      target.includes("classe") ||
      target.includes("maternelle") ||
      target.includes("elementaire") ||
      target.includes("élémentaire") ||
      target.includes("college") ||
      target.includes("collège") ||
      target.includes("lycee") ||
      target.includes("lycée");

    if (dedicatedSchool && !(target.includes("tout public") || target.includes("famille") || target.includes("parents"))) {
      schoolProgramKeys[key] = true;
    } else {
      publicProgramKeys[key] = true;
    }
  });
  ...
  $("jpo-public-stats").innerHTML =
    '<div class="stat statgroup"><div class="stat-title">Décompte des publics</div><div class="stat-main"><div class="num">' +
    totalReserved +
    '</div><span>places réservées</span></div><div class="mini"><div><b>' +
    schoolGroupCount +
    '</b><span>groupes/classes écoles</span></div><div><b>' +
    uniqueWithSchools +
    '</b><span>individus uniques estimés</span></div><div><b>' +
    reservedSlots +
    '</b><span>créneaux réservés affichés</span></div><div><b>' +
    slots.length +
    '</b><span>rendez-vous affichés</span></div></div></div>' +

    '<div class="stat statgroup"><div class="stat-title">Autres chiffres clefs</div><div class="stat-main"><div class="num">' +
    (allSlots || []).length +
    '</div><span>créneaux proposés</span></div><div class="mini"><div><b>' +
    Object.keys(publicProgramKeys).length +
    '</b><span>propositions tout public</span></div><div><b>' +
    Object.keys(schoolProgramKeys).length +
    '</b><span>propositions dédiées aux scolaires</span></div><div><b>' +
    schoolConfirmedReservationsCount +
    '</b><span>réservations confirmées</span></div><div><b>180</b><span>jauge auditorium max</span></div></div></div>' +

    '<div class="stat statgroup"><div class="stat-title">Pics d’affluence</div><div class="stat-main"><div class="num">' +
    maxPeak +
    '</div><span>personnes réservées au plus fort</span></div><div class="peaklist">' +
    (peaks.length
      ? peaks.map(p =>
          '<button type="button" class="peakbtn" data-open-peak="' +
          esc(p.key) +
          '">' +
          esc(formatDate(p.dateIso)) +
          ' · ' +
          esc(showTime(p.start)) +
          ' — ' +
          p.total +
          ' pers. (' +
          p.count +
      ...
      : '<div class="lab">Aucune réservation enregistrée avec ces filtres.</div>') +
    '</div></div>';
}
