(function () {
  ...
  const DEMANDES_ECOLES_CSV = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQsA_JKLqgZg_VcrL_-g7PEp_8hVy8XudkP6EBXbiOlHNindQlK4zF8n4Ul3u1xuJu2rTOmr_ySckJN/pub?gid=2115667462&single=true&output=csv";
  ...
  function parseCsvRowsOnly(text) {
    const rows = [];
  ...
  function findHeaderIndex(headers, aliases) {
    ...
    for (const alias of aliases) {
      ...
      if (idx !== -1) return idx;
      idx = normalized.findIndex(h => h.includes(a));
      if (idx !== -1) return idx;
    }
    return -1;
  }

  function schoolValidationStatus(value) {
    const s = norm(value || "");
  ...
  function reservationTokensFromId(id) {
    const parts = String(id || "").split("|").map(x => x.trim());
    if (parts.length < 4) return null;
    return {
      raw: String(id || ""),
      full: parts.length >= 5 ? parts.slice(0, 5).join("|") : "",
      timeRoom: parts.slice(0, 4).join("|"),
      dateStartRoom: [parts[0], parts[1], parts[3]].join("|"),
      students: parts.length >= 7 ? numberFrom(parts[6]) : 0,
      adults: parts.length >= 8 ? numberFrom(parts[7]) : 0
    };
  }

  function slotMatchTokens(slot) {
    const id = slotReservationId(slot);
    const parts = String(id || "").split("|").map(x => x.trim());
    return {
      full: parts.length >= 5 ? parts.slice(0, 5).join("|") : "",
      timeRoom: parts.length >= 4 ? parts.slice(0, 4).join("|") : "",
      dateStartRoom: parts.length >= 4 ? [parts[0], parts[1], parts[3]].join("|") : ""
    };
  }

  function buildSchoolReservationList(csvText) {
    const rows = parseCsvRowsOnly(csvText);
    if (!rows.length) return [];

    const headers = rows[0] || [];
    const data = rows.slice(1);
    const cIds = findHeaderIndex(headers, ["IDs créneaux", "IDs creneaux", "ID créneau", "ID creneau"]);
    const cValid = findHeaderIndex(headers, ["VALIDATION CRD", "Validation CRD", "STATUT", "Statut"]);
    const cStudents = findHeaderIndex(headers, ["Nombre d'élèves", "Nombre eleves", "Élèves", "Eleves", "Participants"]);
    const cAdults = findHeaderIndex(headers, ["Accompagnateurs", "Nombre d'accompagnateurs", "Adultes"]);
    const cTotal = findHeaderIndex(headers, ["Nombre total", "Effectif total", "Total", "Effectif"]);

    const out = [];
    data.forEach(row => {
      const status = cValid !== -1 ? schoolValidationStatus(row[cValid]) : "pending";
      if (status === "cancelled") return;

      let idsCell = cIds !== -1 ? String(row[cIds] || "") : "";
      if (!idsCell) {
        idsCell = String((row || []).find(cell => /2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(String(cell || ""))) || "");
      }
      if (!idsCell) return;

      splitReservationIds(idsCell).forEach(rawId => {
        const tokens = reservationTokensFromId(rawId);
        if (!tokens) return;
        let total = tokens.students + tokens.adults;
        if (!total && cTotal !== -1) total = numberFrom(row[cTotal]);
        if (!total && (cStudents !== -1 || cAdults !== -1)) {
          total = numberFrom(cStudents !== -1 ? row[cStudents] : "") + numberFrom(cAdults !== -1 ? row[cAdults] : "");
        }
        if (!total) return;
        out.push({
          full: tokens.full,
          timeRoom: tokens.timeRoom,
          dateStartRoom: tokens.dateStartRoom,
          total,
          status
        });
      });
  ...
  function applySchoolReservationsSafely(csvText) {
    let schoolReservations = [];
    try {
      schoolReservations = buildSchoolReservationList(csvText || "");
    } catch (err) {
      console.warn("Réservations écoles ignorées côté public", err);
      return;
    }
    if (!schoolReservations.length || !Array.isArray(allSlots) || !allSlots.length) return;

    allSlots = allSlots.map(slot => {
      try {
        const tokens = slotMatchTokens(slot);
        let schoolConfirmed = 0;
        let schoolPending = 0;

        schoolReservations.forEach(r => {
          const match =
            (tokens.full && r.full && r.full === tokens.full) ||
            (tokens.timeRoom && r.timeRoom && r.timeRoom === tokens.timeRoom) ||
            (tokens.dateStartRoom && r.dateStartRoom && r.dateStartRoom === tokens.dateStartRoom);
          if (!match) return;
          if (r.status === "confirmed") schoolConfirmed += r.total;
          else schoolPending += r.total;
        });

        const schoolReserved = schoolConfirmed + schoolPending;
        const publicReserved = Number(slot.publicReserved || 0);
        const capacity = Number(slot.capacity || 0);
        const remaining = Math.max(0, capacity - publicReserved - schoolReserved);
    ...
    window.JPO_PUBLIC_SCHOOL_RESERVATIONS_DEBUG = schoolReservations;
    try { renderControls(); render(); } catch (err) { console.warn("Affichage public après réservations écoles ignoré", err); }
  }

  function loadSchoolReservationsAfterInitialRender() {
    fetch(DEMANDES_ECOLES_CSV)
      .then(r => { if (!r.ok) throw new Error("CSV demandes écoles HTTP " + r.status); return r.text(); })
      .then(text => applySchoolReservationsSafely(text))
      .catch(err => console.warn("CSV écoles non chargé côté public", err));
  }
  ...
  Promise.all([
    ...
    .then(([propText, publicText]) => {
      ...
      bind();
      render();
      loadSchoolReservationsAfterInitialRender();
    })
