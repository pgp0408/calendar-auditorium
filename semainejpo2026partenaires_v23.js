document.addEventListener("DOMContentLoaded", function () {
  ...
  function applyReservations(slots,map){return slots.map(s=>{let res=map[s.id];
    if(!res && isOtherRoomKey(s.roomKey)){
      // Compatibilite avec les anciennes demandes qui stockaient les salles "Autre" sous la cle unique "other".
      // On n'accepte qu'une correspondance exacte avec le titre, jamais un prefixe date/heure/other trop large.
      res=map[legacyOtherSlotId(s)];
    }
    if(!res){res={confirmed:0,details:[]};
      // Pour les salles nommees classiques, on conserve la tolerance par prefixe.
      // Pour les salles "Autre", elle est volontairement desactivee pour ne pas additionner
      // plusieurs lieux differents au meme horaire.
      if(!isOtherRoomKey(s.roomKey)){
        const prefix=[s.dateIso,compact(s.start),compact(s.end),s.roomKey].join("|")+"|";
        Object.keys(map).forEach(id=>{if(id.indexOf(prefix)===0){res.confirmed+=map[id].confirmed;(map[id].details||[]).forEach(d=>res.details.push(d));}});
      }
    } const extra=(res.details||[]).map(d=>(d.establishment||"")+" "+(d.group||"")).join(" "); return {...s,confirmed:res.confirmed||0,details:res.details||[],search:norm(s.search+" "+extra)};});}
