document.addEventListener("DOMContentLoaded", function () {
  ...
  const STORAGE_KEY = "jps_ecoles_v23_panier";
  const LAST_GROUP_KEY = "jps_ecoles_v23_last_group";
  ...
  root.innerHTML = `
    <style>
      ...
      #jps-ecoles .timegroup.overlap{
        border-color:#bfdbfe;
        box-shadow:0 10px 26px rgba(37,99,235,.06);
      }

      #jps-ecoles .overlap-note{
        display:inline-flex;
        align-items:center;
        border-radius:999px;
        background:#dbeafe;
    ...
    <div class="wrap">
      ...
      <div class="content">
        ...
        <div class="tools">
          ...
          <div class="toolbox">
            <label for="jps-pole-filter">Filtrer par pôle</label>
            <select id="jps-pole-filter">
              <option value="all">Tous les pôles</option>
              <option value="auditorium">Auditorium</option>
              <option value="voix">Voix / chant / chœur</option>
              <option value="orchestre">Orchestre / instruments</option>
          ...
          <div class="toolbox">
            <label>Parcours conseillés</label>
            <div class="quick-buttons">
              <button type="button" data-suggest-duration="60">Je viens 1h</button>
              <button type="button" data-suggest-duration="90">Je viens 1h30</button>
              <button type="button" data-suggest-duration="120">Je viens 2h</button>
            </div>
          </div>
        </div>
  ...
  function visibleSlotsForActiveDay() {
    return allSlots.filter(slot => slot.dateIso === activeDay && slotMatchesLevel(slot) && slotMatchesPole(slot));
  }

  function representativeSlotForTime(slots, start, end) {
  ...
  function buildSuggestion(slots, minutesWanted) {
    ...
    for (let i = 0; i <= keys.length - count; i++) {
      ...
      for (let j = 0; j < windowKeys.length; j++) {
        ...
        const slot = representativeSlotForTime(slots, parts[0], parts[1]);
        if (!slot) ok = false;
        chosen.push(slot);
      }

      if (ok && chosen.every(Boolean)) return chosen;
    }
  ...
  function renderSuggestions(slots) {
    ...
    const durations = [
      { min: 60, label: "Parcours possible sur 1h" },
      { min: 90, label: "Parcours possible sur 1h30" },
      { min: 120, label: "Parcours possible sur 2h" }
    ];

    const suggestions = durations
      .map(d => ({ ...d, slots: buildSuggestion(slots, d.min) }))
  ...
  function slotsOverlap(a, b) {
    ...
    const aEnd = slotEndMinutes(a);
    const bStart = slotStartMinutes(b);
    const bEnd = slotEndMinutes(b);

    if (aStart === null || aEnd === null || bStart === null || bEnd === null) return false;
    return aStart < bEnd && bStart < aEnd;
  }

  function buildOverlapGroups(slotsForDay) {
    const sorted = slotsForDay
      ...
      .sort((a, b) => {
        ...
        const endDiff = slotEndMinutes(a) - slotEndMinutes(b);
    ...
    sorted.forEach(slot => {
      ...
      // Si le créneau démarre avant la fin de la fenêtre précédente, il chevauche cette fenêtre.
      // S'il commence exactement à la fin, on crée une nouvelle fenêtre : c'est un enchaînement, pas un chevauchement.
      if (last && start < last.endMin) {
        last.items.push(slot);
        last.endMin = Math.max(last.endMin, end);
        last.startMin = Math.min(last.startMin, start);
      } else {
        groups.push({
          startMin: start,
          endMin: end,
    ...
    return groups.map(group => {
      const uniqueTimes = Array.from(new Set(group.items.map(slotTimeKey)));
      return {
        ...group,
        mixedTimes: uniqueTimes.length > 1,
        exactLabel: uniqueTimes.length === 1
          ? showTime(group.items[0].start) + "–" + showTime(group.items[0].end)
          : showTime(timeFromMin(group.startMin)) + "–" + showTime(timeFromMin(group.endMin))
      };
    });
  }

  function renderOverlapGroup(group) {
    ...
    const title = group.mixedTimes ? "Fenêtre " + group.exactLabel : group.exactLabel;
    const subtitle = group.mixedTimes
      ? "Ces propositions se chevauchent ou se recoupent : les horaires exacts sont indiqués sur chaque carte."
      : "Choix possibles sur ce créneau.";

    return (
      '<section class="timegroup ' + (group.mixedTimes ? "overlap" : "") + '">' +
        '<div class="timegroup-head">' +
          '<div>' +
            '<div class="timegroup-title">' + esc(title) + '</div>' +
            '<div class="timegroup-sub">' + esc(subtitle) + '</div>' +
            (group.mixedTimes ? '<div class="overlap-note">Créneaux affichés côte à côte car ils se chevauchent</div>' : '') +
          '</div>' +
  ...
  function render() {
    ...
    const overlapGroups = buildOverlapGroups(visible);
      ...
      '<section class="day">' +
        '<div class="dayhead">' +
          '<div>' +
            ...
            '<div class="daymeta">Choisissez une fenêtre horaire, puis ajoutez une activité au parcours d’une classe.</div>' +
          '</div>' +
          '<div class="badge">' + visible.length + ' créneau(x)</div>' +
        '</div>' +
        '<div class="daybody">' +
          '<p class="choice-hint">Mode d’emploi : les propositions qui se chevauchent sont affichées côte à côte, même si leurs horaires ne sont pas strictement identiques. Les horaires exacts restent visibles sur chaque carte.</p>' +
          (overlapGroups.length ? overlapGroups.map(renderOverlapGroup).join("") : '<div class="empty">Aucun créneau disponible pour cette journée.</div>') +
        '</div>' +
      '</section>';
  ...
  ["jps-booking", "jps-cart", "jps-info", "jps-confirm"].forEach(id => {
  ...
  ["jps-level-filter", "jps-pole-filter"].forEach(id => {
    $(id).addEventListener("change", render);
  });

  root.querySelectorAll("[data-suggest-duration]").forEach(button => {
    button.addEventListener("click", function () {
      const target = $("jps-suggestions");
