document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    <style>
      ...
      #auditorium-calendar .ac-dot-ok { background: #16a34a; }
      #auditorium-calendar .ac-dot-wait { background: #f59e0b; }
      ...
      #auditorium-calendar .ac-item-ok { border-left-color: #16a34a; }
      #auditorium-calendar .ac-item-wait { border-left-color: #f59e0b; }
      ...
      @media (max-width: 980px) {
        #auditorium-calendar .ac-layout { grid-template-columns: 1fr; }
        #auditorium-calendar .ac-list { max-height: none; }
      }
    </style>
  ...
  fetch(CSV_URL)
    ...
    .then(function (text) {
      ...
      allEvents = rows.map(function (row) {
        const cfg = statusCfg(row[7]);
        const dateIso = parseDateFR(row[2]);

        if (!cfg || !dateIso) return null;

        return {
          demandeur: (row[1] || "Réservation").toString().trim(),
          details: (row[5] || "").toString().trim(),
          heure: formatTime(row[3]),
          duree: (row[4] || "").toString().trim(),
          statut: cfg.label,
          cls: cfg.cls,
          dateIso: dateIso,
          start: dateIso + "T" + ((row[3] || "00:00:00").toString().trim())
        };
      }).filter(Boolean);
