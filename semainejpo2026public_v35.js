(function () {
  ...
  function buildSchoolReservationsFromPublicCSV(rows) {
    const maps = { exact: {}, dateStartRoomTitle: {}, dateStartTitle: {}, dateStartRoom: {}, dateStart: {}, debug: [] };
    (rows || []).forEach(row => {
      try {
        ...
        splitReservationIds(rawIds).forEach(rawId => {
          ...
          addSchoolReservation(maps.dateStart, [info.date, info.start].join("|"), status, total);
  ...
  function schoolReservationForPublicSlot(slot, maps) {
    ...
    return (maps.exact && maps.exact[k.exact]) ||
      ...
      (maps.dateStart && maps.dateStart[k.dateStart]) ||
      emptySchoolCount();
  }
  ...
  Promise.all([
    ...
    .then(([propText, publicText]) => {
      ...
      // Chargement non bloquant des réservations scolaires :
      // même si ce CSV échoue ou change de format, la page public reste celle de la V23.
      fetch(DEMANDES_CSV)
        .then(r => r && r.ok ? r.text() : "")
        .then(txt => {
          if (!txt) return;
          const schoolMaps = buildSchoolReservationsFromPublicCSV(parseCSV(txt));
          allSlots = applySchoolReservationsSafely(allSlots, schoolMaps);
          window.JPO_PUBLIC_DEBUG = {
            schoolReservations: schoolMaps,
            schoolReservationCount: schoolMaps && schoolMaps.debug ? schoolMaps.debug.length : 0,
        ...
        .catch(err => {
          window.JPO_PUBLIC_DEBUG = Object.assign(window.JPO_PUBLIC_DEBUG || {}, { schoolReservationsError: String(err && err.message ? err.message : err) });
        });
    })
