(function () {
  ...
  const CART_STORAGE_KEY = "jpo_public_v19_panier";
  const CART_PLACES_STORAGE_KEY = "jpo_public_v19_places";
  ...
  function loadCartPlaces() {
    const n = parseInt(localStorage.getItem(CART_PLACES_STORAGE_KEY) || "1", 10);
    return n >= 1 && n <= 6 ? n : 1;
  }

  function saveCartPlaces() {
    try { localStorage.setItem(CART_PLACES_STORAGE_KEY, String(cartPlaces)); } catch (err) {}
  }
  ...
  function currentCartSlots() {
    ...
    return cart.map(item => byId[item.id]).filter(Boolean);
  }

  function addToCart(slotId) {
    const slot = allSlots.find(x => x.id === slotId);
  ...
  function renderCartModal() {
    ...
    body.innerHTML = '<div class="cartplaces"><label for="jpo-public-cart-places"><strong>Nombre de places souhaitées pour chaque rendez-vous</strong></label><br><select id="jpo-public-cart-places" style="margin-top:8px;max-width:180px"><option value="1">1 place</option><option value="2">2 places</option><option value="3">3 places</option><option value="4">4 places</option><option value="5">5 places</option><option value="6">6 places</option></select><div style="margin-top:8px">Ce nombre sera appliqué à tous les rendez-vous du panier.</div></div>' +
    ...
    if (select) {
      ...
      select.addEventListener("change", function(){
        cartPlaces = Math.max(1, Math.min(6, parseInt(select.value || "1", 10) || 1));
        saveCartPlaces();
        renderCartBar();
        renderCartModal();
        render();
  ...
  function openCartModal() {
    renderCartModal();
    $("jpo-public-cart-modal").classList.add("open");
  }
  ...
  function bind() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (e.target.closest("[data-close-cart]")) $("jpo-public-cart-modal").classList.remove("open");
    });
