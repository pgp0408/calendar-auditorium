(function () {
  ...
  function normalizeReservationId(raw) {
    ...
    return [parts[0], compactTime(parts[1]), compactTime(parts[2]), parts[3], slug(parts[4])].join("|");
  }

  function reservationStatusEcole(v) {
    const s = norm(v || "");
  ...
  function countFromSchoolId(raw, row) {
    ...
    if (!total && row) {
      total = numberFrom(get(row, ["Nombre d’élèves", "Nombre d'eleves", "Nombre d'élèves", "Élèves", "Eleves", "Participants", "Nombre de participants"])) +
              numberFrom(get(row, ["Accompagnateurs", "Adultes", "Nombre d’accompagnateurs", "Nombre d'accompagnateurs"]));
    }
    return total;
  }

  function addSchoolCount(map, key, status, total) {
    if (!key || !total) return;
    if (!map[key]) map[key] = { confirmed: 0, pending: 0, total: 0 };
  ...
  function schoolKeysFromId(raw) {
    ...
    return {
      ...
      dayStartRoomTitle: [date, start, room, title].join("|"),
      dayStartTitle: [date, start, title].join("|"),
      dayStartRoom: [date, start, room].join("|"),
      dayStart: [date, start].join("|"),
      parts
    };
  }
  ...
  function slotSchoolKeys(slot) {
    ...
    return {
      exact: [date, start, end, room, title].join("|"),
      prefix: [date, start, end, room].join("|") + "|",
      dayStartRoomTitle: [date, start, room, title].join("|"),
      dayStartTitle: [date, start, title].join("|"),
      dayStartRoom: [date, start, room].join("|"),
      dayStart: [date, start].join("|")
  ...
  function buildSchoolReservations(rows) {
    const map = { exact: {}, dayStartRoomTitle: {}, dayStartTitle: {}, dayStartRoom: {}, dayStart: {}, debug: [] };
    (rows || []).forEach(row => {
      const status = reservationStatusEcole(get(row, ["VALIDATION CRD", "Validation CRD", "STATUT CRD", "Statut CRD", "Statut"]));
      ...
      String(rawIds).split(/[;\n]+/).map(x => x.trim()).filter(Boolean).forEach(line => {
        ...
        if (!keys) return;
        const total = countFromSchoolId(line, row);
        if (!total) return;
        addSchoolCount(map.exact, keys.exact, status, total);
        addSchoolCount(map.dayStartRoomTitle, keys.dayStartRoomTitle, status, total);
        addSchoolCount(map.dayStartTitle, keys.dayStartTitle, status, total);
        addSchoolCount(map.dayStartRoom, keys.dayStartRoom, status, total);
        addSchoolCount(map.dayStart, keys.dayStart, status, total);
        map.debug.push({ id: keys.exact, total, status, raw: line });
      });
    });
    return map;
  }
  ...
  function mergeCounts(list) {
    const out = { confirmed: 0, pending: 0, total: 0 };
    (list || []).forEach(x => {
      if (!x) return;
      out.confirmed += Number(x.confirmed || 0);
      out.pending += Number(x.pending || 0);
      out.total += Number(x.total || 0);
    });
    return out;
  }

  function getSchoolReservationForSlot(slot, schoolMap) {
    const empty = { confirmed: 0, pending: 0, total: 0 };
    if (!slot || !schoolMap) return empty;
    const keys = slotSchoolKeys(slot);
    const direct = (schoolMap.exact && schoolMap.exact[keys.exact]) ||
                   (schoolMap.dayStartRoomTitle && schoolMap.dayStartRoomTitle[keys.dayStartRoomTitle]) ||
                   (schoolMap.dayStartTitle && schoolMap.dayStartTitle[keys.dayStartTitle]) ||
                   (schoolMap.dayStartRoom && schoolMap.dayStartRoom[keys.dayStartRoom]);
    if (direct) return direct;
    // Filet de secours : même date + même heure de début, uniquement si une seule réservation scolaire existe à cette heure.
    const loose = schoolMap.dayStart && schoolMap.dayStart[keys.dayStart];
    return loose || empty;
  }
  ...
  function applyPublicReservations(slots, reservations, schoolReservations) {
    ...
    return slots.map(slot => {
      try {
        ...
        const sc = getSchoolReservationForSlot(slot, schoolMap);
  ...
  function removeAutoSlotsWithRoomConflicts(slots) {
    return (slots || []).filter(slot => {
      ...
      const hasConflict = (slots || []).some(other => {
        if (other === slot) return false;
        if (other.autoLabel) return false;
  ...
  Promise.all([
    ...
    .then(([propText, publicText, demandesText]) => {
      ...
      let schoolReservationsMap = {};
      try { publicReservationsMap = buildPublicReservations(parseCSV(publicText || "")); }
      catch (err) { console.error("Erreur réservations public JPO", err); publicReservationsMap = {}; }
      try { schoolReservationsMap = buildSchoolReservations(parseCSV(demandesText || "")); }
      catch (err) { console.error("Erreur réservations scolaires lues par JPO public", err); schoolReservationsMap = {}; }
      try { allSlots = applyPublicReservations(propositionSlots, publicReservationsMap, schoolReservationsMap); }
      catch (err) {
        console.error("Erreur application réservations JPO", err);
