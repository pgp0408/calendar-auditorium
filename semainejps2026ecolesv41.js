document.addEventListener("DOMContentLoaded", function () {
  ...
  const STORAGE_KEY = "jps_ecoles_v34_panier";
  const LAST_GROUP_KEY = "jps_ecoles_v34_last_group";
  ...
  function normalizePublicReservationId(raw) {
    const parts = String(raw || "").split("|").map(x => String(x || "").trim());
    if (parts.length < 5) return "";
    const date = parts[0];
    const start = compact(parts[1]);
    const end = compact(parts[2]);
    const room = parts[3];
  ...
  function buildPublicReservations(rows) {
    ...
    const headers = rows[0] || [];
    const data = rows.slice(1);
    const C_ID = findCol(headers, ["ID créneau", "ID creneau", "Id créneau", "Id creneau", "IDs créneaux", "IDs creneaux"]);
    ...
    data.forEach(row => {
      ...
      const status = C_STATUS !== -1 ? publicReservationStatus(row[C_STATUS]) : "pending";
      if (!rawId || status === "cancelled") return;

      const places = C_PLACES !== -1 ? parseNumber(row[C_PLACES]) : 0;
      if (!places) return;

      splitReservationIds(rawId)
        .map(normalizePublicReservationId)
        .filter(Boolean)
        .forEach(id => {
  ...
  function applyReservations(slots, reservations, publicReservations) {
    return slots.map(slot => {
      ...
      const publicReservation = (publicReservations && publicReservations[slot.id])
        ? publicReservations[slot.id]
        : { confirmed: 0, pending: 0, total: 0 };
      const publicTaken = Number(publicReservation.total || 0);
      const remaining = Math.max(0, slot.capacity - reservation.confirmed - reservation.pending - publicTaken);

      return {
        ...slot,
        confirmed: reservation.confirmed,
        pending: reservation.pending,
        publicConfirmed: Number(publicReservation.confirmed || 0),
        publicPending: Number(publicReservation.pending || 0),
        publicReserved: publicTaken,
        remaining,
        full: remaining <= 0
      };
    });
  ...
  Promise.all([
    ...
    fetch(PUBLIC_RESERVATIONS_CSV).then(response => {
      if (!response.ok) throw new Error("Réservations public HTTP " + response.status);
      return response.text();
    }).catch(() => "")
  ])
    .then(([propositionsText, demandesText, publicReservationsText]) => {
      const propositionsRows = parseCSV(propositionsText);
      const demandesRows = parseCSV(demandesText);
      const publicReservationsRows = parseCSV(publicReservationsText || "");
      const reservations = buildReservations(demandesRows);
      const publicReservations = buildPublicReservations(publicReservationsRows);
