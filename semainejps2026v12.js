document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    <style>
      ...
      #semainejps-calendar .jps-list-modal{
        display:none;
        position:fixed;
        inset:0;
      ...
      #semainejps-calendar .jps-list-modal.open{display:flex}
      #semainejps-calendar .jps-modalbox,
      #semainejps-calendar .jps-day-modalbox,
      #semainejps-calendar .jps-list-modalbox{
        width:min(900px,100%);
        max-height:90vh;
        overflow:auto;
        background:#fff;
      ...
      #semainejps-calendar .jps-list-modalbox{width:min(1100px,100%)}
      #semainejps-calendar .jps-close,
      #semainejps-calendar .jps-day-close,
      #semainejps-calendar .jps-list-close{
        position:absolute;
        top:12px;
        right:14px;
        border:0;
      ...
      #semainejps-calendar .jps-list-modaltitle{
      ...
      #semainejps-calendar .jps-timetable{
        min-width:1120px;
        display:grid;
        grid-template-columns:95px repeat(6,minmax(155px,1fr));
      }
      ...
      #semainejps-calendar .jps-list-item{
        border:1px solid #e7eaee;
      ...
      @media(max-width:1150px){
        ...
        #semainejps-calendar .jps-frise.public{grid-template-columns:1fr}
        #semainejps-calendar .jps-alerts{grid-template-columns:1fr 1fr}
      }
      @media(max-width:760px){
        ...
        #semainejps-calendar .jps-modal,
        #semainejps-calendar .jps-day-modal,
        #semainejps-calendar .jps-list-modal{padding:10px}
        #semainejps-calendar .jps-modalbox,
        #semainejps-calendar .jps-day-modalbox,
        #semainejps-calendar .jps-list-modalbox{padding:20px;border-radius:20px}
      }
    </style>
      ...
      <div class="jps-header">
        <div class="jps-header-grid">
          <div>
            ...
            <p class="jps-sub">
              V12 interne — tableau de pilotage de la semaine inaugurale. Cette version prend en charge les propositions automatiques sur plusieurs créneaux.
            </p>
          </div>
    ...
    <div id="jps-list-modal" class="jps-list-modal">
      <div class="jps-list-modalbox">
        <button type="button" id="jps-list-close" class="jps-list-close">&times;</button>
        <h3 class="jps-list-modaltitle">Propositions détaillées</h3>
        <div id="jps-list-content"></div>
      </div>
    </div>
  ...
  function roomInfo(v){
    ...
    if(s.includes("peu importe") || s.includes("indifferent") || s.includes("indifférent") || s.includes("a definir") || s.includes("à définir")) {
      return { key:"any", short:"Peu importe", label:"Peu importe", capacity:"à affecter" };
    }
  ...
  function isAutoEvent(ev){
    return !!ev.autoGenerated;
  }

  function isRealProgramming(ev){
    return ev.statusKind !== "refused";
  }
  ...
  function renderAlerts(data){
    ...
    DAYS.filter(d => d.group === "school").forEach(day => {
      ...
      empty.forEach(slot => emptySchoolSlots.push(day.name + " " + slot));
    });

    const conflicts = getDayConflicts(realData);

    $("jps-alerts").innerHTML =
      '<div class="jps-alert ' + (emptySchoolSlots.length ? "empty" : "ok") + '"><strong>Créneaux scolaires non comblés</strong>' +
      (emptySchoolSlots.length ? esc(emptySchoolSlots.join(", ")) : "Tous les créneaux scolaires ont au moins une proposition.") +
      '</div>' +
      '<div class="jps-alert ' + (schoolTimeEvents.length < 4 ? "low" : "ok") + '"><strong>Temps scolaire réel</strong>' +
      schoolTimeEvents.length + ' proposition(s) entre 08h30–12h00 / 13h00–17h00.</div>' +
      '<div class="jps-alert ok"><strong>Hors temps scolaire / tout public</strong>' +
      outOfSchoolEvents.length + ' proposition(s) identifiée(s).</div>' +
      '<div class="jps-alert ' + (conflicts ? "low" : "ok") + '"><strong>Litiges / chevauchements</strong>' +
      (conflicts ? conflicts + ' chevauchement(s) dans une même salle.' : "Aucun litige majeur détecté.") +
      '<br>Créneaux automatiques : ' + autoEvents.length + '</div>';
  }
  ...
  function slotSummary(dayIso, momentKey, data, mode){
    ...
    const autoEvents = events.filter(isAutoEvent);
    ...
    return { events, realEvents, autoEvents, scoreClass, scoreText };
  }
      ...
      moments.forEach(moment => {
        ...
        if(!summary.events.length){
          ...
          Object.keys(byRoom).forEach(roomKey => {
            ...
            chip.className = "jps-roomchip " + (isAutoEvent(main) ? "auto" : main.statusKind);
  ...
  function renderDayButtons(data){
    ...
    DAYS.forEach(day => {
      ...
      const autoEvents = dayEvents.filter(isAutoEvent);
      ...
      if(conflicts){
        ...
        warningClass = "empty";
        warningText = "À combler";
      } else {
        warningText = realEvents.length + " proposition(s)";
      }
      ...
      btn.innerHTML =
        ...
        '<div class="jps-day-btn-meta">' +
          ...
          '<strong>Automatique :</strong> ' + autoEvents.length + ' · <strong>Total :</strong> ' + realEvents.length +
        '</div>' +
  ...
  function renderEventCard(ev){
    ...
    div.className = "jps-slot-card " + (isAutoEvent(ev) ? "auto" : ev.statusKind);
    const end = ev.allDay ? "" : eventEndMinutes(ev);
    const timeLabel = ev.allDay ? "Journée entière" : ev.time + (end ? "–" + minutesToTime(end) : "");
    div.innerHTML =
      '<span class="jps-slot-time">' + esc(timeLabel) + '</span>' +
      '<span class="jps-slot-title">' + esc(ev.title) + '</span>' +
      '<span class="jps-slot-meta">' + esc(ev.statusLabel) + (isAutoEvent(ev) ? " · Auto" : "") + '</span>';
  ...
  function openDayModal(dayIso){
    ...
    $("jps-day-modal-sub").textContent = "Planning par créneaux de 30 minutes et par salle. Les programmations automatiques apparaissent sur les créneaux générés.";
  ...
  function renderEventListHTML(events){
    if(!events.length) return '<div class="jps-list-item">Aucune proposition à afficher.</div>';
    return '<div class="jps-list-grid">' + events.map(ev =>
      '<div class="jps-list-item ' + ev.statusKind + '" data-event-id="' + ev.uid + '">' +
        ...
        '<div class="jps-list-item-meta">' +
          ...
          (ev.autoGenerated ? '<div><strong>Mode :</strong> Programmation automatique</div>' : '') +
        '</div>' +
      '</div>'
    ).join("") + '</div>';
  }
  ...
  function getListEvents(data){
    ...
    data.forEach(ev => {
      if(ev.autoGenerated){
        const key = ev.sourceUid || (norm(ev.title) + "|" + norm(ev.roomLabel));
        if(seenAuto[key]) return;
        seenAuto[key] = true;
      }
      result.push(ev);
    });
  ...
  function openListModal(){
    ...
    data.forEach(ev => {
      const key = ev.autoGenerated ? "auto" : ev.dateIso;
    ...
    $("jps-list-content").innerHTML = keys.map(key => {
      const title = key === "auto" ? "Programmations automatiques" : formatDateFR(key);
    ...
    $("jps-list-content").querySelectorAll("[data-event-id]").forEach(el => {
      el.onclick = () => {
        const ev = allEvents.find(e => e.uid === el.getAttribute("data-event-id"));
        if(ev) openModal(ev);
      };
    });
  }
  ...
  fetch(CSV_URL)
    ...
    .then(text => {
      ...
      if([C_TITLE,C_ROOM,C_STATUS].some(i => i === -1)){
        throw new Error("Colonnes introuvables : vérifie Intitulé du projet, Lieu souhaité et STATUT.");
      }

      const events = [];

      dataRows.forEach((row, idx) => {
        const title = row[C_TITLE] || "Projet sans titre";
        ...
        const baseUid = "ev" + idx + "_" + norm(title).replace(/[^a-z0-9]+/g,"-");
        ...
        if(autoMode.includes("oui")){
          ...
          if(autoMode.includes("mercredi")){
            autoDays.push("2026-06-17");
          }

          autoDays.forEach(autoDateIso => {
            AUTO_SLOTS.forEach(pair => {
              ...
              events.push({
                ...common,
                uid: baseUid + "_" + autoDateIso + "_" + start.replace(":",""),
                sourceUid: baseUid,
                dateIso: autoDateIso,
                time: start,
                duration: "1 heure",
        ...
        if(!dateIso || dateIso < "2026-06-15" || dateIso > "2026-06-21") return;

        const allDay = isAllDay(duration);

        events.push({
          ...common,
          uid: baseUid + "_" + dateIso,
          sourceUid: baseUid,
          dateIso,
