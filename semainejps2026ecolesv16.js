document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    <style>
      ...
      #jps-ecoles .card{transition:transform .15s ease,box-shadow .15s ease,border-color .15s ease}#jps-ecoles .card:not(.is-full):hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(15,23,42,.08);border-color:#cbd5e1}
  ...
  function renderSingle(s) {
    ...
    return '<article class="card ' + (disabled ? 'is-full' : '') + '"><div class="top"><div class="meta">' + esc(showTime(s.start)) + '–' + esc(showTime(s.end)) + '</div><div class="title">' + esc(s.title) + '</div><div><span class="room-tag ' + tagClass(s) + '">' + esc(s.roomLabel) + '</span>' + (s.pole ? '<span class="room-tag">' + esc(s.pole) + '</span>' : '') + '</div>' + (preview ? '<div class="desc-preview">' + esc(preview) + '</div>' : '') + '</div><div class="body"><div class="capgrid"><div class="cap"><strong>' + s.capacity + '</strong><span>jauge</span></div><div class="cap"><strong>' + s.confirmed + '</strong><span>confirmés</span></div><div class="cap"><strong>' + s.pending + '</strong><span>en attente</span></div><div class="cap"><strong>' + rem + '</strong><span>restants</span></div></div><div class="status ' + st.cls + '">' + st.txt + '<br>' + rem + ' place(s) encore disponible(s)</div><div class="card-actions"><button class="btn2" type="button" data-info-slot-id="' + esc(s.id) + '">Voir le détail</button>' + (disabled ? '<button class="btn disabled" type="button">Complet / réservé</button>' : '<button class="btn" type="button" data-add-slot-id="' + esc(s.id) + '">Ajouter au panier</button>') + '</div></div></article>';
  }
