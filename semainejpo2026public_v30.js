(function () {
  ...
  function normalizeSchoolSlotId(raw) {
    const parts = String(raw || "").split("|").map(x => x.trim());
    if (parts.length < 5) return "";
    return parts.slice(0, 5).join("|");
  }

  function schoolCountFromId(raw) {
    const parts = String(raw || "").split("|").map(x => x.trim());
    if (parts.length >= 8) {
      return numberFrom(parts[6]) + numberFrom(parts[7]);
    }
    return 0;
  }
  ...
  function buildSchoolReservations(rows) {
    ...
    try {
      rows.forEach(row => {
        ...
        if (!rawIds) {
          Object.keys(row || {}).some(k => {
            ...
            if (/2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(v)) { rawIds = v; return true; }
            return false;
          });
        }
        if (!rawIds) return;

        splitReservationIds(rawIds).forEach(raw => {
          const id = normalizeSchoolSlotId(raw);
          const total = schoolCountFromId(raw);
          if (!id || !total) return;
          if (!map[id]) map[id] = { confirmed: 0, pending: 0, total: 0 };
      ...
      console.error("Erreur lecture réservations écoles JPO public", err);
      return {};
    }
    return map;
  }

  function reservationForSlot(reservations, slot) {
    const technicalId = slotTechnicalId(slot);
    const found = reservations[technicalId] || { confirmed: 0, pending: 0, total: 0, details: [] };
    const out = {
    ...
    const prefix = [slot.dateIso, compactTime(slot.start), compactTime(slot.end), slot.roomKey].join("|") + "|";
    Object.keys(reservations || {}).forEach(id => {
      if (id === technicalId) return;
      if (id.indexOf(prefix) !== 0) return;
      out.confirmed += Number(reservations[id].confirmed || 0);
      out.pending += Number(reservations[id].pending || 0);
      out.total += Number(reservations[id].total || 0);
      if (Array.isArray(reservations[id].details)) out.details = out.details.concat(reservations[id].details);
    });
    return out;
  }

  function applyAllReservations(slots, publicReservations, schoolReservations) {
    return slots.map(slot => {
      try {
        const technicalId = slotTechnicalId(slot);
        const pub = reservationForSlot(publicReservations || {}, slot);
        const school = reservationForSlot(schoolReservations || {}, slot);
        const publicOccupied = Number(pub.total || 0);
        const schoolOccupied = Number(school.total || 0);
        ...
        return Object.assign({}, slot, {
          ...
          publicConfirmed: Number(pub.confirmed || 0),
          publicPending: Number(pub.pending || 0),
          publicReserved: publicOccupied,
          schoolConfirmed: Number(school.confirmed || 0),
          schoolPending: Number(school.pending || 0),
          schoolReserved: schoolOccupied,
          totalReserved: occupied,
          publicRemaining: remaining,
          publicReservationDetails: pub.details || [],
  ...
  function renderStats(slots) {
    ...
    $("jpo-public-stats").innerHTML = [
      ...
      [schoolTaken, "places déjà bloquées écoles"]
  ...
  function openSlot(id) {
    ...
    $("jpo-public-modal-body").innerHTML =
      ...
      rowHtml("Réservations", "Public : " + (s.publicReserved || 0) + " · Écoles : " + (s.schoolReserved || 0) + " · Restantes : " + (s.publicRemaining ?? s.capacity)) +
  ...
  Promise.all([
    ...
    .then(([propText, publicText, schoolText]) => {
      ...
      try {
        schoolReservationsMap = buildSchoolReservations(parseCSV(schoolText || ""));
      } catch (err) {
        console.error("Erreur réservations écoles dans JPO public", err);
        schoolReservationsMap = {};
      }
      allSlots = applyAllReservations(propositionSlots, publicReservationsMap, schoolReservationsMap);
      if (!publicText) $("jpo-public-error").textContent = "Information : les réservations public n’ont pas pu être lues. Le programme reste affiché.";
      renderControls();
      bind();
      render();
    })
