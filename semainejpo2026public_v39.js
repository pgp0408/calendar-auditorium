document.addEventListener("DOMContentLoaded", function () {
  ...
  let activeFocus = "all";
  let searchTerm = "";
  let schoolPublicStats = emptySchoolPublicStats();
  ...
  root.innerHTML = `
    <style>
      ...
      #jpo-public .stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-bottom:16px}
      #jpo-public .statgroup{border:1px solid var(--border);border-radius:20px;padding:15px;background:#fff;box-shadow:0 10px 24px rgba(15,23,42,.05)}
      #jpo-public .statgroup.publics{border-color:#e9d5ff;background:#faf5ff}
      #jpo-public .statgroup.keys{border-color:#bfdbfe;background:#eff6ff}
      #jpo-public .statgroup.peaks{border-color:#fde68a;background:#fffbeb}
      #jpo-public .statgroup-title{font-size:14px;font-weight:900;text-transform:uppercase;letter-spacing:.02em;margin-bottom:10px;color:#111827}
      #jpo-public .statgrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
      #jpo-public .stat{border:1px solid rgba(15,23,42,.08);border-radius:14px;padding:10px;background:rgba(255,255,255,.74)}
      ...
      #jpo-public .peaklist{margin-top:10px;display:grid;gap:6px;color:#78350f;font-size:12px;font-weight:850;line-height:1.35}
      #jpo-public .peakline{padding-left:9px;border-left:3px solid #fde68a}
      ...
      @media(max-width:840px){#jpo-public .toolbar,#jpo-public .stats{grid-template-columns:1fr}#jpo-public .statgrid{grid-template-columns:1fr 1fr}#jpo-public .row{grid-template-columns:1fr;gap:4px}#jpo-public .head,#jpo-public .content{padding:18px}}
    </style>
    <div class="wrap">
      <header class="head">
        <h2>Journées Portes Ouvertes du Conservatoire Henri Tomasi</h2>
        <p class="sub">Du 15 au 21 juin 2026, le Conservatoire ouvre ses portes au public avec une sélection de concerts, auditions, ateliers ouverts, démonstrations et rencontres.</p>
        <div class="intro"><strong>Bienvenue au Conservatoire.</strong><br>Cette page rassemble les propositions accessibles au grand public, en dehors des créneaux spécifiquement réservés aux établissements scolaires. Familles, futurs élèves, curieux, amateurs de musique, de danse ou de théâtre : vous pouvez venir découvrir librement les rendez-vous proposés tout au long de la semaine.</div>
  ...
  function emptySchoolPublicStats() {
    return { reservedPlaces: 0, uniqueGroups: 0, uniquePeople: 0, reservedSlotCount: 0, confirmedRequests: 0, peakReserved: 0, topSlots: [] };
  }

  function splitReservationIds(value) {
    return String(value || "").split(/[;\n]+/).map(x => x.trim()).filter(Boolean);
  }

  function findReservationIdsInRow(row) {
    const direct = get(row, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux", "ID créneau", "ID creneau"]);
    if (direct) return direct;
    for (const key of Object.keys(row || {})) {
      const value = String(row[key] || "");
      if (/2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(value)) return value;
    }
    return "";
  }

  function parseReservationLine(line) {
    const parts = String(line || "").split("|").map(x => x.trim());
    if (parts.length < 5) return null;
    const id = parts.slice(0, 5).join("|");
    const title = parts[4] || "Créneau réservé";
    let students = 0, adults = 0, group = parts[5] || "Groupe non précisé";
    if (parts.length >= 8) {
      students = numberFrom(parts[parts.length - 2]);
      adults = numberFrom(parts[parts.length - 1]);
      group = parts.slice(5, -2).join("|").trim() || group;
    }
    const total = students + adults;
    return { id, title, group, students, adults, total };
  }

  function buildSchoolPublicStats(rows) {
    const stats = emptySchoolPublicStats();
    const groupMax = {}, uniqueIds = {}, bySlot = {};
    (rows || []).forEach(row => {
      const validation = norm(get(row, ["VALIDATION CRD", "Validation CRD"]));
      if (validation === "non") return;
      if (validation === "oui") stats.confirmedRequests += 1;
      const establishment = get(row, ["Établissement", "Etablissement", "Nom de l’établissement", "Nom de l'etablissement", "Structure", "École", "Ecole", "Nom de l’école", "Nom de l'ecole", "Organisme"]);
      splitReservationIds(findReservationIdsInRow(row)).forEach(line => {
        const parsed = parseReservationLine(line);
        if (!parsed || !parsed.id || !parsed.total) return;
        stats.reservedPlaces += parsed.total;
        uniqueIds[parsed.id] = true;
        const groupKey = norm((establishment || "") + "|" + (parsed.group || ""));
        if (groupKey) groupMax[groupKey] = Math.max(groupMax[groupKey] || 0, parsed.total);
        if (!bySlot[parsed.id]) bySlot[parsed.id] = { id: parsed.id, title: parsed.title, total: 0 };
        bySlot[parsed.id].total += parsed.total;
      });
    });
    stats.uniqueGroups = Object.keys(groupMax).length;
    stats.uniquePeople = Object.values(groupMax).reduce((sum, value) => sum + value, 0);
    stats.reservedSlotCount = Object.keys(uniqueIds).length;
    stats.topSlots = Object.values(bySlot).sort((a, b) => b.total - a.total).slice(0, 3);
    stats.peakReserved = stats.topSlots.length ? stats.topSlots[0].total : 0;
    return stats;
  }

  function renderStatGroup(title, className, items, extraHtml) {
    return '<section class="statgroup ' + esc(className || "") + '"><div class="statgroup-title">' + esc(title) + '</div><div class="statgrid">' +
      items.map(item => '<div class="stat"><div class="num">' + esc(item[0]) + '</div><div class="lab">' + esc(item[1]) + '</div></div>').join("") +
      '</div>' + (extraHtml || "") + '</section>';
  }

  function renderStats(slots) {
    const totalSlots = allSlots.length;
    const publicSlots = allSlots.filter(isPublicVisible).length;
    const schoolOnlySlots = Math.max(0, totalSlots - publicSlots);
    const peaks = (schoolPublicStats.topSlots || []).map(s => '<div class="peakline">' + esc(s.total + ' — ' + s.title) + '</div>').join("");
    $("jpo-public-stats").innerHTML =
      renderStatGroup("Décompte des publics", "publics", [
        [schoolPublicStats.reservedPlaces, "places réservées"],
        [schoolPublicStats.uniqueGroups, "groupes/classes écoles"],
        [schoolPublicStats.uniquePeople, "individus uniques estimés"],
        [schoolPublicStats.reservedSlotCount, "créneaux réservés affichés"]
      ]) +
      renderStatGroup("Autres chiffres clefs", "keys", [
        [totalSlots, "créneaux proposés"],
        [publicSlots, "propositions tout public"],
        [schoolOnlySlots, "propositions dédiées aux scolaires"],
        [schoolPublicStats.confirmedRequests, "réservations confirmées"]
      ]) +
      renderStatGroup("Pics d’affluence", "peaks", [[schoolPublicStats.peakReserved, "personnes réservées au plus fort"]], peaks ? '<div class="peaklist">' + peaks + '</div>' : "");
  }
  ...
  function renderFocus(slots) {
    ...
    $("jpo-public-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Temps forts de la semaine</div><div class="focus-sub">Les rendez-vous à l’auditorium ou dans les grandes salles avec une jauge réelle d’au moins 35 personnes sont mis en avant automatiquement.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
  ...
  Promise.all([
    ...
    .then(([propositionsText, demandesText]) => {
      allSlots = buildSlots(parseCSV(propositionsText));
      schoolPublicStats = buildSchoolPublicStats(parseCSV(demandesText));
      renderControls();
      bind();
      render();
    })
