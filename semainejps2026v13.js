document.addEventListener("DOMContentLoaded", function () {
      ...
      <div class="jps-header">
        <div class="jps-header-grid">
          <div>
            ...
            <p class="jps-sub">V13 consolidée — version interne avec alertes cliquables, planning complet, colonne “Peu importe”, propositions détaillées et programmation automatique.</p>
          </div>
  ...
  function roomInfo(v){ const s=norm(v); if(s.includes("auditorium")) return {key:"auditorium",short:"Aud.",label:"Auditorium",capacity:"200 places"}; if(s.includes("orchestre")) return {key:"orchestre",short:"Orch.",label:"Salle d’orchestre",capacity:"40 places"}; if(s.includes("chant")) return {key:"chant",short:"Chant",label:"Salle de chant",capacity:"40 places"}; if(s.includes("danse")) return {key:"danse",short:"Danse",label:"Studio de danse",capacity:"40 places"}; if(s.includes("theatre") || s.includes("théâtre")) return {key:"theatre",short:"Théâtre",label:"Salle de théâtre",capacity:"40 places"}; if(s.includes("peu importe") || s.includes("indifferent") || s.includes("indifférent") || s.includes("a definir") || s.includes("à définir")) return {key:"any",short:"Peu importe",label:"Peu importe",capacity:"à affecter"}; return {key:"any",short:"Peu importe",label:v || "Peu importe",capacity:"à affecter"}; }
  ...
  function durationToMinutes(duration){ const d=norm(duration); if(d.includes("45")) return 45; if(d.includes("30")) return 30; if(d.includes("1h30") || d.includes("1 h 30")) return 90; if(d.includes("2h") || d.includes("2 h")) return 120; if(d.includes("1h") || d.includes("1 h")) return 60; if(d.includes("demi")) return 180; if(d.includes("journee")) return null; return 60; }
  function eventStartMinutes(ev){ if(ev.allDay) return timeToMinutes("08:30"); return timeToMinutes(ev.time); }
  function eventEndMinutes(ev){ if(ev.autoEnd) return timeToMinutes(ev.autoEnd); if(ev.allDay) return timeToMinutes("21:00"); const start=eventStartMinutes(ev); if(start===null) return null; return start+durationToMinutes(ev.duration); }
  ...
  function renderEventListHTML(events){ if(!events.length) return '<div class="jps-alert-line">Aucune proposition à afficher.</div>'; return '<div class="jps-list-grid">' + events.map(ev => '<div class="jps-list-item '+ev.statusKind+'" data-event-id="'+ev.uid+'"><div class="jps-list-item-title">'+esc(ev.title)+'</div><div class="jps-list-item-meta"><div><strong>Date :</strong> '+esc(formatDateFR(ev.dateIso))+'</div><div><strong>Horaire :</strong> '+esc(ev.allDay ? "Journée entière" : ev.time + (ev.autoEnd ? "–"+ev.autoEnd : ""))+'</div><div><strong>Lieu :</strong> '+esc(ev.roomLabel)+'</div><div><strong>Statut :</strong> '+esc(ev.statusLabel)+'</div>'+(ev.autoGenerated ? '<div><strong>Mode :</strong> Programmation automatique</div>' : '')+'</div></div>').join("") + '</div>'; }
  ...
  function openDayModal(dayIso){ const data=visibleEvents(); const day=DAYS.find(d=>d.iso===dayIso); const dayEvents=data.filter(e=>e.dateIso===dayIso); $("jps-day-modal-title").textContent=day.name+" "+day.short; $("jps-day-modal-sub").textContent="Planning par créneaux de 30 minutes et par salle. Les programmations automatiques apparaissent sur les créneaux générés."; const detail=$("jps-day-detail"); detail.innerHTML=""; const wrap=document.createElement("div"); wrap.className="jps-timetable-wrap"; const table=document.createElement("div"); table.className="jps-timetable"; table.innerHTML='<div class="jps-th">Horaire</div>'+ROOMS.map(room=>'<div class="jps-th">'+esc(room.label)+'<br><span class="jps-count">'+esc(room.capacity)+'</span></div>').join(""); TIME_SLOTS.forEach(slot=>{ const hour=document.createElement("div"); hour.className="jps-hour"; hour.textContent=minutesToTime(slot).replace(":","h"); table.appendChild(hour); ROOMS.forEach(room=>{ const cell=document.createElement("div"); const isSchoolSlot=isSchoolDay(dayIso)&&((slot>=510&&slot<720)||(slot>=780&&slot<1020)); cell.className="jps-cell "+(isSchoolSlot?"schooltime":""); const events=dayEvents.filter(ev=>ev.roomKey===room.key && eventOccupiesSlot(ev,slot)); if(!events.length){ cell.classList.add("empty"); cell.innerHTML='<div class="jps-slot-empty">Libre</div>'; } else { events.forEach(ev=>cell.appendChild(renderEventCard(ev))); } table.appendChild(cell); }); }); wrap.appendChild(table); detail.appendChild(wrap); $("jps-day-modal").classList.add("open"); }
  ...
  fetch(CSV_URL).then(r=>{ if(!r.ok) throw new Error("HTTP "+r.status); return r.text(); }).then(text=>{
    ...
    dataRows.forEach((row,idx)=>{
      const title=row[C_TITLE]||"Projet sans titre"; const status=statusInfo(row[C_STATUS]); const room=roomInfo(row[C_ROOM]); const duration=C_DUR!==-1 ? row[C_DUR] : ""; const autoLabel=C_AUTO!==-1 ? row[C_AUTO] : ""; const autoMode=norm(autoLabel); const baseUid="ev"+idx+"_"+norm(title).replace(/[^a-z0-9]+/g,"-");
      const common={ name:C_NAME!==-1?row[C_NAME]:"", discipline:C_DISC!==-1?row[C_DISC]:"", title, type:C_TYPE!==-1?row[C_TYPE]:"", publicTarget:C_PUBLIC!==-1?row[C_PUBLIC]:"", duration, flexibility:C_FLEX!==-1?row[C_FLEX]:"", alternatives:C_ALT!==-1?row[C_ALT]:"", roomKey:room.key, roomShort:room.short, roomLabel:room.label, roomCapacity:room.capacity, estimatedCapacity:C_CAP!==-1?row[C_CAP]:"", tech:C_TECH!==-1?row[C_TECH]:"", description:C_DESC!==-1?row[C_DESC]:"", statusKind:status.kind, statusLabel:status.label, autoLabel:autoLabel || "Non" };
