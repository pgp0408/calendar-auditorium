document.addEventListener("DOMContentLoaded", function () {
  ...
  const ROOMS = [
    ...
    { key: "any", short: "Peu importe", label: "Peu importe", capacity: "à affecter" }
  ];
  ...
  root.innerHTML = `
    <style>
      ...
      #semainejps-calendar .jps-card{
        max-width:1450px;margin:0 auto;background:#fff;border:1px solid var(--jps-border);
        border-radius:24px;box-shadow:0 18px 50px rgba(15,23,42,.08);overflow:hidden;
      }
      #semainejps-calendar .jps-header{
        padding:32px;
        background:radial-gradient(circle at top right,rgba(96,165,250,.18),transparent 30%),
        radial-gradient(circle at top left,rgba(22,163,74,.13),transparent 28%),#fff;
      ...
      #semainejps-calendar .jps-filter{
        border:1px solid #d1d5db;background:#fff;border-radius:999px;padding:10px 14px;
      ...
      #semainejps-calendar .jps-pill{
        display:inline-flex;align-items:center;gap:8px;border:1px solid var(--jps-border);
        background:#fff;border-radius:999px;padding:8px 12px;font-size:13px;font-weight:800;color:#374151;
      }
      ...
      #semainejps-calendar .jps-alert{
        border:1px solid var(--jps-border);border-radius:16px;padding:13px;background:#f9fafb;
        font-size:13px;line-height:1.35;cursor:pointer;transition:transform .15s ease,box-shadow .15s ease;
      }
      ...
      #semainejps-calendar .jps-alert.highlight{outline:3px solid rgba(124,58,237,.25)}
      ...
      #semainejps-calendar .jps-day-btn{
        appearance:none;border:1px solid #e7eaee;background:#fff;border-radius:20px;padding:18px;
        cursor:pointer;text-align:left;transition:transform .15s ease,box-shadow .15s ease,border-color .15s ease;
      }
      ...
      #semainejps-calendar .jps-day-btn.jps-focus{outline:3px solid rgba(124,58,237,.25)}
      ...
      #semainejps-calendar .jps-frise-day{
        border:1px solid #e7eaee;border-radius:20px;background:#fff;overflow:hidden;cursor:pointer;
      ...
      #semainejps-calendar .jps-action-btn{
        border:0;background:#111827;color:#fff;border-radius:16px;padding:15px 18px;font-weight:900;
        cursor:pointer;box-shadow:0 12px 28px rgba(15,23,42,.12);
      }
      #semainejps-calendar .jps-error{margin-top:16px;color:#b91c1c;font-weight:800;white-space:pre-wrap}
      #semainejps-calendar .jps-modal,.jps-day-modal,.jps-list-modal,.jps-alert-modal{
      ...
      #semainejps-calendar .jps-modalbox,#semainejps-calendar .jps-day-modalbox,#semainejps-calendar .jps-list-modalbox,#semainejps-calendar .jps-alert-modalbox{
        width:min(900px,100%);max-height:90vh;overflow:auto;background:#fff;border-radius:26px;
        padding:28px;position:relative;box-shadow:0 30px 80px rgba(0,0,0,.24);
      }
      ...
      #semainejps-calendar .jps-close,#semainejps-calendar .jps-day-close,#semainejps-calendar .jps-list-close,#semainejps-calendar .jps-alert-close{
        position:absolute;top:12px;right:14px;border:0;background:transparent;font-size:32px;cursor:pointer;
      }
      #semainejps-calendar .jps-modaltitle,#semainejps-calendar .jps-day-modaltitle,#semainejps-calendar .jps-list-modaltitle,#semainejps-calendar .jps-alert-modaltitle{
        margin:0 42px 18px 0;font-size:28px;line-height:1.15;font-weight:900;
      }
      ...
      #semainejps-calendar .jps-timetable{min-width:1120px;display:grid;grid-template-columns:95px repeat(6,minmax(155px,1fr))}
      ...
      #semainejps-calendar .jps-slot-card.permanent{background:#7c3aed;color:#fff}
      ...
      @media(max-width:760px){
        ...
        #semainejps-calendar .jps-titlebar{flex-direction:column;align-items:flex-start}
        #semainejps-calendar .jps-row{grid-template-columns:1fr;gap:4px}
        #semainejps-calendar .jps-modal,#semainejps-calendar .jps-day-modal,#semainejps-calendar .jps-list-modal,#semainejps-calendar .jps-alert-modal{padding:10px}
        #semainejps-calendar .jps-modalbox,#semainejps-calendar .jps-day-modalbox,#semainejps-calendar .jps-list-modalbox,#semainejps-calendar .jps-alert-modalbox{padding:20px;border-radius:20px}
      }
    </style>
    ...
    <div class="jps-card">
      <div class="jps-header">
        <div class="jps-header-grid">
          <div>
            ...
            <p class="jps-sub">
              Tableau de pilotage de la semaine inaugurale. Les alertes sont cliquables, le planning inclut une colonne “Peu importe” et les propositions détaillées s’ouvrent désormais en popup.
            </p>
          </div>
      ...
      <div class="jps-content">
        ...
        <section class="jps-section">
          <div class="jps-titlebar">
            <div>
              ...
              <p>Lundi, mardi, jeudi et vendredi : uniquement 08h30–12h00 et 13h00–17h00.</p>
            </div>
        ...
        <section class="jps-section">
          <div class="jps-titlebar">
            <div>
              ...
              <p>Lecture synthétique des journées tout public.</p>
            </div>
        ...
        <section class="jps-section">
          <button type="button" id="jps-open-list" class="jps-action-btn">Voir les propositions détaillées</button>
        </section>
      </div>
    </div>
  ...
  function parseCSVLine(line){
    ...
    for(let i=0;i<line.length;i++){
      ...
      else if(ch === "," && !quoted){ res.push(cur); cur=""; }
      else cur += ch;
    }
  ...
  function findCol(headers, possibilities){
    ...
    for(const p of possibilities){ const n=norm(p); const exact=hs.findIndex(h=>h===n); if(exact!==-1) return exact; }
    for(const p of possibilities){ const n=norm(p); const partial=hs.findIndex(h=>h.includes(n)); if(partial!==-1) return partial; }
    return -1;
  }
  ...
  function formatTime(v){ if(!v) return "Non précisée"; return v.toString().trim().slice(0,5); }
  ...
  function roomInfo(v){
    ...
    if(s.includes("peu importe") || s.includes("indifferent") || s.includes("indifférent") || s.includes("a definir") || s.includes("à définir")) return {key:"any",short:"Peu importe",label:"Peu importe",capacity:"à affecter"};
    return {key:"any",short:"Peu importe",label:v || "Peu importe",capacity:"à affecter"};
  }
  ...
  function timeToMinutes(time){
    ...
    const parts=time.split(":"); if(parts.length<2) return null;
  ...
  function eventEndMinutes(ev){ if(ev.allDay) return timeToMinutes("21:00"); const start=eventStartMinutes(ev); if(start===null) return null; return start+durationToMinutes(ev.duration); }
  ...
  function isPermanentExpo(ev){
    const title=norm(ev.title), room=norm(ev.roomLabel);
    return ev.allDay && (title.includes("cuivre") || room.includes("chant"));
  }

  function isRealProgramming(ev){ return !isPermanentExpo(ev) && ev.statusKind !== "refused"; }

  function isSchoolTime(ev){
    if(!isSchoolDay(ev.dateIso)) return false;
    if(ev.allDay) return !isPermanentExpo(ev);
    const minutes=timeToMinutes(ev.time);
    if(minutes===null) return false;
    return (minutes>=510 && minutes<720) || (minutes>=780 && minutes<1020);
  }
  ...
  function schoolSlotKey(ev){
    ...
    if(minutes>=510 && minutes<720) return "matin";
    if(minutes>=780 && minutes<1020) return "apresmidi";
    return null;
  }
  ...
  function visibleEvents(){
    ...
    return allEvents.filter(e=>selected.includes(e.statusKind)).sort((a,b)=>{
      ...
      return (eventStartMinutes(a)||0)-(eventStartMinutes(b)||0);
    });
  }
  ...
  function getConflictEvents(events){
    const map={};
    events.filter(isRealProgramming).forEach(ev=>{
      if(ev.allDay) return;
      const key=ev.dateIso+"|"+ev.time+"|"+ev.roomKey;
  ...
  function renderAlerts(data){
    ...
    DAYS.filter(d=>d.group==="school").forEach(day=>{
      getEmptySchoolSlotsForDay(day.iso,data).forEach(slot=>emptySchoolSlots.push({day,slot}));
    });
    const conflicts=getConflictEvents(realData);
    const permanentCount=data.filter(isPermanentExpo).length;

    lastAlertData={emptySchoolSlots,schoolTimeEvents,outOfSchoolEvents,conflicts};

    $("jps-alerts").innerHTML =
      '<div id="jps-alert-empty" class="jps-alert ' + (emptySchoolSlots.length ? "empty" : "ok") + '"><strong>Créneaux scolaires non comblés</strong>' +
      (emptySchoolSlots.length ? emptySchoolSlots.length + " créneau(x) à traiter." : "Tous les créneaux scolaires sont couverts.") + '</div>' +
      '<div id="jps-alert-school" class="jps-alert ' + (schoolTimeEvents.length < 4 ? "low" : "ok") + '"><strong>Temps scolaire réel</strong>' +
      schoolTimeEvents.length + ' proposition(s) entre 08h30–12h00 / 13h00–17h00.</div>' +
      '<div id="jps-alert-public" class="jps-alert ok"><strong>Hors temps scolaire / tout public</strong>' +
      outOfSchoolEvents.length + ' proposition(s) identifiée(s).</div>' +
      '<div id="jps-alert-conflicts" class="jps-alert ' + (conflicts.length ? "low" : "ok") + '"><strong>Litiges / chevauchements</strong>' +
      (conflicts.length ? conflicts.length + ' proposition(s) concernée(s).' : "Aucun litige majeur détecté.") +
      '<br>Fil rouge : ' + permanentCount + '</div>';
  ...
  function renderEmptySlotsContent(slots){
    if(!slots.length) return '<div class="jps-alert-line">Tous les créneaux scolaires sont couverts.</div>';
    return '<div class="jps-alert-list">' + slots.map(s =>
      '<div class="jps-alert-line"><strong>' + esc(s.day.name + " " + s.day.short) + '</strong><br>Créneau à combler : ' + esc(s.slot) + '</div>'
    ).join("") + '</div>';
  }

  function renderEventListHTML(events){
    if(!events.length) return '<div class="jps-alert-line">Aucune proposition à afficher.</div>';
    return '<div class="jps-list-grid">' + events.map(ev =>
      '<div class="jps-list-item ' + ev.statusKind + '" data-event-id="' + ev.uid + '">' +
  ...
  function openAlertModal(title, html){
    $("jps-alert-title").textContent=title;
    $("jps-alert-content").innerHTML=html;
    $("jps-alert-modal").classList.add("open");
    $("jps-alert-content").querySelectorAll("[data-event-id]").forEach(el=>{
      el.onclick=()=>{ const ev=allEvents.find(e=>e.uid===el.getAttribute("data-event-id")); if(ev) openModal(ev); };
    });
  }
  ...
  function slotSummary(dayIso,momentKey,data,mode){
    let events;
    if(mode==="school"){
      events=data.filter(e=>e.dateIso===dayIso && isSchoolTime(e) && (schoolSlotKey(e)===momentKey || schoolSlotKey(e)==="journee"));
    } else {
    ...
    let scoreClass="empty", scoreText="Vide";
    if(realEvents.length===1){scoreClass="low";scoreText="1 action";}
    else if(realEvents.length===2){scoreClass="ok";scoreText="2 actions";}
    else if(realEvents.length>=3){scoreClass="full";scoreText=realEvents.length+" actions";}
    else if(permanentEvents.length){scoreClass="low";scoreText="Fil rouge";}
  ...
  function renderFrise(containerId,group,data){
    ...
    DAYS.filter(day=>day.group===group).forEach(day=>{
      ...
      dayBox.innerHTML='<div class="jps-frise-head"><div class="jps-frise-name">'+esc(day.name)+'</div><div class="jps-frise-date">'+esc(day.short)+'</div></div><div class="jps-frise-body"></div>';
      ...
      moments.forEach(moment=>{
        ...
        slot.innerHTML='<div class="jps-frise-slothead"><span class="jps-frise-slotname">'+esc(moment.label)+'</span><span class="jps-frise-score '+summary.scoreClass+'">'+esc(summary.scoreText)+'</span></div>' +
          (moment.period ? '<div class="jps-count" style="margin-bottom:7px">'+esc(moment.period)+'</div>' : '') + '<div class="jps-roomchips"></div>';
        const chips=slot.querySelector(".jps-roomchips");
        if(!summary.events.length){ chips.innerHTML='<span class="jps-roomchip empty">Aucune salle mobilisée</span>'; }
        else{
          const byRoom={};
          summary.events.forEach(ev=>{ if(!byRoom[ev.roomKey]) byRoom[ev.roomKey]=[]; byRoom[ev.roomKey].push(ev); });
          Object.keys(byRoom).forEach(roomKey=>{
            const evs=byRoom[roomKey], main=evs[0];
      ...
      dayBox.onclick=()=>openDayModal(day.iso);
  ...
  function renderDayButtons(data){
    ...
    DAYS.forEach(day=>{
      ...
      let warningClass="ok", warningText="Programmation à suivre";
      if(conflicts){ warningClass="conflict"; warningText=conflicts+" litige(s) salle / horaire"; }
      else if(isSchoolDay(day.iso)&&emptySchoolSlots.length){ warningClass="empty"; warningText="Créneau(x) scolaire(s) à combler : "+emptySchoolSlots.join(", "); }
      else if(isSchoolDay(day.iso)){ warningText=schoolEvents.length+" proposition(s) sur temps scolaire"; }
      else if(!realEvents.length){ warningClass="empty"; warningText="À combler hors expo permanente"; }
      else { warningText=realEvents.length+" proposition(s) hors fil rouge"; }
      const btn=document.createElement("button");
      btn.type="button";
      btn.className="jps-day-btn";
      btn.setAttribute("data-day",day.iso);
      btn.innerHTML='<div class="jps-day-btn-top"><div><div class="jps-day-btn-name">'+esc(day.name)+'</div><div class="jps-day-btn-date">'+esc(day.short)+'</div></div><span class="jps-day-btn-count">'+dayEvents.length+'</span></div>' +
        '<div class="jps-day-btn-meta"><strong>Salles :</strong> '+esc(rooms.length ? rooms.join(", ") : "aucune")+'<br><strong>Fil rouge :</strong> '+permanent.length+' · <strong>Hors fil rouge :</strong> '+realEvents.length+'</div>' +
        '<div class="jps-day-btn-warning '+warningClass+'">'+esc(warningText)+'</div>';
      btn.onclick=()=>openDayModal(day.iso);
  ...
  function renderEventCard(ev){
    ...
    const timeLabel=ev.allDay ? "Créneau disponible" : ev.time + (end ? "–"+minutesToTime(end) : "");
    div.innerHTML='<span class="jps-slot-time">'+esc(timeLabel)+'</span><span class="jps-slot-title">'+esc(ev.title)+'</span><span class="jps-slot-meta">'+esc(ev.statusLabel)+(isPermanentExpo(ev) ? " · Fil rouge" : "")+'</span>';
    div.onclick=e=>{ e.stopPropagation(); openModal(ev); };
    return div;
  }
  ...
  function openDayModal(dayIso){
    ...
    $("jps-day-modal-sub").textContent="Planning par créneaux de 30 minutes et par salle. La colonne « Peu importe » regroupe les propositions sans salle affectée.";
    ...
    table.innerHTML='<div class="jps-th">Horaire</div>'+ROOMS.map(room=>'<div class="jps-th">'+esc(room.label)+'<br><span class="jps-count">'+esc(room.capacity)+'</span></div>').join("");
    TIME_SLOTS.forEach(slot=>{
      ...
      ROOMS.forEach(room=>{
        ...
        if(!events.length){ cell.classList.add("empty"); cell.innerHTML='<div class="jps-slot-empty">Libre</div>'; }
        else events.forEach(ev=>cell.appendChild(renderEventCard(ev)));
  ...
  function getListEvents(data){
    const seenPermanent={}; const result=[];
    data.forEach(ev=>{
  ...
  function openListModal(){
    const data=getListEvents(visibleEvents());
    const byDate={};
    data.forEach(ev=>{
      const key=isPermanentExpo(ev) ? "permanent" : ev.dateIso;
    ...
    $("jps-list-content").innerHTML=keys.map(key=>{
      const title=key==="permanent" ? "Fil rouge / toute la semaine" : formatDateFR(key);
      return '<div class="jps-list-date">'+esc(title)+'</div>' + renderEventListHTML(byDate[key]);
    }).join("");
    $("jps-list-modal").classList.add("open");
    $("jps-list-content").querySelectorAll("[data-event-id]").forEach(el=>{
      el.onclick=()=>{ const ev=allEvents.find(e=>e.uid===el.getAttribute("data-event-id")); if(ev) openModal(ev); };
    });
  }
  ...
  $("jps-close").onclick=()=>close("jps-modal");
  $("jps-modal").onclick=e=>{ if(e.target===$("jps-modal")) close("jps-modal"); };
  $("jps-day-close").onclick=()=>close("jps-day-modal");
  $("jps-day-modal").onclick=e=>{ if(e.target===$("jps-day-modal")) close("jps-day-modal"); };
  $("jps-list-close").onclick=()=>close("jps-list-modal");
  $("jps-list-modal").onclick=e=>{ if(e.target===$("jps-list-modal")) close("jps-list-modal"); };
  $("jps-alert-close").onclick=()=>close("jps-alert-modal");
  $("jps-alert-modal").onclick=e=>{ if(e.target===$("jps-alert-modal")) close("jps-alert-modal"); };
  ...
  fetch(CSV_URL)
    ...
    .then(text=>{
      ...
      allEvents=dataRows.map((row,idx)=>{
      ...
      refresh();
      if(!allEvents.length) $("jps-error").textContent="Aucune proposition trouvée entre le 15 et le 21 juin 2026.";
    })
