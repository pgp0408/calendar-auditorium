document.addEventListener("DOMContentLoaded", function () {
  ...
  const STORAGE_KEY = "jps_ecoles_v24_panier";
  const LAST_GROUP_KEY = "jps_ecoles_v24_last_group";
  ...
  function renderStartWindowGroup(group) {
    ...
    const subtitle = "Activités qui commencent dans cette demi-heure. L’horaire exact est indiqué sur chaque carte.";

    return (
      '<section class="timegroup window">' +
        '<div class="timegroup-head">' +
          '<div>' +
            '<div class="timegroup-title">Départs ' + esc(label) + '</div>' +
            '<div class="timegroup-sub">' + esc(subtitle) + '</div>' +
            (group.mixedStarts ? '<div class="window-note">Horaires proches regroupés pour comparer les choix</div>' : '') +
          '</div>' +
  ...
  function render() {
    ...
    $("jps-days").innerHTML =
      '<section class="day">' +
        ...
        '</div>' +
        '<div class="daybody">' +
          '<p class="choice-hint">Mode d’emploi : les propositions sont regroupées par demi-heure de départ. Cela permet de comparer facilement les choix proches, sans créer de grands blocs illisibles. L’horaire exact reste indiqué sur chaque carte.</p>' +
