document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <div class="intro"><strong>Logique retenue :</strong> auditorium affiché systématiquement ; autres créneaux affichés uniquement lorsque le cumul des publics réservés atteint au moins ${AFFLUENCE_THRESHOLD} personnes sur le même horaire.</div>
      </header>
      <main class="content">
        ...
        <div class="toolbar">
          ...
          <div><label>Affichage</label><select id="jpo-partners-mode"><option value="all">Auditorium + affluence</option><option value="auditorium">Auditorium uniquement</option><option value="affluence">Public réservé uniquement</option></select></div>
        </div>
  ...
  function buildReservations(text){
    ...
    const C_IDS=findCol(h,["IDs créneaux","IDs creneaux","IDS créneaux","IDS creneaux","ID créneaux","ID creneaux"]);
    const C_VALID=findCol(h,["VALIDATION CRD","Validation CRD","Statut CRD","STATUT CRD","Statut de la demande","Validation","Statut"]);
    const C_ETAB=findCol(h,["Établissement","Etablissement","Nom de l’établissement","Nom de l'etablissement","Structure","École","Ecole","Organisme"]);
    data.forEach(r=>{
      const valid=C_VALID!==-1?norm(r[C_VALID]):"";
      // Pour cette page partenaires, le chiffre attendu est le public reserve/positionne.
      // On exclut seulement les demandes explicitement refusees, annulees ou non retenues.
      if(valid && (valid.includes("refus") || valid.includes("annul") || valid === "non" || valid.includes("non reten")))return;
  ...
  function applyReservations(slots,map){return slots.map(s=>{let res=map[s.id]; if(!res){res={confirmed:0,details:[]}; const prefix=[s.dateIso,compact(s.start),compact(s.end),s.roomKey].join("|")+"|"; Object.keys(map).forEach(id=>{if(id.indexOf(prefix)===0){res.confirmed+=map[id].confirmed;(map[id].details||[]).forEach(d=>res.details.push(d));}});} const extra=(res.details||[]).map(d=>(d.establishment||"")+" "+(d.group||"")).join(" "); return {...s,confirmed:res.confirmed||0,details:res.details||[],search:norm(s.search+" "+extra)};});}
  ...
  function renderStats(slots){const aud=allSlots.filter(s=>s.roomKey==="auditorium").length; const aff=allSlots.filter(s=>confirmedAtMoment(s)>=AFFLUENCE_THRESHOLD).length; const max=allSlots.reduce((m,s)=>Math.max(m,confirmedAtMoment(s)),0); $("jpo-partners-stats").innerHTML=[[slots.length,"repères affichés"],[aud,"événements auditorium"],[aff,"créneaux fréquentés"],[max,"pic confirmé"]].map(x=>'<div class="stat"><div class="num">'+x[0]+'</div><div class="lab">'+esc(x[1])+'</div></div>').join("");}
  function renderCard(s){const aff=confirmedAtMoment(s);return '<article class="card '+(isRepere(s)?'strong':'')+'"><div class="top"><div class="chips">'+(s.roomKey==="auditorium"?'<span class="chip gold">Auditorium</span>':'')+(aff>=AFFLUENCE_THRESHOLD?'<span class="chip green">Public réservé</span>':'')+'<span class="chip blue">'+esc(formatDate(s.dateIso))+'</span><span class="chip">'+esc(showTime(s.start))+'</span></div><div class="title">'+esc(s.title)+'</div>'+(s.description?'<div class="desc">'+esc(s.description)+'</div>':'')+'</div><div class="cardbody"><strong>'+esc(s.roomLabel)+'</strong><br>'+esc(s.type||"Proposition")+(aff?'<br><strong>Public réservé sur ce créneau : '+aff+' personne(s)</strong>':'')+'<br>Repère partenaire — aucune réservation depuis cette page</div></article>';}
