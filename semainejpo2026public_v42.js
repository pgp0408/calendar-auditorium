(function () {
  ...
  function renderStats(slots) {
    const totalReserved = slots.reduce((sum, s) => sum + Number(s.totalReserved || 0), 0);
    const reservedSlots = slots.filter(s => Number(s.totalReserved || 0) > 0).length;
    ...
    const visibleSchoolKeys = {};
    slots.forEach(slot => {
      const keys = schoolKeysForPublicSlot(slot);
      Object.keys(keys).forEach(k => { visibleSchoolKeys[keys[k]] = true; });
    });
    const schoolGroups = {};
    (schoolDebug || []).forEach((item, index) => {
      ...
      const matchesVisibleSlot = Object.keys(keys).some(k => visibleSchoolKeys[keys[k]]);
      if (!matchesVisibleSlot) return;
    ...
    const schoolTakenFallback = slots.reduce((sum, s) => sum + Number(s.schoolReserved || 0), 0);
    const uniqueWithSchools = publicUniqueEstimate(slots) + (schoolUnique || schoolTakenFallback);
    const schoolGroupCount = Object.keys(schoolGroups).length || slots.filter(s => Number(s.schoolReserved || 0) > 0).length;
  ...
  Promise.all([
    ...
    .then(([propText, publicText, schoolText]) => {
      ...
      schoolConfirmedReservationsCount = (schoolRows || []).filter(row => schoolReservationStatus(get(row, ["VALIDATION CRD", "Validation CRD", "STATUT CRD", "Statut CRD", "Statut"])) === "confirmed").length;
