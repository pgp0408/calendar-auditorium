document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <p class="sub">Une page de repérage pour identifier les meilleurs moments de venue : événements à l’auditorium et créneaux où l’affluence confirmée est importante dans le Conservatoire, y compris les publics scolaires.</p>
        <div class="intro"><strong>Logique retenue :</strong> auditorium affiché systématiquement ; autres créneaux affichés uniquement lorsque le cumul des publics réservés atteint au moins ${AFFLUENCE_THRESHOLD} personnes sur le même horaire. Le calcul se fait à partir des réservations, pas des jauges théoriques.</div>
      </header>
      <main class="content">
  ...
  function buildReservations(text){
    ...
    const C_VALID=findCol(h,["VALIDATION CRD","Validation CRD","Statut CRD","STATUT CRD"]);
    const C_ETAB=findCol(h,["Établissement","Etablissement","Structure","École","Ecole","Organisme"]);
    data.forEach(r=>{
      const valid=C_VALID!==-1?norm(r[C_VALID]):"";
      if(valid && (valid.includes("non") || valid.includes("refus") || valid.includes("annul"))) return;
      ...
      String(ids||"").split(";").map(x=>x.trim()).filter(Boolean).forEach(line=>{
        ...
        const students=num(p[6]); const adults=num(p[7]); const total=students+adults;
        if(!id||!total)return;
        if(!map[id])map[id]={confirmed:0,details:[]};
        map[id].confirmed+=total;
        map[id].details.push({establishment:C_ETAB!==-1?r[C_ETAB]:"",group:p[5]||"Groupe",students,adults,total});
      });
    }); return map;
  }

  function idParts(id){const p=String(id||"").split("|"); if(p.length<4)return null; return {dateIso:p[0],start:timeFromCompact(p[1]),end:timeFromCompact(p[2]),roomKey:p[3]};}
  function timeFromCompact(v){const s=String(v||"").replace(/[^0-9]/g,"").padStart(4,"0").slice(0,4); return s.slice(0,2)+":"+s.slice(2,4);}
  function reservationOverlapsSlot(id,slot){const p=idParts(id); if(!p || p.dateIso!==slot.dateIso)return false; const as=mins(slot.start),ae=mins(slot.end),bs=mins(p.start),be=mins(p.end); return as!==null&&ae!==null&&bs!==null&&be!==null&&as<be&&bs<ae;}
  function applyReservations(slots,map){return slots.map(s=>{let res=map[s.id]||{confirmed:0,details:[]}; const extra=(res.details||[]).map(d=>(d.establishment||"")+" "+(d.group||"")).join(" "); return {...s,confirmed:res.confirmed||0,details:res.details||[],search:norm(s.search+" "+extra)};});}
  function confirmedAtMoment(slot){return Object.keys(reservationMapGlobal).reduce((sum,id)=> reservationOverlapsSlot(id,slot) ? sum+(reservationMapGlobal[id].confirmed||0) : sum, 0);}
  ...
  ]).then(([propText,demText])=>{const slots=buildSlots(propText); const reservations=demText?buildReservations(demText):{}; allSlots=applyReservations(slots,reservations); reservationMapGlobal=reservations; window.JPO_PARTENAIRES_DEBUG={slots:allSlots,reservations,confirmedAtMoment}; bind(); render();}).catch(err=>{$("jpo-partners-status").className="error"; $("jpo-partners-status").textContent="Erreur de chargement : "+err.message;});
});
