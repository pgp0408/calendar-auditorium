(function () {
  ...
  function buildPublicReservations(rows) {
    const map = {};
    rows.forEach(row => {
      const id = get(row, ["ID créneau", "ID creneau", "Id créneau", "Id creneau"]);
  ...
  function publicFormUrl(slot) {
    const params = [];
    function add(key, value) {
      params.push(encodeURIComponent(key) + "=" + encodeURIComponent(value || ""));
    }
    add(PUBLIC_FORM_ENTRIES.places, "1");
    add(PUBLIC_FORM_ENTRIES.evenement, slot.title);
    add(PUBLIC_FORM_ENTRIES.date, formatDate(slot.dateIso));
    add(PUBLIC_FORM_ENTRIES.horaire, showTime(slot.start) + (slot.end ? "-" + showTime(slot.end) : ""));
    add(PUBLIC_FORM_ENTRIES.lieu, slot.roomLabel);
    add(PUBLIC_FORM_ENTRIES.idCreneau, slot.reservationId || slotTechnicalId(slot));
  ...
  function renderStats(slots) {
    ...
    const publicRemaining = slots.reduce((sum, s) => sum + Number(s.publicRemaining || 0), 0);
  ...
  function renderCard(slot) {
    ...
    const reserveAction = slot.publicFull
      ...
      : '<a class="btn2" href="' + esc(publicFormUrl(slot)) + '" target="_blank" rel="noopener">Réserver</a>';
  ...
  function openSlot(id) {
    ...
    $("jpo-public-modal-body").innerHTML =
      ...
      (s.publicFull ? rowHtml("Réservation", "Créneau complet") : '<div class="actions" style="margin-top:16px"><a class="btn" href="' + esc(publicFormUrl(s)) + '" target="_blank" rel="noopener">Réserver ce créneau</a></div>');
  ...
  Promise.all([
    ...
    .then(([propText, publicText]) => {
      ...
      allSlots = applyPublicReservations(buildSlots(parseCSV(propText)), publicReservationsMap);
      renderControls();
      bind();
      render();
    })
