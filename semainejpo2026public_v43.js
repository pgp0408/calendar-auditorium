(function () {
  // V42 : version complète prête à copier-coller. Identique à V41 côté logique, encards publics demandés intégrés.
  ...
  let searchTerm = "";
  let schoolConfirmedReservationsCount = 0;
  let teacherSlotCountTotal = 0;
  ...
  root.innerHTML = `
    <style>
      ...
      #jpo-public .statgroup{display:flex;flex-direction:column;gap:10px;min-height:154px}
      #jpo-public .stat-title{font-size:13px;font-weight:950;text-transform:uppercase;letter-spacing:.02em;color:#111827}
      #jpo-public .stat-main{display:flex;align-items:baseline;gap:8px}.stat-main .num{font-size:34px}.stat-main span{font-size:12px;color:#64748b;font-weight:900;line-height:1.2}
      #jpo-public .mini{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.mini div{border:1px solid #e5e7eb;background:#f8fafc;border-radius:12px;padding:8px}.mini b{display:block;font-size:18px;color:#111827}.mini span{display:block;margin-top:2px;font-size:11px;color:#64748b;font-weight:900;line-height:1.2}
      #jpo-public .peaklist{display:grid;gap:6px}.peakbtn{border:1px solid #fde68a;background:#fffbeb;color:#78350f;border-radius:12px;padding:8px;text-align:left;font-size:12px;font-weight:900;cursor:pointer}.peakbtn:hover{background:#fef3c7}
      ...
      #jpo-public .toolbar{display:grid;grid-template-columns:1.4fr .9fr .9fr 1fr 1fr 1fr;gap:10px;margin-bottom:18px;align-items:end}
      ...
      @media(max-width:840px){#jpo-public .toolbar,#jpo-public .stats{grid-template-columns:1fr}#jpo-public .row{grid-template-columns:1fr;gap:4px}#jpo-public .head,#jpo-public .content{padding:18px}#jpo-public .cartbar{bottom:10px;width:calc(100vw - 20px);padding:12px;border-radius:18px}#jpo-public .cartrow{align-items:flex-start}#jpo-public .cartrow .actions{width:100%}#jpo-public .cartrow .actions .btn,#jpo-public .cartrow .actions .btn2{flex:1;justify-content:center}}
    </style>
    <div class="wrap">
      <header class="head">
      ...
      <main class="content">
        ...
        <div class="toolbar">
          ...
          <div class="tool"><label>Demi-journée</label><select id="jpo-public-period"><option value="all">Toute la journée</option><option value="morning">Matin</option><option value="afternoon">Après-midi</option><option value="evening">Soirée</option></select></div>
          ...
          <div class="tool"><label>Proposition automatique</label><select id="jpo-public-auto"><option value="">Composer une idée</option><option value="60">Idée 1h</option><option value="90">Idée 1h30</option><option value="120">Idée 2h</option></select></div>
        </div>
  ...
  function compactTime(v) {
    ...
    const four = raw.match(/^(\d{1,2})(\d{2})$/);
    if (four) return four[1].padStart(2, "0") + four[2];
    const m = minutes(v);
    if (m === null) return raw.replace(/[^0-9]/g, "").padStart(4, "0").slice(0, 4);
    return String(Math.floor(m / 60)).padStart(2, "0") + String(m % 60).padStart(2, "0");
  }

  function get(row, names) {
    // Lecture tolérante des colonnes CSV : accents, casse, espaces et variantes.
    if (!row || !names) return "";
  ...
  function durationToMinutes(v) {
    const d = norm(v);
    if (!d) return 60;
    if (d.includes("1h30") || d.includes("1 h 30") || d.includes("90")) return 90;
    if (d.includes("2h") || d.includes("2 h") || d.includes("120")) return 120;
  ...
  function durationMinutes(v, format) {
    // Même logique que l’outil écoles : FORMAT CRD peut imposer une durée bloquée.
    const f = norm(format);
    if (f.includes("bloque 30") || f.includes("bloquee 30")) return 30;
    if (f.includes("bloque 1h") || f.includes("bloque 1 h") || f.includes("bloquee 1h") || f.includes("bloquee 1 h")) return 60;
    return durationToMinutes(v);
  }
  ...
  function makeSlot(row, forcedDate, forcedStart, forcedEnd, autoLabel, indexSuffix) {
    ...
    let retainedRoom = get(row, ["SALLE RETENUE CRD", "Salle retenue CRD", "Salle retenue"]);
    const precision = get(row, ["PRÉCISION SALLE / LIEU CRD", "PRECISION SALLE / LIEU CRD", "Précision salle", "Precision salle"]);
    const retainedNorm = norm(retainedRoom);
    if (!retainedRoom || retainedNorm.includes("arbitrer") || retainedNorm.includes("definir") || retainedNorm.includes("définir")) {
      retainedRoom = get(row, ["Lieu souhaité", "Lieu", "Salle"]);
    }
    ...
    let capacity = explicitCapacity || (ROOM_DEFAULTS[key] ? ROOM_DEFAULTS[key].capacity : 35);
    if (key === "auditorium") capacity = Math.min(capacity || 180, 180);
    else if (["orchestre", "chant", "choeur", "theatre", "danse1", "danse2", "danse3", "other", "any"].includes(key)) capacity = Math.min(capacity || 35, 35);
  ...
  function normalizeReservationId(raw) {
    const parts = String(raw || "").split("|").map(x => String(x || "").trim());
    if (parts.length < 5) return "";
    const date = parts[0];
    const start = compactTime(parts[1]);
    const end = compactTime(parts[2]);
    const room = parts[3];
    const title = slug(parts[4]);
    if (!date || !start || !end || !room || !title) return "";
    return [date, start, end, room, title].join("|");
  }

  function buildPublicReservations(rows) {
    const map = {};
    rows.forEach(row => {
      const rawId = get(row, ["ID créneau", "ID creneau", "Id créneau", "Id creneau", "IDs créneaux", "IDs creneaux"]);
      const status = publicReservationStatus(get(row, ["STATUT CRD", "Statut CRD", "STATUT", "Statut"]));
      if (!rawId || status === "cancelled") return;
      const places = numberFrom(get(row, ["Nombre de places souhaitées", "Nombre de places souhaitees", "Nombre de places", "Places", "Places souhaitées", "Places souhaitees"]));
      if (!places) return;
      const ids = splitReservationIds(rawId).map(normalizeReservationId).filter(Boolean);
      if (!ids.length) return;
      const detail = {
        name: get(row, ["Nom et prénom", "Nom", "Nom prenom"]),
        email: get(row, ["Adresse email", "Adresse e-mail", "Email"]),
  ...
  function schoolReservationStatus(v) {
    const s = norm(v || "");
    if (s === "non" || s.includes("refus") || s.includes("annul")) return "cancelled";
    if (s === "oui" || s.includes("confirm") || s.includes("valid")) return "confirmed";
    return "pending";
  }

  function normalizeIdParts(raw) {
    const parts = String(raw || "").split("|").map(x => String(x || "").trim());
    ...
    return { date, start, end, room, title, exact: [date, start, end, room, title].join("|"), rawParts: parts };
  }

  function countSchoolPeopleFromId(raw, row) {
    const parts = String(raw || "").split("|").map(x => String(x || "").trim());
    let students = numberFrom(parts[6] || "");
    let adults = numberFrom(parts[7] || "");
    if (students || adults) return students + adults;

    if (row) {
      const rowStudents = numberFrom(get(row, ["Nombre d’élèves", "Nombre d'eleves", "Nombre d'élèves", "Élèves", "Eleves", "Participants", "Nombre de participants"]));
      const rowAdults = numberFrom(get(row, ["Accompagnateurs", "Adultes", "Nombre d’accompagnateurs", "Nombre d'accompagnateurs"]));
      if (rowStudents || rowAdults) return rowStudents + rowAdults;
    }

    const nums = [];
    for (let i = parts.length - 1; i >= 5; i--) {
      const n = numberFrom(parts[i]);
      if (n > 0) nums.push(n);
  ...
  function emptySchoolCount() {
    return { confirmed: 0, pending: 0, total: 0 };
  }

  function addSchoolReservation(map, key, status, total) {
    if (!key || !total) return;
    if (!map[key]) map[key] = emptySchoolCount();
    if (status === "confirmed") map[key].confirmed += total;
    else map[key].pending += total;
    map[key].total += total;
  }

  function buildSchoolReservationsFromPublicCSV(rows) {
    const maps = { exact: {}, dateStartRoomTitle: {}, dateStartTitle: {}, dateStartRoom: {}, debug: [] };
    (rows || []).forEach(row => {
      try {
        const status = schoolReservationStatus(get(row, ["VALIDATION CRD", "Validation CRD", "STATUT CRD", "Statut CRD", "Statut"]));
        if (status === "cancelled") return;
        let rawIds = get(row, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux", "ID créneau", "ID creneau", "IDs des créneaux", "IDs des creneaux"]);
        if (!rawIds) {
          rawIds = Object.keys(row || {}).map(k => row[k]).find(cell => /2026-06-\d{2}\|/.test(String(cell || ""))) || "";
        }
        splitReservationIds(rawIds).forEach(rawId => {
          const info = normalizeIdParts(rawId);
          if (!info) return;
          const total = countSchoolPeopleFromId(rawId, row);
          if (!total) return;
          addSchoolReservation(maps.exact, info.exact, status, total);
          addSchoolReservation(maps.dateStartRoomTitle, [info.date, info.start, info.room, info.title].join("|"), status, total);
          addSchoolReservation(maps.dateStartTitle, [info.date, info.start, info.title].join("|"), status, total);
          addSchoolReservation(maps.dateStartRoom, [info.date, info.start, info.room].join("|"), status, total);
          maps.debug.push({ id: info.exact, status, total, raw: rawId });
        });
      } catch (err) {
        // Une ligne de réservation scolaire mal formée ne doit jamais casser le public.
      }
    });
    return maps;
  }

  function schoolKeysForPublicSlot(slot) {
    const date = slot.dateIso || "";
    const start = compactTime(slot.start || "");
    const end = compactTime(slot.end || "");
    const room = slot.roomKey || "";
    const title = slug(slot.title || "");
    return {
      exact: [date, start, end, room, title].join("|"),
      dateStartRoomTitle: [date, start, room, title].join("|"),
      dateStartTitle: [date, start, title].join("|"),
      dateStartRoom: [date, start, room].join("|")
    };
  }

  function schoolReservationForPublicSlot(slot, maps) {
    if (!slot || !maps) return emptySchoolCount();
    const k = schoolKeysForPublicSlot(slot);
    return (maps.exact && maps.exact[k.exact]) ||
      (maps.dateStartRoomTitle && maps.dateStartRoomTitle[k.dateStartRoomTitle]) ||
      (maps.dateStartTitle && maps.dateStartTitle[k.dateStartTitle]) ||
      (maps.dateStartRoom && maps.dateStartRoom[k.dateStartRoom]) ||
      emptySchoolCount();
  }

  function applySchoolReservationsSafely(slots, schoolMaps) {
    return (slots || []).map(slot => {
      try {
        const sc = schoolReservationForPublicSlot(slot, schoolMaps);
        const schoolTotal = Number(sc.total || 0);
        const publicTotal = Number(slot.publicReserved || 0);
        const remaining = Math.max(0, Number(slot.capacity || 0) - publicTotal - schoolTotal);
        return Object.assign({}, slot, {
          schoolConfirmed: Number(sc.confirmed || 0),
          schoolPending: Number(sc.pending || 0),
          schoolReserved: schoolTotal,
          totalReserved: publicTotal + schoolTotal,
  ...
  function removeAutoSlotsWithRoomConflictsPublic(slots) {
    return (slots || []).filter(slot => {
      if (!slot.autoLabel) return true;
      const a = minutes(slot.start), b = minutes(slot.end);
      if (a === null || b === null) return true;
      return !(slots || []).some(other => {
        if (other === slot || other.autoLabel) return false;
        if (other.dateIso !== slot.dateIso || other.roomKey !== slot.roomKey) return false;
        const c = minutes(other.start), d = minutes(other.end);
  ...
  function withPublicReservationDefaults(slot) {
    ...
    return Object.assign({}, slot, {
      ...
      schoolConfirmed: Number(slot && slot.schoolConfirmed ? slot.schoolConfirmed : 0),
      schoolPending: Number(slot && slot.schoolPending ? slot.schoolPending : 0),
      schoolReserved: Number(slot && slot.schoolReserved ? slot.schoolReserved : 0),
      totalReserved: Number(slot && slot.totalReserved ? slot.totalReserved : Number(slot && slot.publicReserved ? slot.publicReserved : 0)),
      publicFull: Boolean(slot && slot.publicFull)
    });
  }

  function applyPublicReservations(slots, reservations, schoolMaps) {
    return slots.map(slot => {
      try {
        const technicalId = slotTechnicalId(slot);
        const r = reservations[technicalId] || { confirmed: 0, pending: 0, total: 0, details: [] };
        const sc = schoolReservationForPublicSlot(slot, schoolMaps || {});
        const publicOccupied = Number(r.total || 0);
        const schoolOccupied = Number(sc.total || 0);
        const remaining = Math.max(0, Number(slot.capacity || 0) - publicOccupied - schoolOccupied);
        return Object.assign({}, slot, {
          reservationId: technicalId,
          publicConfirmed: Number(r.confirmed || 0),
          publicPending: Number(r.pending || 0),
          publicReserved: publicOccupied,
          schoolConfirmed: Number(sc.confirmed || 0),
          schoolPending: Number(sc.pending || 0),
          schoolReserved: schoolOccupied,
          totalReserved: publicOccupied + schoolOccupied,
          publicRemaining: remaining,
  ...
  function buildSlots(rows) {
    ...
    const cleanedSlots = removeAutoSlotsWithRoomConflictsPublic(slots);
    const seen = new Set();
    return cleanedSlots.filter(s => {
  ...
  function countNonAcceptedProposedSlots(rows) {
    let count = 0;
    (rows || []).filter(row => !isAccepted(row)).forEach(row => {
      if (shouldAuto(row)) {
        count += AUTO_DAYS.length * AUTO_SLOTS.length;
      } else {
        count += 1;
      }
    });
  ...
  function schoolRowHasReservationIds(row) {
    if (!row) return false;
    let rawIds = get(row, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux", "ID créneau", "ID creneau", "IDs des créneaux", "IDs des creneaux"]);
    if (!rawIds) {
      rawIds = Object.keys(row || {}).map(k => row[k]).find(cell => /2026-06-\d{2}\|/.test(String(cell || ""))) || "";
    }
    return splitReservationIds(rawIds).some(rawId => Boolean(normalizeIdParts(rawId)));
  }

  function countConfirmedSchoolRequestRows(rows) {
    return (rows || []).filter(row => {
      const status = schoolReservationStatus(get(row, ["VALIDATION CRD", "Validation CRD", "STATUT CRD", "Statut CRD", "Statut"]));
      return status !== "cancelled" && schoolRowHasReservationIds(row);
    }).length;
  }
  ...
  function halfDayForSlot(slot) {
    const m = minutes(slot && slot.start);
  ...
  function periodLabel(period) {
    ...
    if (period === "evening") return "soirée";
    return "journée";
  }

  function slotDurationMinutes(slot) {
    const a = minutes(slot && slot.start), b = minutes(slot && slot.end);
    if (a === null || b === null || b <= a) return 30;
    return b - a;
  }

  function personKey(detail, index) {
    const key = norm([detail && detail.email, detail && detail.phone, detail && detail.name].filter(Boolean).join("|"));
    return key || ("ligne-" + index);
  }

  function publicUniqueEstimate(slots) {
    const people = {};
    let fallback = 0;
    (slots || []).forEach(slot => {
      const details = slot.publicReservationDetails || [];
      if (!details.length) fallback += Number(slot.publicReserved || 0);
      details.forEach((detail, index) => {
        const key = personKey(detail, index);
        people[key] = Math.max(people[key] || 0, Number(detail.places || 0));
      });
    });
    const estimated = Object.keys(people).reduce((sum, key) => sum + people[key], 0);
  ...
  function peakMoments(slots, limit) {
    ...
    (slots || []).forEach(slot => {
      ...
      if (!map[key]) map[key] = { key, dateIso: slot.dateIso, start: slot.start, total: 0, publicTotal: 0, schoolTotal: 0, count: 0, slots: [] };
      map[key].total += Number(slot.totalReserved || 0);
      map[key].publicTotal += Number(slot.publicReserved || 0);
      map[key].schoolTotal += Number(slot.schoolReserved || 0);
      map[key].count += 1;
      map[key].slots.push(slot);
    });
    return Object.values(map).filter(p => p.total > 0).sort((a, b) => b.total - a.total || (a.dateIso || "").localeCompare(b.dateIso || "") || (minutes(a.start) || 0) - (minutes(b.start) || 0)).slice(0, limit || 5);
  }

  function openPeak(key) {
    const slots = baseFiltered();
    const peak = peakMoments(slots, 50).find(p => p.key === key);
    if (!peak) return;
    $("jpo-public-modal-title").textContent = "Pic d’affluence — " + formatDate(peak.dateIso) + " à " + showTime(peak.start);
    $("jpo-public-modal-body").innerHTML =
      rowHtml("Total réservé", peak.total + " place(s) — public " + peak.publicTotal + " / scolaires " + peak.schoolTotal) +
      rowHtml("Créneaux concernés", String(peak.count)) +
      '<div class="cards" style="padding:0;margin-top:14px">' + peak.slots.map(s =>
        '<article class="card"><div class="top"><div class="title">' + esc(s.title) + '</div></div><div class="body"><strong>' + esc(s.roomLabel) + '</strong><br>' +
        'Public : ' + (s.publicReserved || 0) + ' · Scolaires : ' + (s.schoolReserved || 0) + ' · Restantes : ' + (s.publicRemaining || 0) +
        '<div class="actions"><button type="button" class="btn" data-slot-id="' + esc(s.id) + '">Voir le détail</button></div></div></article>'
      ).join("") + '</div>';
  ...
  function routeScore(route) {
    ...
    const rooms = {}, poles = {};
    route.forEach(slot => {
      if (isHighlight(slot)) score += 20;
      rooms[slot.roomKey] = true;
      poles[norm(slot.pole || slot.discipline || "")] = true;
      score += Math.min(20, Number(slot.publicRemaining || 0));
    });
    return score + Object.keys(rooms).length * 6 + Object.keys(poles).length * 8;
  }

  function findPublicAutoRoute(wanted) {
    const candidates = baseFiltered().filter(slot => !slot.publicFull && Number(slot.publicRemaining || 0) > 0).slice().sort(compareSlots);
    const byDay = {};
    candidates.forEach(slot => { (byDay[slot.dateIso] ||= []).push(slot); });
    let best = [];
    Object.keys(byDay).forEach(day => {
      const list = byDay[day];
      list.forEach((first, idx) => {
        const route = [first];
        const used = { [highlightProgramKey(first)]: true };
        let total = slotDurationMinutes(first);
        for (let i = idx + 1; i < list.length && total < wanted + 30; i++) {
          const prev = route[route.length - 1];
          const gap = (minutes(list[i].start) ?? 0) - (minutes(prev.end) ?? 0);
          const key = highlightProgramKey(list[i]);
          if (gap < 0 || gap > 30 || used[key]) continue;
          route.push(list[i]);
          used[key] = true;
          total += slotDurationMinutes(list[i]);
          if (total >= wanted) break;
        }
        if (total >= wanted && total <= wanted + 30) {
          if (!best.length || routeScore(route) > routeScore(best)) best = route.slice();
        }
      });
    });
    return best;
  ...
  function openPublicAuto(duration) {
    const wanted = parseInt(duration || "0", 10);
    if (!wanted) return;
    const route = findPublicAutoRoute(wanted);
    $("jpo-public-modal-title").textContent = "Proposition automatique — " + (wanted === 60 ? "1h" : wanted === 90 ? "1h30" : "2h") + " " + periodLabel(activePeriod);
    if (!route.length) {
      $("jpo-public-modal-body").innerHTML = '<div class="empty">Aucune proposition automatique compatible avec les filtres actuels. Essayez un autre jour, une autre demi-journée ou retirez un filtre.</div>';
      $("jpo-public-modal").classList.add("open");
      return;
    }
    const total = route.reduce((sum, slot) => sum + slotDurationMinutes(slot), 0);
    $("jpo-public-modal-body").innerHTML =
      rowHtml("Durée proposée", total + " minute(s)") +
      rowHtml("Jour", formatDate(route[0].dateIso)) +
      '<div class="cards" style="padding:0;margin-top:14px">' + route.map(slot =>
        '<article class="card"><div class="top"><div class="title">' + esc(showTime(slot.start)) + '–' + esc(showTime(slot.end)) + ' · ' + esc(slot.title) + '</div></div><div class="body"><strong>' + esc(slot.roomLabel) + '</strong><br>' + esc(reservationLabel(slot)) + '<div class="actions"><button type="button" class="btn2" data-add-cart="' + esc(slot.id) + '">Ajouter au panier</button><button type="button" class="btn" data-slot-id="' + esc(slot.id) + '">Détail</button></div></div></article>'
      ).join("") + '</div>';
    $("jpo-public-modal").classList.add("open");
  }
  ...
  function baseFiltered() {
    return allSlots.filter(isPublicVisible).filter(slot => {
      ...
      if (activePeriod !== "all" && halfDayForSlot(slot) !== activePeriod) return false;
  ...
  function renderStats(slots) {
    ...
    const statsSlots = (allSlots && allSlots.length) ? allSlots : displaySlots;

    const totalReserved = statsSlots.reduce((sum, s) => sum + Number(s.totalReserved || 0), 0);
    const reservedSlots = statsSlots.filter(s => Number(s.totalReserved || 0) > 0).length;
    const schoolDebug = (window.JPO_PUBLIC_DEBUG && window.JPO_PUBLIC_DEBUG.schoolReservations && window.JPO_PUBLIC_DEBUG.schoolReservations.debug) ? window.JPO_PUBLIC_DEBUG.schoolReservations.debug : [];

    const allSchoolKeys = {};
    statsSlots.forEach(slot => {
      const keys = schoolKeysForPublicSlot(slot);
      Object.keys(keys).forEach(k => { allSchoolKeys[keys[k]] = true; });
    });

    const schoolGroups = {};
    (schoolDebug || []).forEach((item, index) => {
      const info = normalizeIdParts(item && item.raw ? item.raw : (item && item.id ? item.id : ""));
      if (!info) return;
      const keys = {
        exact: info.exact,
        dateStartRoomTitle: [info.date, info.start, info.room, info.title].join("|"),
        dateStartTitle: [info.date, info.start, info.title].join("|"),
        dateStartRoom: [info.date, info.start, info.room].join("|")
      };
      const matchesKnownSlot = Object.keys(keys).some(k => allSchoolKeys[keys[k]]);
      if (!matchesKnownSlot) return;
      const parts = String(item.raw || "").split("|").map(x => String(x || "").trim());
      const group = norm(parts[5] || ("groupe-scolaire-" + index));
      const key = group || ("groupe-scolaire-" + index);
      schoolGroups[key] = Math.max(schoolGroups[key] || 0, Number(item.total || 0));
    });

    const schoolUnique = Object.keys(schoolGroups).reduce((sum, key) => sum + schoolGroups[key], 0);
    const schoolTakenFallback = statsSlots.reduce((sum, s) => sum + Number(s.schoolReserved || 0), 0);
    const uniqueWithSchools = publicUniqueEstimate(statsSlots) + (schoolUnique || schoolTakenFallback);
    const schoolGroupCount = Object.keys(schoolGroups).length || statsSlots.filter(s => Number(s.schoolReserved || 0) > 0).length;

    const publicProgramKeys = {};
    const schoolProgramKeys = {};
    (allSlots || []).forEach(slot => {
      const key = String(slot.line || "") + "|" + highlightProgramKey(slot);
      const target = norm(slot.publicTarget || "");
      const dedicatedSchool = target.includes("scolaire") || target.includes("ecole") || target.includes("école") || target.includes("etablissement") || target.includes("établissement") || target.includes("classe") || target.includes("maternelle") || target.includes("elementaire") || target.includes("élémentaire") || target.includes("college") || target.includes("collège") || target.includes("lycee") || target.includes("lycée");
      if (dedicatedSchool && !(target.includes("tout public") || target.includes("famille") || target.includes("parents"))) schoolProgramKeys[key] = true;
      else publicProgramKeys[key] = true;
    });

    const peaks = peakMoments(displaySlots, 3);
    const maxPeak = peaks.length ? peaks[0].total : 0;
    const proposedTotal = teacherSlotCountTotal || (allSlots || []).length;

    $("jpo-public-stats").innerHTML =
      '<div class="stat statgroup"><div class="stat-title">Décompte des publics</div><div class="stat-main"><div class="num">' + totalReserved + '</div><span>places réservées</span></div><div class="mini"><div><b>' + schoolGroupCount + '</b><span>groupes/classes écoles</span></div><div><b>' + uniqueWithSchools + '</b><span>individus uniques estimés</span></div><div><b>' + reservedSlots + '</b><span>créneaux réservés affichés</span></div><div><b>' + displaySlots.length + '</b><span>rendez-vous affichés</span></div></div></div>' +
      '<div class="stat statgroup"><div class="stat-title">Autres chiffres clefs</div><div class="stat-main"><div class="num">' + proposedTotal + '</div><span>créneaux proposés</span></div><div class="mini"><div><b>' + Object.keys(publicProgramKeys).length + '</b><span>propositions tout public</span></div><div><b>' + Object.keys(schoolProgramKeys).length + '</b><span>propositions dédiées aux scolaires</span></div><div><b>' + schoolConfirmedReservationsCount + '</b><span>réservations confirmées</span></div><div><b>180</b><span>jauge auditorium max</span></div></div></div>' +
      '<div class="stat statgroup"><div class="stat-title">Pics d’affluence</div><div class="stat-main"><div class="num">' + maxPeak + '</div><span>personnes réservées au plus fort</span></div><div class="peaklist">' + (peaks.length ? peaks.map(p => '<button type="button" class="peakbtn" data-open-peak="' + esc(p.key) + '">' + esc(formatDate(p.dateIso)) + ' · ' + esc(showTime(p.start)) + ' — ' + p.total + ' pers. (' + p.count + ' créneau(x))</button>').join("") : '<div class="lab">Aucune réservation enregistrée avec ces filtres.</div>') + '</div></div>';
  }
  ...
  function openSlot(id) {
    ...
    $("jpo-public-modal-body").innerHTML =
      ...
      rowHtml("Réservations", "Public : " + (s.publicReserved || 0) + " · Scolaires : " + (s.schoolReserved || 0) + " · Restantes : " + (s.publicRemaining ?? s.capacity)) +
  ...
  function bind() {
    ["jpo-public-search", "jpo-public-day", "jpo-public-period", "jpo-public-pole", "jpo-public-focus"].forEach(id => {
      $(id).addEventListener(id === "jpo-public-search" ? "input" : "change", function () {
        ...
        activePeriod = $("jpo-public-period").value;
        activePole = $("jpo-public-pole").value;
        activeFocus = $("jpo-public-focus").value;
        render();
      });
    });
    const autoSelect = $("jpo-public-auto");
    if (autoSelect) autoSelect.addEventListener("change", function () { const value = autoSelect.value; autoSelect.value = ""; if (value) openPublicAuto(value); });
    ...
    document.addEventListener("click", function (e) {
      ...
      const peak = e.target.closest("[data-open-peak]");
      if (peak && root.contains(peak)) { openPeak(peak.getAttribute("data-open-peak")); return; }
  ...
  Promise.all([
    ...
    fetch(DEMANDES_CSV).then(r => { if (!r.ok) throw new Error("CSV demandes écoles HTTP " + r.status); return r.text(); }).catch(() => "")
  ])
    .then(([propText, publicText, schoolText]) => {
      const propositionRows = parseCSV(propText);
      const publicRows = parseCSV(publicText || "");
      const schoolRows = parseCSV(schoolText || "");
      const propositionSlots = buildSlots(propositionRows);
      teacherSlotCountTotal = propositionSlots.length + countNonAcceptedProposedSlots(propositionRows);
      schoolConfirmedReservationsCount = countConfirmedSchoolRequestRows(schoolRows);
      let schoolMaps = { exact: {}, dateStartRoomTitle: {}, dateStartTitle: {}, dateStartRoom: {}, debug: [] };
      const warnings = [];

      try {
        publicReservationsMap = buildPublicReservations(publicRows);
      } catch (err) {
        console.error("Erreur réservations public JPO", err);
        publicReservationsMap = {};
        warnings.push("Réservations public non lues");
      }

      try {
        schoolMaps = buildSchoolReservationsFromPublicCSV(schoolRows);
      } catch (err) {
        console.error("Erreur réservations écoles JPO", err);
        warnings.push("Réservations scolaires non lues");
      }

      allSlots = applyPublicReservations(propositionSlots, publicReservationsMap, schoolMaps);
      window.JPO_PUBLIC_DEBUG = {
        slots: allSlots,
        publicReservations: publicReservationsMap,
        schoolReservations: schoolMaps,
        rawRowsEcolesCount: schoolRows.length || 0,
        parsedSchoolReservationCount: schoolMaps && schoolMaps.debug ? schoolMaps.debug.length : 0,
        bernstein: allSlots.filter(s => norm(s.title).includes("bernstein")),
        warnings,
        teacherSlotCountTotal,
        schoolConfirmedReservationsCount
      };

      if (warnings.length) {
        $("jpo-public-error").textContent = "Information : " + warnings.join(" ; ") + ". Le programme reste affiché.";
      }
