document.addEventListener("DOMContentLoaded", function () {
  ...
  function makeSlot(row, forcedDate, forcedStart, forcedEnd, autoLabel, indexSuffix) {
    ...
    const declaredCapacity = numberFrom(get(row, ["CAPACITÉ CRD"]));
    const estimated = numberFrom(get(row, ["Nombre estimé de participants / spectateurs"]));
    const capacity = declaredCapacity || estimated || (ROOM_DEFAULTS[key] ? ROOM_DEFAULTS[key].capacity : 35);
    const highlightGauge = estimated || declaredCapacity || 0;
  ...
  function isHighlight(slot) {
    return slot.roomKey === "auditorium" || (LARGE_ROOM_KEYS.includes(slot.roomKey) && Number(slot.highlightGauge || 0) >= 35);
  }
  ...
  function renderFocus(slots) {
    ...
    $("jpo-public-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Temps forts de la semaine</div><div class="focus-sub">Les propositions à l’auditorium ou les grands formats de 35 personnes minimum sont mis en avant automatiquement.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
