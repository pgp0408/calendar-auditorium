document.addEventListener("DOMContentLoaded", function () {
  ...
  const STORAGE_KEY = "jps_ecoles_v28_panier";
  const LAST_GROUP_KEY = "jps_ecoles_v28_last_group";
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <div class="head">
        ...
        <p class="sub">
          Dispositif de réservation de modules et de parcours pour les établissements scolaires, enseignants, associations et organismes souhaitant bénéficier des propositions du Conservatoire.
        </p>
        <div class="notice">
          Choisissez des modules par horaire pour composer un parcours adapté à votre groupe. Les places demandées sont placées en attente dès l’envoi et retirées des disponibilités, sous réserve de validation finale par le Conservatoire.<br>
  ...
  function renderSuggestions(slots) {
    ...
    box.innerHTML =
      '<div class="choice-hint"><strong>Idées de parcours pour cette journée</strong><br>Ces exemples aident à se repérer. Cliquez sur “Préparer cette idée” pour renseigner une classe : l’outil vérifiera ensuite les jauges avant d’ajouter au panier.</div>' +
  ...
  function renderNextSuggestions(slot) {
    ...
    box.innerHTML =
      ...
      '<div class="nextbox-help">Pour construire un parcours, ajoutez d’abord le créneau en cours si vous voulez le garder. Vous pourrez ensuite préparer un des créneaux ci-dessous.</div>' +
      '<div class="nextlist">' +
        next.map(candidate =>
          '<div class="nextitem">' +
            ...
            '<button type="button" class="btn2" data-next-slot-id="' + esc(candidate.id) + '">Préparer cette suite</button>' +
          '</div>'
        ).join("") +
      '</div>';
  }
