(function () {
    ...
    rows.forEach(row => {
      ...
      if (!rawIds) {
        Object.keys(row || {}).some(k => {
          const v = row[k];
          if (/2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(String(v || ""))) {
            rawIds = String(v || "");
            return true;
          }
          return false;
      ...
      String(rawIds).split(/[;\n]+/).map(x => x.trim()).filter(Boolean).forEach(line => {
        ...
        if (!total) total = numberFrom(get(row, ["Nombre total", "Total", "Effectif", "Participants"]));
        if (!id || !total) return;
        if (!map[id]) map[id] = { confirmed: 0, pending: 0, total: 0 };
        if (validation === "confirmed") map[id].confirmed += total;
  ...
  function withPublicReservationDefaults(slot) {
    ...
    return Object.assign({}, slot, {
      ...
      totalReserved: Number(slot && slot.totalReserved ? slot.totalReserved : 0),
  ...
  function applyPublicReservations(slots, reservations, schoolReservations) {
    ...
    return slots.map(slot => {
      try {
        ...
        const s = schoolMap[technicalId] || { confirmed: 0, pending: 0, total: 0 };
        const publicOccupied = Number(r.total || 0);
        const schoolOccupied = Number(s.total || 0);
        ...
        return Object.assign({}, slot, {
          ...
          schoolConfirmed: Number(s.confirmed || 0),
          schoolPending: Number(s.pending || 0),
  ...
  function renderCard(slot) {
    ...
    return '<article class="card' + strong + '"><div class="top"><div class="tagline">' +
      ...
      (Number(slot.schoolReserved || 0) ? '<div style="margin-top:4px;color:#64748b;font-weight:850;font-size:12px">' + Number(slot.schoolReserved || 0) + ' place(s) déjà bloquée(s) par les réservations écoles</div>' : '') +
  ...
  Promise.all([
    ...
    .then(([propText, publicText, schoolText]) => {
      ...
      try {
        ...
        const schoolReservationsMap = buildSchoolReservations(parseCSV(schoolText || ""));
        ...
        $("jpo-public-error").textContent = "Information : les réservations public ou écoles n’ont pas pu être lues correctement. Le programme reste affiché et les boutons de réservation restent disponibles.";
      }
      renderControls();
      bind();
      render();
