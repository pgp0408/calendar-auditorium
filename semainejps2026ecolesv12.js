document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderChoice(s) {
    ...
    return '<article class="choice ' + (disabled ? 'full' : '') + '" ' + (!disabled ? 'onclick="window.JPS_ECOLES_ADD_SLOT(\'' + encodedId + '\')" style="cursor:pointer"' : '') + '>' +
      '<div class="choice-top"><div><div class="choice-title">' + esc(s.title) + '</div><div class="place">' + esc(s.roomLabel) + ' · ' + esc(s.pole || '') + '</div></div><span class="badge ' + badgeClass(s.pole, s.roomKey) + '">' + esc(s.roomShort) + '</span></div>' +
      '<div class="capacity"><div class="cap"><strong>' + s.capacity + '</strong><span>jauge</span></div><div class="cap"><strong>' + s.pending + '</strong><span>en attente</span></div><div class="cap ' + remainingClass({ remaining: remainingTxt }) + '"><strong>' + remainingTxt + '</strong><span>restants</span></div></div>' +
      '<div class="place">' + s.confirmed + ' confirmé(s) · ' + s.pending + ' en attente. Les demandes en attente bloquent les places.</div>' +
      '<div class="actions">' + (disabled ? '<button type="button" class="btn disabled">Complet / réservé</button>' : '<button type="button" class="btn" onclick="event.stopPropagation(); window.JPS_ECOLES_ADD_SLOT(\'' + encodedId + '\')">Ajouter au parcours</button>') + '</div>' +
      '</article>';
  }

  function debugClick(message) {
    window.JPS_ECOLES_DEBUG = window.JPS_ECOLES_DEBUG || {};
    window.JPS_ECOLES_DEBUG.lastClick = message;
  }

  function openSlotByEncodedId(encodedSlotId) {
    const slotId = decodeURIComponent(encodedSlotId || "");
    debugClick("clic reçu pour " + slotId);
  ...
  window.JPS_ECOLES_ADD_SLOT = openSlotByEncodedId;

  function bindAddButtons() {
    // Conservé pour compatibilité, mais les boutons utilisent désormais window.JPS_ECOLES_ADD_SLOT().
  }
  ...
  document.addEventListener("click", function(e) {
    const openEl = e.target.closest("[data-open-slot]");
    if (openEl && root.contains(openEl)) {
      e.preventDefault();
      e.stopPropagation();
      openSlotByEncodedId(openEl.getAttribute("data-open-slot"));
      return;
    }
  ...
  $("jps-debug-click").addEventListener("click", function() {
    alert("Le JS V12 est bien chargé et les clics fonctionnent.");
  });
