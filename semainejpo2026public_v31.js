(function () {
  ...
  function makeSlot(row, forcedDate, forcedStart, forcedEnd, autoLabel, indexSuffix) {
    ...
    const retainedRoom = finalRoom && !norm(finalRoom).includes("arbitrer") ? finalRoom : fallbackRoom;
    const room = roomInfo(retainedRoom, precision);
    const key = room.key;
  ...
  function getReservationForSlot(slot, reservations) {
    ...
    Object.keys(reservations || {}).forEach(id => {
      if (id.indexOf(prefix) === 0) {
        sum.confirmed += Number(reservations[id].confirmed || 0);
        sum.pending += Number(reservations[id].pending || 0);
        sum.total += Number(reservations[id].total || 0);
        if (reservations[id].details) sum.details = sum.details.concat(reservations[id].details);
      }
    });
