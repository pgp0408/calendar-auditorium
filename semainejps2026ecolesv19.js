document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    <style>
      ...
      #jps-ecoles .wrap{max-width:1060px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:24px;overflow:hidden;box-shadow:0 18px 50px rgba(15,23,42,.08)}
      ...
      #jps-ecoles .content{padding:24px}#jps-ecoles .stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:20px}#jps-ecoles .stat{border:1px solid #e5e7eb;border-radius:16px;padding:13px;background:#fff}#jps-ecoles .num{font-size:26px;font-weight:900;line-height:1}#jps-ecoles .lab{margin-top:5px;color:#64748b;font-size:12px;font-weight:800;line-height:1.3}
      #jps-ecoles .filters{display:grid;grid-template-columns:repeat(auto-fit,minmax(135px,1fr));gap:8px;margin-bottom:22px}#jps-ecoles .filter{border:1px solid #d1d5db;background:#fff;border-radius:16px;padding:12px 14px;font-size:14px;font-weight:900;cursor:pointer;text-align:center}#jps-ecoles .filter.active{background:#111827;color:#fff;border-color:#111827;box-shadow:0 10px 22px rgba(15,23,42,.14)}
      #jps-ecoles .day{margin-bottom:28px;border:1px solid #e7eaee;border-radius:24px;overflow:hidden;background:#fff}#jps-ecoles .dayhead{display:flex;justify-content:space-between;gap:12px;padding:20px;background:#f8fafc;border-bottom:1px solid #e5e7eb}#jps-ecoles .daytitle{font-size:24px;font-weight:900;text-transform:uppercase;letter-spacing:-.03em}#jps-ecoles .daymeta{margin-top:4px;color:#64748b;font-size:14px;font-weight:800;line-height:1.45}#jps-ecoles .badge{border-radius:999px;background:#e2e8f0;color:#334155;padding:7px 10px;font-size:12px;font-weight:900;white-space:nowrap}#jps-ecoles .daybody{padding:16px}
      ...
      #jps-ecoles .timegroup{margin-bottom:18px;border:1px solid #e7eaee;border-radius:22px;overflow:hidden;background:#fff}#jps-ecoles .timegroup-head{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:16px 18px;background:#f8fafc;border-bottom:1px solid #e5e7eb}#jps-ecoles .timegroup-title{font-size:23px;font-weight:900;letter-spacing:-.03em}#jps-ecoles .timegroup-sub{font-size:13px;font-weight:800;color:#64748b;margin-top:3px}#jps-ecoles .timegroup-count{border-radius:999px;background:#e2e8f0;color:#334155;padding:6px 10px;font-size:12px;font-weight:900;white-space:nowrap}#jps-ecoles .timegroup-body{padding:14px}#jps-ecoles .choice-hint{margin:0 0 16px;color:#475569;font-weight:800;line-height:1.45;background:#f8fafc;border:1px solid #e5e7eb;border-radius:16px;padding:13px}
      ...
      #jps-ecoles .card{transition:transform .15s ease,box-shadow .15s ease,border-color .15s ease;border-radius:20px}#jps-ecoles .card:not(.is-full):hover{transform:translateY(-2px);box-shadow:0 14px 30px rgba(15,23,42,.08);border-color:#cbd5e1}#jps-ecoles .simple-remaining{display:flex;align-items:baseline;gap:6px;margin:12px 0 8px}.simple-remaining strong{font-size:32px;line-height:1;font-weight:900;color:#166534}.simple-remaining span{font-size:13px;color:#475569;font-weight:900}.simple-remaining.low strong{color:#9a3412}.simple-remaining.zero strong{color:#991b1b}.simple-meta{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.simple-pill{border-radius:999px;background:#f1f5f9;color:#334155;padding:5px 8px;font-size:11px;font-weight:900}
      #jps-ecoles .choice-label{display:inline-flex;align-items:center;gap:6px;border-radius:999px;background:#eef2ff;color:#3730a3;padding:5px 9px;font-size:11px;font-weight:900;margin-top:8px}
      #jps-ecoles .room-tag{display:inline-flex;border-radius:999px;background:#f1f5f9;color:#334155;padding:5px 8px;font-size:11px;font-weight:900;margin-right:5px;margin-top:6px}
      #jps-ecoles .room-tag.auditorium{background:#dbeafe;color:#1e40af}.room-tag.voix{background:#ede9fe;color:#5b21b6}.room-tag.orchestre{background:#fef3c7;color:#92400e}.room-tag.theatre{background:#fee2e2;color:#991b1b}.room-tag.danse{background:#dcfce7;color:#166534}
      #jps-ecoles .desc-preview{margin:10px 0 0;color:#475569;font-size:13px;line-height:1.45;font-weight:700}
      #jps-ecoles .card-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}
    ...
    <div id="jps-info" class="modal"><div class="box"><button type="button" class="close" data-close="jps-info">&times;</button><h3 id="jps-i-title" class="modaltitle"></h3><div class="row"><span class="label">Horaire</span><span id="jps-i-time"></span></div><div class="row"><span class="label">Lieu</span><span id="jps-i-room"></span></div><div class="row"><span class="label">Pôle</span><span id="jps-i-pole"></span></div><div class="row"><span class="label">Public</span><span id="jps-i-public"></span></div><div class="row"><span class="label">Type</span><span id="jps-i-type"></span></div><div class="row"><span class="label">Description</span><span id="jps-i-desc"></span></div><div class="row"><span class="label">Disponibilité</span><span id="jps-i-capacity"></span></div><div class="actions"><button type="button" id="jps-i-add" class="btn">Ajouter ce créneau au panier</button><button type="button" data-close="jps-info" class="btn2">Fermer</button></div></div></div>
  `;
  ...
  function buildSlots(rows) {
    ...
    data.forEach((r, i) => {
      ...
      // V19 : plafond prudent de réservation publique.
      // Même si le Sheet contient encore 200 / 40, l'outil écoles limite les réservations à 160 / 30.
      // Si une capacité CRD plus basse est renseignée, elle reste respectée.
      if (room.key === "auditorium") capacity = Math.min(capacity, 160);
      else if (["orchestre", "chant", "choeur", "theatre", "danse1", "danse2", "danse3", "other"].includes(room.key)) capacity = Math.min(capacity, 30);
  ...
  function renderFilters() {
    ...
    root.querySelectorAll(".filter").forEach(b => b.addEventListener("click", function () {
      activeDay = b.dataset.day;
      root.querySelectorAll(".filter").forEach(x => x.classList.remove("active"));
  ...
  function tagClass(s) {
    const p = norm(s.pole || "");
    if (s.roomKey === "auditorium") return "auditorium";
    if (p.includes("voix") || p.includes("chant") || p.includes("choeur") || p.includes("chœur")) return "voix";
    if (p.includes("orchestre") || p.includes("instrument")) return "orchestre";
    if (p.includes("theatre") || p.includes("théâtre")) return "theatre";
    if (p.includes("danse")) return "danse";
    return "";
  }

  function shortDesc(s) {
    const txt = String(s.description || "").trim();
    if (!txt) return "";
    return txt.length > 135 ? txt.slice(0, 132).trim() + "…" : txt;
  }

  function renderSingle(s) {
    const st = status(s), inCart = cartFor(s.id), rem = Math.max(0, s.remaining - inCart), disabled = s.full || rem <= 0;
    const preview = shortDesc(s);
    const remClass = rem <= 0 ? "zero" : rem <= 8 ? "low" : "";
    return '<article class="card ' + (disabled ? 'is-full' : '') + '"><div class="top"><div class="choice-label">' + esc(showTime(s.start)) + '–' + esc(showTime(s.end)) + '</div><div class="title" style="margin-top:8px">' + esc(s.title) + '</div><div class="simple-meta"><span class="simple-pill">' + esc(s.roomLabel) + '</span>' + (s.pole ? '<span class="simple-pill">' + esc(s.pole) + '</span>' : '') + '</div>' + (preview ? '<div class="desc-preview">' + esc(preview) + '</div>' : '') + '</div><div class="body"><div class="simple-remaining ' + remClass + '"><strong>' + rem + '</strong><span>places disponibles</span></div><div class="meta">' + s.confirmed + ' confirmée(s) · ' + s.pending + ' en attente · jauge ' + s.capacity + '</div><div class="card-actions"><button class="btn2" type="button" data-info-slot-id="' + esc(s.id) + '">Détail</button>' + (disabled ? '<button class="btn disabled" type="button">Complet</button>' : '<button class="btn" type="button" data-add-slot-id="' + esc(s.id) + '">Ajouter au parcours</button>') + '</div></div></article>';
  }
  ...
  function render() {
    ...
    const days = DAYS.filter(d => d.iso === activeDay);
    $("jps-days").innerHTML = days.map(d => {
      ...
      return '<section class="day"><div class="dayhead"><div><div class="daytitle">' + esc(d.label) + '</div><div class="daymeta">Choisissez un horaire, puis ajoutez une activité au parcours d’une classe.</div></div><div class="badge">' + daySlots.length + ' créneau(x)</div></div><div class="daybody"><p class="choice-hint">Mode d’emploi : cliquez sur “Détail” pour comprendre la proposition, ou sur “Ajouter au parcours” pour inscrire une classe sur ce créneau.</p>' + (timeBlocks || '<div class="empty">Aucun créneau disponible pour cette journée.</div>') + '</div></section>';
    }).join("");
    bindAddButtons();
    renderCart();
  }
  ...
  function openInfoById(slotId) {
    ...
    currentSlot = s;
    $("jps-i-title").textContent = s.title;
    $("jps-i-time").textContent = formatDate(s.dateIso) + " — " + showTime(s.start) + "-" + showTime(s.end);
    $("jps-i-room").textContent = s.roomLabel;
    $("jps-i-pole").textContent = s.pole || "—";
    $("jps-i-public").textContent = s.publicTarget || "—";
    $("jps-i-type").textContent = s.type || "—";
    $("jps-i-desc").textContent = s.description || "—";
    $("jps-i-capacity").textContent = s.capacity + " places · " + s.confirmed + " confirmée(s) · " + s.pending + " en attente · " + Math.max(0, s.remaining - cartFor(s.id)) + " restante(s)";
    const add = $("jps-i-add");
    if (s.full || cartFor(s.id) >= s.remaining) {
      add.className = "btn disabled";
      add.textContent = "Complet / réservé";
    } else {
      add.className = "btn";
      add.textContent = "Ajouter ce créneau au panier";
    }
    openModal("jps-info");
  }
  ...
  document.addEventListener("click", function(e) {
    ...
    const infoBtn = e.target.closest("[data-info-slot-id]");
    if (infoBtn && root.contains(infoBtn)) {
      e.preventDefault();
      e.stopPropagation();
      openInfoById(infoBtn.getAttribute("data-info-slot-id"));
      return;
    }
  ...
  ["jps-booking", "jps-cart", "jps-info"].forEach(id => $(id).addEventListener("click", e => { if (e.target === $(id)) closeModal(id); }));
