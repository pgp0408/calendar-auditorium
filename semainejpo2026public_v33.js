(function () {
  ...
  function blockedDuration(format, fallback) {
    ...
    return durationToMinutes(fallback || format);
  }

  function roomInfo(v, precision) {
    const s = norm(v);
    const p = String(precision || "").trim();
    if (s.includes("auditorium")) return Object.assign({ key: "auditorium" }, ROOM_DEFAULTS.auditorium);
    if (s.includes("orchestre")) return Object.assign({ key: "orchestre" }, ROOM_DEFAULTS.orchestre);
    if (s.includes("chant") && !s.includes("choeur") && !s.includes("chœur")) return Object.assign({ key: "chant" }, ROOM_DEFAULTS.chant);
    if (s.includes("choeur") || s.includes("chœur")) return Object.assign({ key: "choeur" }, ROOM_DEFAULTS.choeur);
    if (s.includes("theatre") || s.includes("théâtre")) return Object.assign({ key: "theatre" }, ROOM_DEFAULTS.theatre);
    if (s.includes("danse 1") || s.includes("studio de danse 1")) return Object.assign({ key: "danse1" }, ROOM_DEFAULTS.danse1);
    if (s.includes("danse 2") || s.includes("studio de danse 2")) return Object.assign({ key: "danse2" }, ROOM_DEFAULTS.danse2);
    if (s.includes("danse 3") || s.includes("studio de danse 3")) return Object.assign({ key: "danse3" }, ROOM_DEFAULTS.danse3);
    if (s.includes("danse")) return Object.assign({ key: "danse1" }, ROOM_DEFAULTS.danse1);
    if (s.includes("eveil") || s.includes("éveil")) return Object.assign({ key: "eveil" }, ROOM_DEFAULTS.eveil);
    if (s.includes("autre") || p) return { key: "other", label: p || ROOM_DEFAULTS.other.label, capacity: ROOM_DEFAULTS.other.capacity };
    if (s.includes("arbitrer") || s.includes("peu importe") || s.includes("a definir") || s.includes("à définir")) return Object.assign({ key: "any" }, ROOM_DEFAULTS.any);
    return { key: "other", label: v || ROOM_DEFAULTS.other.label, capacity: ROOM_DEFAULTS.other.capacity };
  }

  function roomKey(v, precision) {
    return roomInfo(v, precision).key;
  }
  ...
  function makeSlot(row, forcedDate, forcedStart, forcedEnd, autoLabel, indexSuffix) {
    ...
    const duration = get(row, ["Durée estimée", "Durée"]);
    const format = get(row, ["FORMAT CRD"]);
    const dur = blockedDuration(format, duration) || durationMinutes(duration, format);
    const end = forcedEnd || (startMin !== null ? timeFromMin(startMin + dur) : "");
    const finalRoom = get(row, ["SALLE RETENUE CRD", "Salle retenue CRD", "Salle retenue"]);
    const fallbackRoom = get(row, ["Lieu souhaité", "Lieu", "Salle"]);
    const precision = get(row, ["PRÉCISION SALLE / LIEU CRD", "PRECISION SALLE / LIEU CRD", "Précision salle", "Precision salle", "Précision lieu", "Precision lieu"]);
    const finalRoomNorm = norm(finalRoom);
    const retainedRoom = finalRoom && !finalRoomNorm.includes("arbitrer") ? finalRoom : fallbackRoom;
    const retainedPrecision = finalRoom && !finalRoomNorm.includes("arbitrer") ? precision : "";
    const room = roomInfo(retainedRoom, retainedPrecision);
    const key = room.key;
    const explicitCapacity = numberFrom(get(row, ["CAPACITÉ CRD", "CAPACITE CRD", "Capacité CRD", "Capacite CRD"]));
    ...
    return {
      ...
      roomKey: key,
      roomLabel: room.label || precision || retainedRoom || (ROOM_DEFAULTS[key] ? ROOM_DEFAULTS[key].label : "Lieu à préciser"),
      capacity,
      estimated,
      realCapacity,
      ...
      pole,
      autoLabel: autoLabel || "",
      autoGenerated: Boolean(autoLabel),
  ...
  function normalizeReservationId(raw) {
    const parts = String(raw || "").split("|").map(x => x.trim()).filter((x, i) => i < 5 || x !== "");
    if (parts.length < 5) return "";
    return parts.slice(0, 5).join("|");
  }

  function slotPrefix(slot) {
    return [slot.dateIso, compactTime(slot.start), compactTime(slot.end), slot.roomKey].join("|") + "|";
  }

  function mergeReservationInto(target, source) {
    if (!source) return target;
    target.confirmed += Number(source.confirmed || 0);
    target.pending += Number(source.pending || 0);
    target.total += Number(source.total || 0);
    if (source.details) target.details = target.details.concat(source.details);
    return target;
  }

  function reservationIdParts(id) {
  ...
  function reservationMatchesSlotId(slot, id) {
    if (!slot || !id) return false;
    const slotId = normalizeReservationId(slotReservationId(slot));
    const target = reservationIdParts(slotId);
    const candidate = reservationIdParts(normalizeReservationId(id));

    if (!target.date || !candidate.date) return false;
    if (candidate.date !== target.date) return false;
    if (candidate.start !== target.start) return false;
    if (candidate.room !== target.room) return false;

    // Correspondance idéale : même fin + même titre.
    if (candidate.end === target.end && candidate.title === target.title) return true;

    // Cas réel constaté : le même événement peut être construit en 14h00-14h30
    // côté écoles et 14h00-15h00 côté public selon FORMAT CRD / durée.
    // On accepte donc date + début + salle + titre identique, même si l'heure de fin diffère.
    if (candidate.title && target.title && candidate.title === target.title) return true;

    // Dernier filet : même date + début + fin + salle, même si le titre a légèrement varié.
    if (candidate.end === target.end) return true;

    return false;
  }

  function getReservationForSlot(slot, reservations) {
    const empty = { confirmed: 0, pending: 0, total: 0, details: [] };
    if (!slot || !reservations) return empty;
    const exact = normalizeReservationId(slotReservationId(slot));
    if (exact && reservations[exact]) return reservations[exact];

    const sum = { confirmed: 0, pending: 0, total: 0, details: [] };
    Object.keys(reservations || {}).forEach(id => {
      if (reservationMatchesSlotId(slot, id)) mergeReservationInto(sum, reservations[id]);
    });
    return sum.total ? sum : empty;
  }

  function schoolReservationStatus(v) {
    const s = norm(v || "");
    if (s === "non" || s.includes("refus") || s.includes("annul")) return "cancelled";
    if (s === "oui" || s.includes("confirm") || s.includes("valide")) return "confirmed";
    return "pending";
  }

  function schoolCountFromIdParts(parts, row) {
    // Les IDs ecoles ont normalement :
    // date|debut|fin|salle|titre|groupe|eleves|adultes
    // Mais si le nom du groupe contient accidentellement un "|", les indices fixes
    // parts[6]/parts[7] deviennent faux. On lit donc les deux derniers blocs numeriques.
    const clean = (parts || []).map(x => String(x || "").trim()).filter(Boolean);
    let students = 0;
    let adults = 0;
    for (let i = clean.length - 1; i >= 0; i--) {
      const n = numberFrom(clean[i]);
      if (!adults && n) { adults = n; continue; }
      if (!students && n) { students = n; break; }
    }
    let total = students + adults;
    if (!total && row) {
      total = numberFrom(get(row, ["Nombre d’élèves", "Nombre d'eleves", "Nombre d'élèves", "Eleves", "Élèves", "Participants", "Nombre de participants"])) +
              numberFrom(get(row, ["Accompagnateurs", "Adultes", "Nombre d’accompagnateurs", "Nombre d'accompagnateurs"]));
    }
    return total;
  }

  function addReservationCount(map, key, status, total, detail) {
    if (!key || !total) return;
    if (!map[key]) map[key] = { confirmed: 0, pending: 0, total: 0, details: [] };
  ...
  function schoolKeysFromId(raw) {
    ...
    const room = parts[3] || "";
    const title = parts[4] || "";
    if (!date || !start || !room) return null;
    return {
      full: [date, start, end, room, title].join("|"),
      title: [date, start, room, title].join("|"),
      loose: [date, start, room].join("|"),
      parts
    };
  }

  function slotSchoolKeys(slot) {
  ...
  function mergeReservation(a, b) {
    const out = {
      confirmed: Number(a && a.confirmed || 0) + Number(b && b.confirmed || 0),
      pending: Number(a && a.pending || 0) + Number(b && b.pending || 0),
      total: Number(a && a.total || 0) + Number(b && b.total || 0),
      details: []
    };
    if (a && a.details) out.details = out.details.concat(a.details);
    if (b && b.details) out.details = out.details.concat(b.details);
    return out;
  }

  function buildSchoolReservations(rows) {
    const map = { byFull: {}, byTitle: {}, byLoose: {}, debug: [] };
    (rows || []).forEach(row => {
      ...
      if (!rawIds) {
        rawIds = Object.keys(row || {}).map(k => row[k]).find(cell => /2026-06-\d{2}\|\d{3,4}\|\d{3,4}\|/.test(String(cell || ""))) || "";
      }

      String(rawIds).split(/[;\n]+/).map(x => x.trim()).filter(Boolean).forEach(line => {
        const keys = schoolKeysFromId(line);
        if (!keys) return;
        const total = schoolCountFromIdParts(keys.parts, row);
        if (!total) return;
        const detail = { id: keys.full, total, status };
        addReservationCount(map.byFull, keys.full, status, total, detail);
        addReservationCount(map.byTitle, keys.title, status, total, detail);
        addReservationCount(map.byLoose, keys.loose, status, total, detail);
        map.debug.push(detail);
      });
    });
    return map;
  }

  function getSchoolReservationForSlot(slot, schoolReservations) {
    const empty = { confirmed: 0, pending: 0, total: 0, details: [] };
    if (!slot || !schoolReservations) return empty;
    const keys = slotSchoolKeys(slot);

    // Ordre volontaire : exact, puis meme titre avec duree differente,
    // puis secours date+debut+salle pour les cas type Bernstein 14h00.
    return (schoolReservations.byFull && schoolReservations.byFull[keys.full]) ||
           (schoolReservations.byTitle && schoolReservations.byTitle[keys.title]) ||
           (schoolReservations.byLoose && schoolReservations.byLoose[keys.loose]) ||
           empty;
  }
  ...
  function buildPublicReservations(rows) {
    ...
    rows.forEach(row => {
      ...
      ids.forEach(raw => {
        const id = normalizeReservationId(raw);
        if (!id) return;
  ...
  function applyPublicReservations(slots, publicReservations, schoolReservations) {
    const publicMap = publicReservations || {};
    const schoolMap = schoolReservations || {};
    return slots.map(slot => {
      try {
        const technicalId = normalizeReservationId(slotTechnicalId(slot));
        const p = getReservationForSlot(Object.assign({}, slot, { reservationId: technicalId }), publicMap);
        const sc = getSchoolReservationForSlot(Object.assign({}, slot, { reservationId: technicalId }), schoolMap);
        const publicOccupied = Number(p.total || 0);
        const schoolOccupied = Number(sc.total || 0);
        const occupied = publicOccupied + schoolOccupied;
        const remaining = Math.max(0, Number(slot.capacity || 0) - occupied);
        return Object.assign({}, slot, {
          reservationId: technicalId,
          publicConfirmed: Number(p.confirmed || 0),
          publicPending: Number(p.pending || 0),
          ...
          totalReserved: occupied,
          publicRemaining: remaining,
          publicReservationDetails: p.details || [],
  ...
  function buildSlots(rows) {
    ...
    const cleaned = removeAutoSlotsWithRoomConflicts(slots);
    const seen = new Set();
  ...
  Promise.all([
    ...
    .then(([propText, publicText, demandesText]) => {
      const propositionSlots = buildSlots(parseCSV(propText));
      try {
        publicReservationsMap = buildPublicReservations(parseCSV(publicText || ""));
        const schoolReservationsMap = buildSchoolReservations(parseCSV(demandesText || ""));
        allSlots = applyPublicReservations(propositionSlots, publicReservationsMap, schoolReservationsMap);
        window.JPO_PUBLIC_DEBUG = {
          slots: allSlots,
          publicReservations: publicReservationsMap,
          schoolReservations: schoolReservationsMap,
          schoolReservationCount: schoolReservationsMap && schoolReservationsMap.debug ? schoolReservationsMap.debug.length : 0,
          bernstein: allSlots.filter(s => norm(s.title).includes("bernstein"))
        };
      } catch (err) {
        console.error("Erreur réservations JPO", err);
        try {
          publicReservationsMap = buildPublicReservations(parseCSV(publicText || ""));
        ...
        $("jpo-public-error").textContent = "Information : certaines réservations n’ont pas pu être lues correctement. Le programme reste affiché et les boutons de réservation restent disponibles.";
      }
      renderControls();
      bind();
      render();
