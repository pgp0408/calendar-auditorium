(function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <div class="freebox"><strong>Entrée libre et gratuite</strong>, dans la limite des places disponibles. Aucune réservation n’est nécessaire depuis cette page. Pour toute question ou pour être orienté vers les propositions les plus adaptées, contactez-nous :
  ...
  const PRIORITY_HIGHLIGHTS = [
    { label: "Concert des orchestres", terms: ["concert des orchestres"] },
    { label: "Audition piano", terms: ["audition piano", "audition de piano"] },
    { label: "Audition clavecin / guitares", terms: ["audition clavecin", "audition de clavecin", "clavecin guitare", "clavecin guitares"] },
    { label: "Chœurs", terms: ["choeurs d enfants", "choeur d enfants", "chœurs d enfants", "chœur d enfants", "choeur chav", "chœur chav", "choeurs", "chœurs"] },
    { label: "BRONX", terms: ["bronx"] }
  ];

  function priorityHighlightRank(slot) {
    const hay = norm([slot.title, slot.description, slot.type, slot.discipline, slot.roomLabel].join(" "));
    for (let i = 0; i < PRIORITY_HIGHLIGHTS.length; i++) {
      if (PRIORITY_HIGHLIGHTS[i].terms.some(term => hay.includes(norm(term)))) return i;
    }
    return 999;
  }
  ...
  function priorityHighlights(slots) {
    const selected = [];
    const selectedKeys = new Set();

    slots
      .filter(slot => priorityHighlightRank(slot) < 999)
      .sort((a, b) => priorityHighlightRank(a) - priorityHighlightRank(b) || compareSlots(a, b))
      .forEach(slot => {
  ...
  function renderFocus(slots) {
    const highlights = priorityHighlights(slots).slice(0, 6);
    if (!highlights.length) { $("jpo-public-focus-section").innerHTML = ""; return; }
    $("jpo-public-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Temps forts de la semaine</div><div class="focus-sub">Sélection éditoriale : concerts des orchestres, audition piano, audition clavecin / guitares, chœurs et BRONX. Les autres propositions ne remontent plus dans ce bandeau, et une même programmation n’apparaît qu’une seule fois.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
