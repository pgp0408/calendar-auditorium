document.addEventListener("DOMContentLoaded", function () {
  ...
  const DAYS = [
    ["2026-06-15", "Lundi", "15/06"],
    ["2026-06-16", "Mardi", "16/06"],
    ["2026-06-17", "Mercredi", "17/06"],
    ["2026-06-18", "Jeudi", "18/06"],
    ["2026-06-19", "Vendredi", "19/06"],
    ["2026-06-20", "Samedi", "20/06"],
    ["2026-06-21", "Dimanche", "21/06"]
  ];

  const ROOMS = [
    { key: "auditorium", label: "Auditorium", capacity: 200 },
    { key: "orchestre", label: "Salle d’orchestre", capacity: 40 },
    { key: "chant", label: "Salle de chant", capacity: 40 },
    { key: "danse", label: "Studio danse", capacity: 40 },
    { key: "theatre", label: "Studio théâtre", capacity: 40 },
    { key: "exterieur", label: "Extérieur", capacity: 0 },
    { key: "autre", label: "Autre / à définir", capacity: 0 }
  ];

  const MOMENTS = [
    { key: "journee", label: "Journée entière" },
    { key: "matin", label: "Matin / scolaires" },
    { key: "apresmidi", label: "Après-midi / scolaires" },
    { key: "soir", label: "Soirée / tout public" }
  ];
  ...
  root.innerHTML = `
    <style>
      ...
      #semainejps-calendar .jps-dot.allDay{background:#7c3aed}
      ...
      #semainejps-calendar .jps-main-title{
        display:flex;
      ...
      #semainejps-calendar .jps-main-title h3{
        margin:0;
        font-size:24px;
        font-weight:900;
        letter-spacing:-.02em;
      ...
      #semainejps-calendar .jps-event.allday{box-shadow:0 0 0 2px rgba(124,58,237,.18),0 8px 18px rgba(15,23,42,.11)}
  ...
  function isAllDay(v){
    const s = norm(v);
    return s.includes("journee entiere") || s.includes("journee") || s.includes("toute la journee");
  }

  function roomInfo(v){
    const s = norm(v);

    if(s.includes("auditorium")) return { key:"auditorium", label:"Auditorium", capacity:200 };
    if(s.includes("orchestre")) return { key:"orchestre", label:"Salle d’orchestre", capacity:40 };
    if(s.includes("chant")) return { key:"chant", label:"Salle de chant", capacity:40 };
    if(s.includes("danse")) return { key:"danse", label:"Studio danse", capacity:40 };
    if(s.includes("theatre") || s.includes("théâtre")) return { key:"theatre", label:"Studio théâtre", capacity:40 };
    if(s.includes("exterieur") || s.includes("extérieur")) return { key:"exterieur", label:"Extérieur", capacity:0 };

    return { key:"autre", label:v || "Autre / à définir", capacity:0 };
  }
  ...
  function openModal(ev){
    ...
    $("jps-m-room-capacity").textContent = ev.roomCapacity ? ev.roomCapacity + " places" : "À définir";
  ...
  function renderAlerts(data){
    ...
    const emptyDays = DAYS
      .filter(([iso]) => !data.some(e => e.dateIso === iso))
      .map(([,label,short]) => label + " " + short);
    ...
    data.forEach(e => {
      ...
      overloadMap[key] = (overloadMap[key] || 0) + 1;
    });

    const conflicts = Object.values(overloadMap).filter(n => n > 1).length;
    const allDayCount = data.filter(e => e.allDay).length;

    alerts.innerHTML =
      '<div class="jps-alert ' + (emptyDays.length ? 'empty' : 'ok') + '"><strong>Jours sans proposition</strong>' + (emptyDays.length ? esc(emptyDays.join(", ")) : "Aucun jour totalement vide.") + '</div>' +
      '<div class="jps-alert ' + (schoolCount < 4 ? 'low' : 'ok') + '"><strong>Offre scolaires</strong>' + schoolCount + ' proposition(s) identifiée(s) sur les jours scolaires.</div>' +
      '<div class="jps-alert ' + (conflicts ? 'low' : 'ok') + '"><strong>Parallèles / conflits</strong>' + (conflicts ? conflicts + ' chevauchement(s) dans une même salle.' : 'Plusieurs événements peuvent coexister si les salles diffèrent.') + '<br>Journée entière : ' + allDayCount + '</div>';
  }
  ...
  function renderEvent(ev){
    ...
    btn.innerHTML =
      ...
      '</span>' +
      '<span class="jps-title">' + esc(ev.title) + '</span>' +
      '<span class="jps-meta">' + esc(ev.publicTarget || "Public à préciser") + '</span>';
  ...
  function renderWeek(){
    ...
    DAYS.forEach(([iso, dayLabel, shortDate]) => {
      const dayEvents = data.filter(e => e.dateIso === iso);

      const day = document.createElement("section");
      day.className = "jps-day";

      day.innerHTML =
        '<div class="jps-dayhead">' +
          '<div class="jps-dayname">' + esc(dayLabel) + '</div>' +
          '<div class="jps-daydate">' + esc(shortDate) + '</div>' +
        '</div>' +
        '<div class="jps-daybody"></div>';

      const body = day.querySelector(".jps-daybody");
          ...
          ROOMS.forEach(room => {
            ...
            roomBox.innerHTML =
              '<div class="jps-roomhead">' +
                ...
                '<span class="jps-capacity">' + (room.capacity ? room.capacity + " places" : "capacité à préciser") + '</span>' +
              '</div>' +
    ...
    .then(text => {
      ...
      allEvents = dataRows.map(row => {
        ...
        return {
          ...
          dateIso,
          time: allDay ? "Journée entière" : formatTime(row[C_TIME]),
          duration,
          allDay,
