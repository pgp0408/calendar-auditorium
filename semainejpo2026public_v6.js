document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <p class="sub">Du 15 au 21 juin 2026, le Conservatoire Henri Tomasi vous accueille pour découvrir ses disciplines, rencontrer les équipes et assister à de nombreux rendez-vous ouverts au public.</p>
        <div class="intro"><strong>Bienvenue aux Journées Portes Ouvertes.</strong><br>Le Conservatoire vous accueille les lundi, mardi, jeudi et vendredi en fin de journée, à partir de 17h30 et jusqu’à 20h, avec de nombreuses propositions accessibles au public. Le mercredi, les rendez-vous sont ouverts tout au long de la journée. Le samedi, l’accueil se poursuit jusqu’à 16h, avant le grand temps de clôture du dimanche, de 16h à 20h, autour de la Fête de la musique.</div>
        <div class="freebox">Accès libre et gratuit, dans la limite des places disponibles. Aucune réservation n’est possible depuis cette page. Pour plus d’informations, contactez-nous : <a href="mailto:communication@crd.corsica">communication@crd.corsica</a> — <a href="tel:+33667579265">06.67.57.92.65</a>.</div>
      </header>
      <main class="content">
  ...
  function renderFocus(slots) {
    ...
    slots.filter(isHighlight).forEach(slot => {
      const key = norm([slot.title, slot.roomKey, slot.type].join("|"));
      if (seenPrograms.has(key)) return;
      seenPrograms.add(key);
      highlights.push(slot);
    });
    const selected = highlights.slice(0, 6);
    if (!selected.length) { $("jpo-public-focus-section").innerHTML = ""; return; }
    $("jpo-public-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Temps forts de la semaine</div><div class="focus-sub">Une sélection non répétitive des grands rendez-vous : auditorium ou grandes salles, sans doublonner une même programmation proposée à plusieurs horaires.</div></div><div class="badge">' + selected.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + selected.map(renderCard).join("") + '</div></div>';
  }
