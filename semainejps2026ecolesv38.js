(function () {
  // V38 : encards publics enrichis avec propositions professeurs, réservations scolaires et visiteurs uniques estimés.
  ...
  let schoolReservationMapsForStats = { exact: {}, dateStartRoomTitle: {}, dateStartTitle: {}, dateStartRoom: {}, debug: [] };
  let publicCart = [];
  ...
  function emptySchoolCount() {
    return { confirmed: 0, pending: 0, total: 0, details: [] };
  }

  function addSchoolReservation(map, key, status, total, detail) {
    if (!key || !total) return;
    ...
    (rows || []).forEach(row => {
      try {
        ...
        splitReservationIds(rawIds).forEach(rawId => {
          ...
          if (!total) return;
          const parts = String(rawId || "").split("|").map(x => String(x || "").trim());
          const establishment = get(row, ["Établissement", "Etablissement", "Structure", "École", "Ecole", "Organisme"]);
          const group = parts[5] || get(row, ["Classe / groupe", "Classe", "Groupe"]) || "Groupe scolaire";
          const detail = { establishment, group, total, status, raw: rawId, id: info.exact };
          addSchoolReservation(maps.exact, info.exact, status, total, detail);
          addSchoolReservation(maps.dateStartRoomTitle, [info.date, info.start, info.room, info.title].join("|"), status, total, detail);
          addSchoolReservation(maps.dateStartTitle, [info.date, info.start, info.title].join("|"), status, total, detail);
          addSchoolReservation(maps.dateStartRoom, [info.date, info.start, info.room].join("|"), status, total, detail);
          maps.debug.push(detail);
        });
      } catch (err) {
  ...
  function withPublicReservationDefaults(slot) {
    ...
    return Object.assign({}, slot, {
      ...
      schoolReservationDetails: slot && slot.schoolReservationDetails ? slot.schoolReservationDetails : [],
    ...
    return slots.map(slot => {
      try {
        ...
        return Object.assign({}, slot, {
          ...
          schoolReservationDetails: sc.details || [],
  ...
  function schoolUniqueEstimate(slots) {
    const groups = {};
    let fallback = 0;
    (slots || []).forEach(slot => {
      const details = slot.schoolReservationDetails || [];
      if (!details.length) fallback += Number(slot.schoolReserved || 0);
      details.forEach((detail, index) => {
        const key = norm([detail && detail.establishment, detail && detail.group].filter(Boolean).join("|")) || ("scolaire-" + index);
        groups[key] = Math.max(groups[key] || 0, Number(detail.total || 0));
      });
    });
    const estimated = Object.keys(groups).reduce((sum, key) => sum + groups[key], 0);
    return estimated || fallback;
  }

  function uniqueGroupCount(slots) {
    const groups = {};
    (slots || []).forEach(slot => {
      (slot.schoolReservationDetails || []).forEach((detail, index) => {
        const key = norm([detail && detail.establishment, detail && detail.group].filter(Boolean).join("|")) || ("scolaire-" + index);
        groups["school:" + key] = true;
      });
      (slot.publicReservationDetails || []).forEach((detail, index) => {
        const key = personKey(detail, index);
        groups["public:" + key] = true;
      });
    });
  ...
  function renderStats(slots) {
    ...
    const professorPrograms = new Set(slots.map(s => slug(s.title || "") + "|" + (s.name || "") + "|" + (s.roomKey || ""))).size;
    const publicTaken = slots.reduce((sum, s) => sum + Number(s.publicReserved || 0), 0);
    const schoolTaken = slots.reduce((sum, s) => sum + Number(s.schoolReserved || 0), 0);
    const totalTaken = publicTaken + schoolTaken;
    const remaining = slots.reduce((sum, s) => sum + Number(s.publicRemaining || 0), 0);
    const full = slots.filter(s => s.publicFull).length;
    const publicUnique = publicUniqueEstimate(slots);
    const schoolUnique = schoolUniqueEstimate(slots);
    const totalUnique = publicUnique + schoolUnique;
    const uniqueReservations = uniqueGroupCount(slots);
    const peaks = peakMoments(slots, 3);
    const maxPeak = peaks.length ? peaks[0].total : 0;
    $("jpo-public-stats").innerHTML =
      '<div class="stat statgroup"><div class="stat-title">Programme public</div><div class="stat-main"><div class="num">' + professorPrograms + '</div><span>propositions professeurs distinctes</span></div><div class="mini"><div><b>' + slots.length + '</b><span>créneaux publics affichés</span></div><div><b>' + highlights + '</b><span>temps forts</span></div><div><b>' + aud + '</b><span>auditorium</span></div><div><b>' + full + '</b><span>complets</span></div></div></div>' +
      '<div class="stat statgroup"><div class="stat-title">Places réservées</div><div class="stat-main"><div class="num">' + totalTaken + '</div><span>public + scolaires</span></div><div class="mini"><div><b>' + publicTaken + '</b><span>tout public</span></div><div><b>' + schoolTaken + '</b><span>scolaires</span></div><div><b>' + totalUnique + '</b><span>visiteurs uniques estimés</span></div><div><b>' + uniqueReservations + '</b><span>réservations / groupes uniques</span></div></div></div>' +
      '<div class="stat statgroup"><div class="stat-title">Moments d’influence</div><div class="stat-main"><div class="num">' + maxPeak + '</div><span>personnes réservées au plus fort</span></div><div class="peaklist">' + (peaks.length ? peaks.map(p => '<button type="button" class="peakbtn" data-open-peak="' + esc(p.key) + '">' + esc(formatDate(p.dateIso)) + ' · ' + esc(showTime(p.start)) + ' — ' + p.total + ' pers. (' + p.count + ' créneau(x))</button>').join("") : '<div class="lab">Aucune réservation enregistrée avec ces filtres.</div>') + '</div><div class="mini" style="margin-top:7px"><div><b>' + remaining + '</b><span>places restantes</span></div><div><b>180</b><span>jauge auditorium max</span></div></div></div>';
  }
  ...
  Promise.all([
    ...
    .then(([propText, publicText, schoolText]) => {
      ...
      try {
        ...
        schoolReservationMapsForStats = schoolMaps;
      } catch (err) {
