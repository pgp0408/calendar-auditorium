(function () {
  ...
  function buildSchoolReservations(rows) {
    ...
    (rows || []).forEach(row => {
      ...
      let rawIds = get(row, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux", "ID créneau", "ID creneau"]);
      if (!rawIds) {
        rawIds = Object.keys(row || {}).map(k => row[k]).find(cell => /2026-06-\d{2}\|\d{3,4}\|\d{3,4}\|/.test(String(cell || ""))) || "";
      }
      String(rawIds).split(/[;\n]+/).map(x => x.trim()).filter(Boolean).forEach(line => {
        const parts = line.split("|").map(x => x.trim());
        const id = normalizeReservationId(line);
        if (!id) return;
        let total = 0;
        if (parts.length >= 8) total = numberFrom(parts[6]) + numberFrom(parts[7]);
        if (!total) {
          total = numberFrom(get(row, ["Nombre d’élèves", "Nombre d'eleves", "Élèves", "Eleves", "Participants"])) + numberFrom(get(row, ["Accompagnateurs", "Adultes"]));
        }
        if (!total) return;
        if (!map[id]) map[id] = { confirmed: 0, pending: 0, total: 0, details: [] };
        if (status === "confirmed") map[id].confirmed += total;
  ...
  function applyPublicReservations(slots, publicReservations, schoolReservations) {
    ...
    return slots.map(slot => {
      try {
        ...
        const sc = getReservationForSlot(Object.assign({}, slot, { reservationId: technicalId }), schoolMap);
