document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <div class="intro"><strong>Logique retenue :</strong> auditorium affiché systématiquement ; autres créneaux affichés uniquement lorsque le cumul des réservations confirmées atteint au moins ${AFFLUENCE_THRESHOLD} personnes sur le même horaire exact, sans additionner les créneaux qui se chevauchent.</div>
      </header>
      <main class="content">
  ...
  function confirmedAtMoment(slot){return allSlots.filter(s=>s.dateIso===slot.dateIso && fmtTime(s.start)===fmtTime(slot.start)).reduce((sum,s)=>sum+(s.confirmed||0),0);}
  ...
  function render(){const slots=filtered(); renderStats(slots); const status=$("jpo-partners-status"); status.className=""; status.innerHTML=''; if(!slots.length){$("jpo-partners-output").innerHTML='<div class="empty">Aucun repère avec ces filtres. Diagnostic : '+allSlots.length+' propositions chargées, '+allSlots.filter(s=>s.roomKey==="auditorium").length+' événements auditorium détectés.</div>';return;} const days=(activeDay==="all"?DAYS:DAYS.filter(d=>d.iso===activeDay)); $("jpo-partners-output").innerHTML=days.map(d=>{const ds=slots.filter(s=>s.dateIso===d.iso); if(!ds.length)return ''; const by={}; ds.forEach(s=>{const k=showTime(s.start); (by[k]||(by[k]=[])).push(s);}); return '<section class="section"><div class="sectionhead"><div><div class="sectiontitle">'+esc(d.label)+'</div><div class="sectionmeta">Auditorium et pics d’affluence confirmée.</div></div><div class="badge">'+ds.length+' repère(s)</div></div><div class="body">'+Object.keys(by).sort((a,b)=>(mins(a)||9999)-(mins(b)||9999)).map(t=>'<div class="timegroup"><div class="timehead"><div class="time">'+esc(t)+'</div><div class="badge">'+by[t].length+'</div></div><div class="cards">'+by[t].map(renderCard).join('')+'</div></div>').join('')+'</div></section>';}).join('') || '<div class="empty">Aucun repère pour ce jour.</div>';}
