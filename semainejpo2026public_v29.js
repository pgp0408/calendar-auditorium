(function () {
  ...
  function schoolReservationStatus(v) {
    const s = norm(v || "");
    if (s.includes("annul") || s.includes("refus") || s === "non") return "cancelled";
    if (s.includes("confirm") || s.includes("valid") || s === "oui") return "confirmed";
    return "pending";
  }

  function splitSchoolIds(v) {
    return String(v || "")
      .split(";")
      .map(x => x.trim())
      .filter(Boolean);
  ...
  function schoolBaseId(rawId) {
    const parts = String(rawId || "").split("|").map(x => x.trim());
    if (parts.length < 5) return "";
    return parts.slice(0, 5).join("|");
  }

  function schoolPrefixId(rawId) {
    const parts = String(rawId || "").split("|").map(x => x.trim());
    if (parts.length < 4) return "";
    return parts.slice(0, 4).join("|") + "|";
  }

  function schoolTotalFromId(rawId) {
    const parts = String(rawId || "").split("|").map(x => x.trim());
    if (parts.length >= 8) return numberFrom(parts[6]) + numberFrom(parts[7]);
    return 0;
  }

  function firstCellMatching(row, pattern) {
  ...
  function buildSchoolReservations(rows) {
    ...
    (rows || []).forEach(row => {
      try {
        const validation = schoolReservationStatus(get(row, ["VALIDATION CRD", "Validation CRD", "STATUT", "Statut"]));
        if (validation === "cancelled") return;

        const idsRaw = get(row, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux", "ID créneau", "ID creneau"]) ||
          firstCellMatching(row, /2026-06-\d{2}\|\d{4}\|\d{4}\|/);
        if (!idsRaw) return;

        splitSchoolIds(idsRaw).forEach(rawId => {
          const baseId = schoolBaseId(rawId);
          if (!baseId) return;
          let total = schoolTotalFromId(rawId);
          if (!total) {
            total = numberFrom(get(row, ["Nombre total", "Effectif total", "Total", "Effectif"])) ||
              numberFrom(get(row, ["Nombre d'élèves", "Nombre eleves", "Élèves", "Eleves", "Participants"])) +
              numberFrom(get(row, ["Accompagnateurs", "Nombre d'accompagnateurs", "Adultes"]));
          }
          if (!total) return;

          if (!map[baseId]) map[baseId] = { confirmed: 0, pending: 0, total: 0 };
          if (validation === "confirmed") map[baseId].confirmed += total;
          else map[baseId].pending += total;
          map[baseId].total += total;
        });
      } catch (err) {
        console.warn("Ligne réservation écoles ignorée côté public", err);
      }
    });
    return map;
  }
  ...
  function applySchoolReservationsToPublicSlots(slots, schoolMap) {
    if (!schoolMap || !Object.keys(schoolMap).length) return slots;
    return (slots || []).map(slot => {
      try {
        const technicalId = slotReservationId(slot);
        const prefix = schoolPrefixId(technicalId);
        let schoolConfirmed = 0;
        let schoolPending = 0;

        Object.keys(schoolMap).forEach(id => {
          if (id === technicalId || (prefix && id.indexOf(prefix) === 0)) {
            schoolConfirmed += schoolMap[id].confirmed || 0;
            schoolPending += schoolMap[id].pending || 0;
          }
        });

        const schoolReserved = schoolConfirmed + schoolPending;
        if (!schoolReserved) return slot;

        const publicReserved = Number(slot.publicReserved || 0);
        const remaining = Math.max(0, Number(slot.capacity || 0) - publicReserved - schoolReserved);
  ...
  Promise.all([
    ...
    .then(([propText, publicText, schoolText]) => {
      ...
      try {
        const schoolMap = buildSchoolReservations(parseCSV(schoolText || ""));
