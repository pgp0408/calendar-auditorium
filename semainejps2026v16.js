document.addEventListener("DOMContentLoaded", function () {
  ...
  function openDayModal(dayIso){
    ...
    const usedRoomKeys={};
    dayEvents.forEach(ev=>{ if(ev.roomKey) usedRoomKeys[ev.roomKey]=true; });
    let dayRooms=ROOMS.filter(room=>usedRoomKeys[room.key]);
    if(!dayRooms.length) dayRooms=ROOMS.slice(0,1);

    $("jps-day-modal-title").textContent=day.name+" "+day.short;
    $("jps-day-modal-sub").textContent="Planning par créneaux de 30 minutes. Seules les salles mobilisées ce jour-là sont affichées pour garder une lecture lisible.";
    ...
    table.style.gridTemplateColumns="95px repeat("+dayRooms.length+", minmax(170px, 1fr))";
    table.style.minWidth=(95 + dayRooms.length * 170) + "px";
