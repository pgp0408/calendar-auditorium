document.addEventListener("DOMContentLoaded", function () {
  ...
  const STORAGE_KEY = "jps_ecoles_v27_panier";
  const LAST_GROUP_KEY = "jps_ecoles_v27_last_group";
    ...
    <div class="wrap">
      ...
      <div class="content">
        ...
        <div class="tools">
          ...
          <div class="toolbox">
            <label>Assistant parcours</label>
            <div class="quick-buttons">
              <button type="button" data-suggest-duration="60" data-suggest-period="all">Parcours 1h</button>
              <button type="button" data-suggest-duration="90" data-suggest-period="all">Parcours 1h30</button>
              <button type="button" data-suggest-duration="120" data-suggest-period="all">Parcours 2h</button>
              <button type="button" data-suggest-duration="60" data-suggest-period="morning">Matin 1h</button>
              <button type="button" data-suggest-duration="90" data-suggest-period="morning">Matin 1h30</button>
              <button type="button" data-suggest-duration="60" data-suggest-period="afternoon">Après-midi 1h</button>
              <button type="button" data-suggest-duration="90" data-suggest-period="afternoon">Après-midi 1h30</button>
            </div>
          </div>
        </div>
  ...
  function findAutoRoute(minutesWanted, group, totalPeople) {
    const candidates = visibleSlotsForActiveDay()
  ...
  function renderSuggestions(slots) {
    ...
    const configs = [
      { min: 60, period: "morning", label: "Matin — parcours 1h" },
      { min: 90, period: "morning", label: "Matin — parcours 1h30" },
      { min: 120, period: "morning", label: "Matin — parcours 2h" },
      { min: 60, period: "afternoon", label: "Après-midi — parcours 1h" },
      { min: 90, period: "afternoon", label: "Après-midi — parcours 1h30" },
      { min: 120, period: "afternoon", label: "Après-midi — parcours 2h" }
    ];
    ...
    box.innerHTML = suggestions.slice(0, 6).map(s =>
      ...
      '<section class="day">' +
        '<div class="dayhead">' +
          '<div>' +
            ...
            '<div class="daymeta">Choisissez une demi-heure de départ, puis ajoutez une activité au parcours d’une classe.</div>' +
          '</div>' +
  ...
  function renderNextSuggestions(slot) {
    ...
    box.style.display = "block";
    box.innerHTML =
      '<div class="nextbox-title">Après ce créneau, autres possibilités sur la même demi-journée</div>' +
      '<div class="nextbox-help">Ajoutez d’abord ce créneau au panier si vous souhaitez le conserver, puis choisissez éventuellement une suite de parcours.</div>' +
      '<div class="nextlist">' +
        next.map(candidate =>
          '<div class="nextitem">' +
            ...
            '<button type="button" class="btn2" data-next-slot-id="' + esc(candidate.id) + '">Voir ce créneau ensuite</button>' +
          '</div>'
        ).join("") +
      '</div>';
  }
  ...
  root.querySelectorAll("[data-suggest-duration]").forEach(button => {
    button.addEventListener("click", function () {
      openAutoBuilder(
        parseInt(button.getAttribute("data-suggest-duration"), 10),
        button.getAttribute("data-suggest-period") || "all"
      );
    });
  });
