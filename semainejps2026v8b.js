document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    <style>
      ...
      #semainejps-calendar .jps-section{margin-bottom:28px}
      ...
      #semainejps-calendar .jps-day-detail{
        display:flex;
        flex-direction:column;
        gap:14px;
      }

      #semainejps-calendar .jps-day-moment{
      ...
      #semainejps-calendar .jps-day-moment-head{
        display:flex;
        justify-content:space-between;
        gap:12px;
        align-items:center;
      ...
      #semainejps-calendar .jps-day-moment-title{
        font-size:14px;
        font-weight:900;
        text-transform:uppercase;
        color:#334155;
      ...
      #semainejps-calendar .jps-day-moment-body{
        padding:14px;
      }

      #semainejps-calendar .jps-roomgroup{margin-bottom:14px}
      #semainejps-calendar .jps-roomgroup:last-child{margin-bottom:0}
    ...
    <div class="jps-card">
        ...
        <div class="jps-header-grid">
          <div>
            ...
            <p class="jps-sub">
              ...
              La V8b distingue le temps scolaire réel : lundi, mardi, jeudi et vendredi, 08h30–12h00 et 13h00–17h00.
            </p>
          </div>
      ...
      <div class="jps-content">
        ...
        <section class="jps-section">
          <div class="jps-titlebar">
            <div>
              ...
              <p>Lundi, mardi, jeudi et vendredi : seuls les créneaux 08h30–12h00 et 13h00–17h00 sont comptés comme temps scolaire.</p>
            </div>
        ...
        <section class="jps-section">
          <div class="jps-titlebar">
            <div>
              <h3>Frise tout public / hors temps scolaire</h3>
              <p>Mercredi, samedi, dimanche, soirées et propositions hors créneaux scolaires.</p>
            </div>
        ...
        <section class="jps-section">
          <div class="jps-titlebar">
            <div>
              ...
              <p>Cliquez sur un jour pour ouvrir le détail complet par moment de journée et par salle.</p>
            </div>
    ...
    <div id="jps-day-modal" class="jps-day-modal">
      <div class="jps-day-modalbox">
        ...
        <div id="jps-day-detail" class="jps-day-detail"></div>
      </div>
    </div>
  ...
  function renderAlerts(data){
    ...
    const schoolSlots = [
      { label:"Lundi matin", date:"2026-06-15", start:"08:30", end:"12:00" },
      { label:"Lundi après-midi", date:"2026-06-15", start:"13:00", end:"17:00" },
      { label:"Mardi matin", date:"2026-06-16", start:"08:30", end:"12:00" },
      { label:"Mardi après-midi", date:"2026-06-16", start:"13:00", end:"17:00" },
      { label:"Jeudi matin", date:"2026-06-18", start:"08:30", end:"12:00" },
      { label:"Jeudi après-midi", date:"2026-06-18", start:"13:00", end:"17:00" },
      { label:"Vendredi matin", date:"2026-06-19", start:"08:30", end:"12:00" },
      { label:"Vendredi après-midi", date:"2026-06-19", start:"13:00", end:"17:00" }
    ];

    const emptySchoolSlots = schoolSlots.filter(slot => {
      return !schoolTimeEvents.some(ev => {
        if(ev.dateIso !== slot.date) return false;
    ...
    $("jps-alerts").innerHTML =
      '<div class="jps-alert ' + (emptySchoolSlots.length ? "empty" : "ok") + '">' +
        '<strong>Créneaux scolaires vides hors expo permanente Cuivres</strong>' +
        (emptySchoolSlots.length ? esc(emptySchoolSlots.map(s => s.label).join(", ")) : "Tous les créneaux scolaires ont au moins une proposition hors fil rouge.") +
      '</div>' +
      '<div class="jps-alert ' + (schoolTimeEvents.length < 4 ? "low" : "ok") + '">' +
        '<strong>Offre temps scolaire réelle</strong>' +
      ...
      '<div class="jps-alert ' + (conflicts ? "low" : "ok") + '">' +
        '<strong>Parallèles / conflits</strong>' +
  ...
  function slotSummary(dayIso, momentKey, data){
    ...
    if((momentKey === "matin" || momentKey === "apresmidi") && isSchoolDay(dayIso)){
      const schoolRelevantEvents = realEvents.filter(isSchoolTime);

      if(schoolRelevantEvents.length === 1){
        scoreClass = "low";
        scoreText = "1 scolaire";
      } else if(schoolRelevantEvents.length === 2){
        scoreClass = "ok";
        scoreText = "2 scolaires";
      } else if(schoolRelevantEvents.length >= 3){
        scoreClass = "full";
        scoreText = schoolRelevantEvents.length + " scolaires";
      } else if(permanentEvents.length){
        scoreClass = "low";
        scoreText = "Fil rouge seul";
      }
    } else {
  ...
  function openDayModal(dayIso){
    ...
    $("jps-day-modal-sub").textContent = dayEvents.length + " proposition(s) affichée(s), dont " + dayEvents.filter(isRealProgramming).length + " hors fil rouge.";
    ...
    MOMENTS.forEach(moment => {
      ...
      const momentBox = document.createElement("section");
      momentBox.className = "jps-day-moment";

      momentBox.innerHTML =
        '<div class="jps-day-moment-head">' +
          '<span class="jps-day-moment-title">' + esc(moment.label) + '</span>' +
          '<span class="jps-count">' + momentEvents.length + ' proposition(s)</span>' +
        '</div>' +
        '<div class="jps-day-moment-body"></div>';

      const body = momentBox.querySelector(".jps-day-moment-body");

      if(!momentEvents.length){
        body.innerHTML = '<div class="jps-empty">Créneau à combler</div>';
      } else {
        const rooms = {};
        momentEvents.forEach(ev => {
          if(!rooms[ev.roomKey]){
            rooms[ev.roomKey] = { label: ev.roomLabel, capacity: ev.roomCapacity, events: [] };
          }
        ...
        Object.keys(rooms).forEach(key => {
          ...
          const wrap = group.querySelector(".jps-events");
          room.events.forEach(ev => wrap.appendChild(renderEvent(ev)));
          body.appendChild(group);
        });
      }

      detail.appendChild(momentBox);
    });
  ...
  function renderDayButtons(data){
    ...
    DAYS.forEach(day => {
      ...
      const warningClass = realEvents.length ? "ok" : "empty";
      const warningText = isSchoolDay(day.iso)
        ? (schoolEvents.length ? schoolEvents.length + " proposition(s) sur temps scolaire" : "À combler sur temps scolaire")
        : (realEvents.length ? realEvents.length + " proposition(s) tout public / hors temps scolaire" : "À combler hors expo permanente");
  ...
  function refresh(){
    ...
    $("jps-public-count").textContent = publicCount + " proposition(s) hors temps scolaire";
  }
