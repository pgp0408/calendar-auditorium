(function () {
  ...
  function findSchoolCol(headers, names) {
    const hs = (headers || []).map(norm);
    for (const name of names) {
      const n = norm(name);
      let idx = hs.findIndex(h => h === n);
      if (idx !== -1) return idx;
      idx = hs.findIndex(h => h.includes(n));
      if (idx !== -1) return idx;
    }
    return -1;
  }

  function schoolStatus(v) {
    const s = norm(v || "");
    if (s.includes("non") || s.includes("annul") || s.includes("refus")) return "cancelled";
    if (s.includes("oui") || s.includes("confirm")) return "confirmed";
    return "pending";
  }

  function normalizeReservationKey(id) {
    const parts = String(id || "").split("|").map(x => x.trim());
    if (parts.length < 5) return "";
    return parts.slice(0, 5).join("|");
  }

  function buildSchoolReservationsFromCsv(text) {
    const map = {};
    try {
      ...
      const cIds = findSchoolCol(headers, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux", "ID créneau", "ID creneau"]);
      const cValid = findSchoolCol(headers, ["VALIDATION CRD", "Validation CRD", "STATUT", "Statut"]);
      const cStudents = findSchoolCol(headers, ["Nombre d'élèves", "Nombre eleves", "Élèves", "Eleves", "Participants"]);
      const cAdults = findSchoolCol(headers, ["Accompagnateurs", "Nombre d'accompagnateurs", "Adultes"]);
      const cTotal = findSchoolCol(headers, ["Nombre total", "Total", "Effectif"]);

      data.forEach(row => {
        const status = cValid !== -1 ? schoolStatus(row[cValid]) : "pending";
        if (status === "cancelled") return;

        let rawIds = cIds !== -1 ? String(row[cIds] || "") : "";
        if (!rawIds) {
          rawIds = String((row || []).find(cell => /2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(String(cell || ""))) || "");
        }
        if (!rawIds) return;

        String(rawIds).split(/[;\n]+/).map(x => x.trim()).filter(Boolean).forEach(line => {
          const key = normalizeReservationKey(line);
          if (!key) return;
          ...
          if (!total && (cStudents !== -1 || cAdults !== -1)) total = numberFrom(row[cStudents]) + numberFrom(row[cAdults]);
          if (!total) return;
  ...
  function applySchoolReservationsToPublicSlots(slots, schoolMap) {
    const map = schoolMap || {};
    return (slots || []).map(slot => {
      try {
        const key = normalizeReservationKey(slotReservationId(slot));
        const school = map[key] || { confirmed: 0, pending: 0, total: 0 };
        const schoolReserved = Number(school.total || 0);
  ...
  Promise.all([
    ...
    fetch(DEMANDES_ECOLES_CSV).then(r => { if (!r.ok) throw new Error("CSV demandes écoles HTTP " + r.status); return r.text(); }).catch(() => "")
  ])
    .then(([propText, publicText, schoolText]) => {
      ...
      try {
        const schoolMap = buildSchoolReservationsFromCsv(schoolText || "");
        allSlots = applySchoolReservationsToPublicSlots(allSlots, schoolMap);
        window.JPO_PUBLIC_SCHOOL_RESERVATIONS_DEBUG = schoolMap;
      } catch (err) {
        console.warn("Réservations écoles non appliquées côté public", err);
      }
      renderControls();
      bind();
      render();
