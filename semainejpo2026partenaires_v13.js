document.addEventListener("DOMContentLoaded", function () {
  ...
  function isAffluenceMoment(s){return confirmedAtMoment(s)>=AFFLUENCE_THRESHOLD;}
  function isRepere(s){
    if(hasAuditorium(s)) return true;
    return hasDirectPublic(s) && isAffluenceMoment(s);
  }
  function filtered(){return allSlots.filter(s=>{
    const aud=hasAuditorium(s), aff=hasDirectPublic(s)&&isAffluenceMoment(s);
  ...
  function renderStats(slots){
    ...
    allSlots.forEach(s=>{if(hasDirectPublic(s)&&isAffluenceMoment(s))affTimes.add(s.dateIso+"|"+fmtTime(s.start));});
  ...
  function renderCard(s){
    const moment=confirmedAtMoment(s);
    const direct=s.confirmed||0;
    return '<article class="card '+(isRepere(s)?'strong':'')+'"><div class="top"><div class="chips">'+
      (hasAuditorium(s)?'<span class="chip gold">Auditorium</span>':'')+
      (direct>0?'<span class="chip green">Public réservé</span>':'')+
      (!hasAuditorium(s)&&moment>=AFFLUENCE_THRESHOLD?'<span class="chip green">Affluence horaire</span>':'')+
      '<span class="chip blue">'+esc(formatDate(s.dateIso))+'</span><span class="chip">'+esc(showTime(s.start))+'</span></div><div class="title">'+esc(s.title)+'</div>'+
      ...
      (direct>0?'<br><strong>Public réservé sur cette proposition : '+direct+' personne(s)</strong>':'')+
      '<br>Repère partenaire — aucune réservation depuis cette page</div></article>';
  }
  function render(){
    ...
    $("jpo-partners-output").innerHTML=days.map(d=>{
      ...
      return '<section class="section"><div class="sectionhead"><div><div class="sectiontitle">'+esc(d.label)+'</div><div class="sectionmeta">Auditorium et horaires avec public réservé confirmé.</div></div><div class="badge">'+ds.length+' repère(s)</div></div><div class="body">'+Object.keys(by).sort((a,b)=>(mins(a)||9999)-(mins(b)||9999)).map(t=>{
        const first=by[t][0]; const total=confirmedAtMoment(first);
        return '<div class="timegroup"><div class="timehead"><div><div class="time">'+esc(t)+'</div>'+(total>=AFFLUENCE_THRESHOLD?'<div class="sectionmeta">Public réservé sur cet horaire : '+total+' personne(s)</div>':'')+'</div><div class="badge">'+by[t].length+'</div></div><div class="cards">'+by[t].map(renderCard).join('')+'</div></div>';
      }).join('')+'</div></section>';
