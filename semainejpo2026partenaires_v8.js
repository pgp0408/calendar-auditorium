document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <p class="sub">Une page de repérage pour identifier les meilleurs moments de venue : événements à l’auditorium et créneaux où les réservations confirmées indiquent une affluence importante dans le Conservatoire, y compris les publics scolaires.</p>
        <div class="intro"><strong>Logique retenue :</strong> auditorium affiché systématiquement ; autres créneaux affichés uniquement lorsque le cumul des réservations validées CRD atteint au moins ${AFFLUENCE_THRESHOLD} personnes sur le même horaire. Les demandes en attente ne sont pas comptées dans le public confirmé.</div>
      </header>
      <main class="content">
        ...
        <div class="toolbar">
          ...
          <div><label>Affichage</label><select id="jpo-partners-mode"><option value="all">Auditorium + affluence</option><option value="auditorium">Auditorium uniquement</option><option value="affluence">Affluence confirmée uniquement</option></select></div>
        </div>
  ...
  function isConfirmedValidation(value){
    const v=norm(value);
    if(!v || v.includes("non") || v.includes("refus")) return false;
    return v === "oui" || v.includes("confirm") || v.includes("valide");
  }

  function buildReservations(text){
    const rows=csvRows(text); const h=rows[0]||[]; const data=rows.slice(1); const map={};
    const C_IDS=findCol(h,["IDs créneaux","IDs creneaux","IDS créneaux","IDS creneaux"]); const C_VALID=findCol(h,["VALIDATION CRD","Validation CRD","Statut CRD","STATUT CRD"]); const C_ETAB=findCol(h,["Établissement","Etablissement","Nom de l’établissement","Nom de l'etablissement","Structure","École","Ecole","Nom de l’école","Nom de l'ecole","Organisme"]);
    data.forEach(r=>{
      const validation=C_VALID!==-1?norm(r[C_VALID]):"";
      if(validation.includes("non") || validation.includes("refus")) return;
      const confirmed=isConfirmedValidation(validation);
      let ids=C_IDS!==-1?r[C_IDS]:"";
      if(!ids){ids=(r.find(c=>/2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(String(c||"")))||"");}
      String(ids||"").split(";").map(x=>x.trim()).filter(Boolean).forEach(line=>{
        const p=line.split("|").map(x=>x.trim()); if(p.length<8)return;
        const id=p.slice(0,5).join("|"); const students=num(p[6]); const adults=num(p[7]); const total=students+adults; if(!id||!total)return;
        if(!map[id])map[id]={confirmed:0,pending:0,details:[]};
        if(confirmed) map[id].confirmed+=total; else map[id].pending+=total;
        map[id].details.push({establishment:C_ETAB!==-1?r[C_ETAB]:"",group:p[5]||"Groupe",students,adults,total,state:confirmed?"confirmed":"pending"});
      });
    }); return map;
  }

  function applyReservations(slots,map){return slots.map(s=>{let res=map[s.id]; if(!res){res={confirmed:0,pending:0,details:[]}; const prefix=[s.dateIso,compact(s.start),compact(s.end),s.roomKey].join("|")+"|"; Object.keys(map).forEach(id=>{if(id.indexOf(prefix)===0){res.confirmed+=map[id].confirmed||0;res.pending+=map[id].pending||0;(map[id].details||[]).forEach(d=>res.details.push(d));}});} const extra=(res.details||[]).map(d=>(d.establishment||"")+" "+(d.group||"")).join(" "); return {...s,confirmed:res.confirmed||0,pending:res.pending||0,details:res.details||[],search:norm(s.search+" "+extra)};});}
  function confirmedAtMoment(slot){return allSlots.filter(s=>overlaps(slot,s)).reduce((sum,s)=>sum+(s.confirmed||0),0);}
  function pendingAtMoment(slot){return allSlots.filter(s=>overlaps(slot,s)).reduce((sum,s)=>sum+(s.pending||0),0);}
  function isRepere(s){return s.roomKey==="auditorium" || confirmedAtMoment(s)>=AFFLUENCE_THRESHOLD;}
  function filtered(){return allSlots.filter(s=>{const aud=s.roomKey==="auditorium", aff=confirmedAtMoment(s)>=AFFLUENCE_THRESHOLD; if(activeMode==="auditorium"&&!aud)return false; if(activeMode==="affluence"&&!aff)return false; if(activeMode==="all"&&!aud&&!aff)return false; if(activeDay!=="all"&&s.dateIso!==activeDay)return false; if(searchTerm&&!s.search.includes(norm(searchTerm)))return false; return true;});}
  function renderStats(slots){const aud=allSlots.filter(s=>s.roomKey==="auditorium").length; const aff=allSlots.filter(s=>confirmedAtMoment(s)>=AFFLUENCE_THRESHOLD).length; const max=allSlots.reduce((m,s)=>Math.max(m,confirmedAtMoment(s)),0); const pending=allSlots.reduce((sum,s)=>sum+(s.pending||0),0); $("jpo-partners-stats").innerHTML=[[slots.length,"repères affichés"],[aud,"événements auditorium"],[aff,"créneaux d’affluence confirmée"],[max,"pic réservé confirmé"]].map(x=>'<div class="stat"><div class="num">'+x[0]+'</div><div class="lab">'+esc(x[1])+'</div></div>').join("");}
  function renderCard(s){const aff=confirmedAtMoment(s);return '<article class="card '+(isRepere(s)?'strong':'')+'"><div class="top"><div class="chips">'+(s.roomKey==="auditorium"?'<span class="chip gold">Auditorium</span>':'')+(aff>=AFFLUENCE_THRESHOLD?'<span class="chip green">Affluence confirmée</span>':'')+'<span class="chip blue">'+esc(formatDate(s.dateIso))+'</span><span class="chip">'+esc(showTime(s.start))+'</span></div><div class="title">'+esc(s.title)+'</div>'+(s.description?'<div class="desc">'+esc(s.description)+'</div>':'')+'</div><div class="cardbody"><strong>'+esc(s.roomLabel)+'</strong><br>'+esc(s.type||"Proposition")+(aff?'<br><strong>Réservations confirmées sur cet horaire : '+aff+' personne(s)</strong>':'')+'<br>Repère partenaire — aucune réservation depuis cette page</div></article>';}
  function render(){const slots=filtered(); renderStats(slots); const status=$("jpo-partners-status"); status.className=""; status.innerHTML=''; if(!slots.length){$("jpo-partners-output").innerHTML='<div class="empty">Aucun repère avec ces filtres. Diagnostic : '+allSlots.length+' propositions chargées, '+allSlots.filter(s=>s.roomKey==="auditorium").length+' événements auditorium détectés.</div>';return;} const days=(activeDay==="all"?DAYS:DAYS.filter(d=>d.iso===activeDay)); $("jpo-partners-output").innerHTML=days.map(d=>{const ds=slots.filter(s=>s.dateIso===d.iso); if(!ds.length)return ''; const by={}; ds.forEach(s=>{const k=showTime(s.start); (by[k]||(by[k]=[])).push(s);}); return '<section class="section"><div class="sectionhead"><div><div class="sectiontitle">'+esc(d.label)+'</div><div class="sectionmeta">Auditorium et pics de réservations confirmées.</div></div><div class="badge">'+ds.length+' repère(s)</div></div><div class="body">'+Object.keys(by).sort((a,b)=>(mins(a)||9999)-(mins(b)||9999)).map(t=>'<div class="timegroup"><div class="timehead"><div class="time">'+esc(t)+'</div><div class="badge">'+by[t].length+'</div></div><div class="cards">'+by[t].map(renderCard).join('')+'</div></div>').join('')+'</div></section>';}).join('') || '<div class="empty">Aucun repère pour ce jour.</div>';}
