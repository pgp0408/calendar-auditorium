...
" && !quoted) {
  ...
  function looksLikeV3IdsCell(value) {
    return /2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(String(value || ""));
  }
  ...
  function detectValidationFromRow(row, validationIndex) {
    ...
    row.forEach(cell => {
      const v = norm(cell);
      if (v === "oui" || v === "non" || v === "en attente") found = v;
    });
    return found;
  }
  ...
  function buildReservationMap(rows) {
    ...
    dataRows.forEach(row => {
      ...
      const idsCell = C_IDS_V3 !== -1 && row[C_IDS_V3]
        ? row[C_IDS_V3]
        : (row.find(looksLikeV3IdsCell) || "");

      if (idsCell) {
        parseV3IdsField(idsCell).forEach(item => {
    ...
    window.JPS_ECOLES_DEBUG.demandesHeaders = headers;
    window.JPS_ECOLES_DEBUG.demandesRowsCount = rows.length;
    window.JPS_ECOLES_DEBUG.reservations = map;
