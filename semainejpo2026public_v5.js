document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <p class="sub">Du 15 au 21 juin 2026, le Conservatoire Henri Tomasi ouvre ses portes au public avec des concerts, auditions, ateliers ouverts, démonstrations et rencontres.</p>
        <div class="intro"><strong>Bienvenue au Conservatoire.</strong><br>Cette page rassemble les rendez-vous accessibles au grand public pendant la Semaine Portes Ouvertes. Le Conservatoire vous accueille les lundi, mardi, jeudi et vendredi en fin de journée, à partir de 17h30 et jusqu’à 20h, avec de nombreuses propositions à découvrir. Les rendez-vous sont également ouverts le mercredi tout au long de la journée, le samedi jusqu’à 16h, puis le dimanche de 16h à 20h pour la Fête de la musique.</div>
  ...
  function uniqueHighlights(slots) {
    ...
    return slots.filter(isHighlight).filter(slot => {
      const key = norm([slot.title, slot.roomLabel, slot.type || slot.discipline || ""].join("|"));
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    });
  ...
  function renderFocus(slots) {
    ...
    $("jpo-public-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Temps forts de la semaine</div><div class="focus-sub">Les rendez-vous à l’auditorium ou dans les grandes salles sont mis en avant automatiquement. Une même programmation n’est affichée qu’une seule fois dans cette sélection, même lorsqu’elle est proposée sur plusieurs créneaux.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
