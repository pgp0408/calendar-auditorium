// V36 : harmonisation JPO, encards synthèse et jauge auditorium alignée à 180 places.
document.addEventListener("DOMContentLoaded", function () {
  ...
  const PUBLIC_RESERVATIONS_CSV =
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vQW1bMZzHG6eX8uFh3OSKx_RLEDeK1TySVyxfG1fRTMhDc5H5Ys67qtOT0GvuBUZhxFfOjswgf2Q1bC/pub?gid=1847062844&single=true&output=csv";
  ...
  const STORAGE_KEY = "jps_ecoles_v33_panier";
  const LAST_GROUP_KEY = "jps_ecoles_v33_last_group";
  ...
  root.innerHTML = `
    <style>
      ...
      #jps-ecoles .overview-stats{
        grid-template-columns:repeat(3,minmax(0,1fr));
      }

      #jps-ecoles .statgroup{
        display:flex;
      ...
      #jps-ecoles .stat-main{
        display:flex;
        align-items:baseline;
        gap:8px;
      }

      #jps-ecoles .stat-main .num{font-size:34px}

      #jps-ecoles .stat-main span{
        font-size:12px;
        color:#64748b;
        font-weight:900;
        line-height:1.2;
      ...
      #jps-ecoles .mini b{display:block;font-size:18px;color:#111827}
      #jps-ecoles .mini span{display:block;margin-top:2px;font-size:11px;color:#64748b;font-weight:900;line-height:1.2}
    ...
    <div class="wrap">
      ...
      <div class="content">
        ...
        <div id="jps-overview-stats" class="stats overview-stats"></div>
  ...
  function buildSlots(rows) {
    ...
    data.forEach((row, index) => {
      ...
      if (room.key === "auditorium") capacity = Math.min(capacity || 180, 180);
      else if (["orchestre", "chant", "choeur", "theatre", "danse1", "danse2", "danse3", "other", "any"].includes(room.key)) capacity = Math.min(capacity, 35);
  ...
  function buildPublicReservations(rows) {
    ...
    const headers = rows[0] || [];
    const data = rows.slice(1);
    const C_ID = findCol(headers, ["ID créneau", "ID creneau", "Id créneau", "Id creneau", "ID", "IDs créneaux", "IDs creneaux"]);
    const C_STATUS = findCol(headers, ["STATUT CRD", "Statut CRD", "STATUT", "Statut"]);
    const C_PLACES = findCol(headers, ["Nombre de places souhaitées", "Nombre de places souhaitees", "Nombre de places", "Places", "Places souhaitées", "Places souhaitees"]);

    data.forEach(row => {
      const rawId = C_ID !== -1 ? row[C_ID] : "";
      const status = publicReservationStatus(C_STATUS !== -1 ? row[C_STATUS] : "");
      if (!rawId || status === "cancelled") return;

      const places = parseNumber(C_PLACES !== -1 ? row[C_PLACES] : "1") || 1;
      const ids = String(rawId).split(/[;\n]+/).map(x => x.trim()).filter(Boolean);
      if (!ids.length) return;

      ids.forEach(id => {
        const normalizedId = id.split("|").slice(0, 5).map(x => x.trim()).join("|");
        if (!normalizedId) return;
        if (!map[normalizedId]) map[normalizedId] = { confirmed: 0, pending: 0, total: 0 };
        if (status === "confirmed") map[normalizedId].confirmed += places;
        else map[normalizedId].pending += places;
        map[normalizedId].total += places;
      });
    });
  ...
  function getReservationForSlot(slot, reservations) {
    ...
    if (!reservation) {
      reservation = { confirmed: 0, pending: 0, refused: 0, total: 0 };
      ...
      Object.keys(reservations).forEach(id => {
        if (id.indexOf(prefix) === 0) {
          ...
          reservation.total += reservations[id].total || 0;
        }
      });
    }
  ...
  function applyReservations(slots, reservations, publicReservations) {
    const publicMap = publicReservations || {};

    return slots.map(slot => {
      const reservation = getReservationForSlot(slot, reservations);
      const publicReservation = getReservationForSlot(slot, publicMap);
      const publicConfirmed = publicReservation.confirmed || 0;
      const publicPending = publicReservation.pending || 0;
      const publicBlocked = publicConfirmed + publicPending;
      const remaining = Math.max(0, slot.capacity - reservation.confirmed - reservation.pending - publicBlocked);
  ...
  function slotMatchesPole(slot) {
    ...
    if (pole === "autre") return hay.includes("autre") || slot.roomKey === "other";
    ...
    return (
      '<article class="card ' + (disabled ? "is-full" : "") + '">' +
        ...
        '<div class="body">' +
          ...
          '</div>' +
          '<div class="meta">' + slot.confirmed + ' confirmée(s) école · ' + slot.pending + ' en attente école · ' + (slot.publicBlocked || 0) + ' place(s) tout public bloquée(s) · jauge ' + slot.capacity + '</div>' +
  ...
  function uniqueSchoolPeopleEstimateFromSlots(slots) {
    // Les mêmes groupes reviennent souvent sur plusieurs créneaux ; on prend l'effectif max par groupe.
    const groups = {};
    (slots || []).forEach(slot => {
      (cart || []).filter(item => item.slotId === slot.id).forEach(item => {
        const key = groupKey(item.group || "Groupe");
        groups[key] = Math.max(groups[key] || 0, cartItemPeople(item));
      });
    });
    return Object.values(groups).reduce((sum, value) => sum + value, 0);
  }

  function renderOverviewStats(visible) {
    const box = $("jps-overview-stats");
    if (!box) return;
    const slots = visible || [];
    const availableSlots = slots.filter(slot => !slot.full && cartFor(slot.id) < slot.remaining).length;
    const remaining = slots.reduce((sum, slot) => sum + Math.max(0, (slot.remaining || 0) - cartFor(slot.id)), 0);
    const confirmed = slots.reduce((sum, slot) => sum + Number(slot.confirmed || 0), 0);
    const pending = slots.reduce((sum, slot) => sum + Number(slot.pending || 0), 0);
    const publicBlocked = slots.reduce((sum, slot) => sum + Number(slot.publicBlocked || 0), 0);
    const auditoriumSlots = slots.filter(slot => slot.roomKey === "auditorium").length;
    const cartSlotsCount = cart.length;
    const cartGroups = Object.keys(byGroup(cart)).length;
    const cartPeopleEstimated = uniqueCartPeopleEstimate();

    box.innerHTML =
      '<div class="stat statgroup"><div class="stat-title">Offre scolaire affichée</div><div class="stat-main"><div class="num">' + slots.length + '</div><span>créneaux avec ces filtres</span></div><div class="mini"><div><b>' + availableSlots + '</b><span>disponibles</span></div><div><b>' + remaining + '</b><span>places restantes</span></div><div><b>' + auditoriumSlots + '</b><span>auditorium</span></div><div><b>180</b><span>jauge auditorium max</span></div></div></div>' +
      '<div class="stat statgroup"><div class="stat-title">Demandes établissements</div><div class="stat-main"><div class="num">' + (confirmed + pending) + '</div><span>places écoles déjà bloquées</span></div><div class="mini"><div><b>' + confirmed + '</b><span>confirmées</span></div><div><b>' + pending + '</b><span>en attente</span></div><div><b>' + publicBlocked + '</b><span>tout public bloqué</span></div><div><b>' + availableSlots + '</b><span>créneaux réservables</span></div></div></div>' +
      '<div class="stat statgroup"><div class="stat-title">Panier en cours</div><div class="stat-main"><div class="num">' + cartSlotsCount + '</div><span>créneaux sélectionnés</span></div><div class="mini"><div><b>' + cartGroups + '</b><span>groupes / classes</span></div><div><b>' + cartPeopleEstimated + '</b><span>personnes estimées</span></div><div><b>' + selectedPeriod().replace("morning","matin").replace("afternoon","après-midi").replace("other","soirée").replace("all","journée") + '</b><span>demi-journée</span></div><div><b>' + selectedLevel().replace("all","tous") + '</b><span>niveau</span></div></div></div>';
  }
  ...
  function openInfoById(slotId) {
    ...
    $("jps-i-capacity").textContent =
      ...
      slot.confirmed + " confirmée(s) école · " +
      slot.pending + " en attente école · " +
      (slot.publicBlocked || 0) + " place(s) tout public bloquée(s) · " +
  ...
  function openBooking(slot) {
    ...
    $("jps-b-places").textContent =
      ...
      slot.pending + " en attente école, " +
      slot.confirmed + " confirmé(s) école, " +
      (slot.publicBlocked || 0) + " place(s) tout public déjà bloquée(s).";
  ...
  Promise.all([
    ...
    fetch(PUBLIC_RESERVATIONS_CSV).then(response => {
      if (!response.ok) throw new Error("Réservations tout public HTTP " + response.status);
      return response.text();
    }).catch(() => "")
  ])
    .then(([propositionsText, demandesText, publicText]) => {
      ...
      const publicReservations = buildPublicReservations(publicRows);

      allSlots = applyReservations(buildSlots(propositionsRows), reservations, publicReservations);
