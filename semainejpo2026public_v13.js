(function () {
  ...
  function priorityHighlights(slots) {
    ...
    PRIORITY_HIGHLIGHTS.forEach((rule, index) => {
      const matches = slots
        .filter(slot => priorityHighlightRank(slot) === index)
        .sort(compareSlots);
      if (!matches.length) return;
      const chosen = matches[0];
      const key = highlightProgramKey(chosen);
    ...
    uniqueHighlights(slots).forEach(slot => {
  ...
  function renderFocus(slots) {
    ...
    $("jpo-public-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Temps forts de la semaine</div><div class="focus-sub">Concert des orchestres, auditions piano, clavecin et guitares, chœurs et BRONX sont affichés en priorité. Chaque programmation n’apparaît qu’une seule fois dans cette sélection.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
