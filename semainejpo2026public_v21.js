(function () {
  ...
  if (window.__JPO_PUBLIC_V21_INITIALIZED__) return;
  window.__JPO_PUBLIC_V21_INITIALIZED__ = true;
  ...
  root.innerHTML = `
    <style>
      ...
      #jpo-public .btn{border:0;background:#111827;color:#fff;border-radius:14px;padding:10px 12px;font-size:13px;font-weight:900;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center}
      #jpo-public .btn2{border:1px solid #cbd5e1;background:#fff;color:#111827;border-radius:14px;padding:10px 12px;font-size:13px;font-weight:900;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center}
      #jpo-public .btn.disabled,#jpo-public .btn2.disabled{opacity:.55;cursor:not-allowed;pointer-events:none}
      #jpo-public .cartbar{display:none;margin:0 0 18px;border:1px solid #bbf7d0;background:#f0fdf4;border-radius:22px;padding:14px;color:#14532d}
      #jpo-public .cartinner{display:flex;justify-content:space-between;gap:14px;align-items:center;flex-wrap:wrap}
      #jpo-public .carttitle{font-size:16px;font-weight:950;color:#111827}.cartmeta{margin-top:4px;font-size:13px;font-weight:850;color:#166534;line-height:1.35}
      #jpo-public .cartactions{display:flex;gap:8px;flex-wrap:wrap;align-items:center}
      #jpo-public .cartlist{border:1px solid #e5e7eb;border-radius:18px;overflow:hidden;margin-top:12px}
      #jpo-public .cartitem{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;padding:12px;border-bottom:1px solid #edf0f3}
      #jpo-public .cartitem:last-child{border-bottom:0}.cartitemtitle{font-weight:950;color:#111827;line-height:1.25}.cartitemmeta{margin-top:4px;color:#64748b;font-size:13px;font-weight:850;line-height:1.35}
      #jpo-public .cartplaces{margin:12px 0;border:1px solid #dbeafe;background:#eff6ff;border-radius:18px;padding:12px;color:#1e3a8a;font-weight:850;line-height:1.4}
    ...
    <div class="wrap">
      ...
      <main class="content">
        ...
        <div id="jpo-public-cartbar" class="cartbar">
          <div class="cartinner">
            <div><div class="carttitle">Panier de réservation</div><div id="jpo-public-cartmeta" class="cartmeta"></div></div>
            <div class="cartactions"><button type="button" class="btn2" id="jpo-public-open-cart">Voir le panier</button><button type="button" class="btn" id="jpo-public-submit-cart">Finaliser la réservation</button></div>
          </div>
        </div>
  ...
  // Panier volontairement conservé en mémoire pendant la navigation.
  // On évite de dépendre de localStorage, qui peut être bloqué par certains navigateurs,
  // réglages de confidentialité ou couches WordPress/cache.
  function loadCart() {
    return [];
  }

  function saveCart() {
    // Pas de stockage persistant : cela évite un plantage au chargement.
  }
  ...
  function saveCartPlaces() {
    // Pas de stockage persistant : cela évite un plantage au chargement.
  }

  function cartIdForSlot(slot) {
    try { return slot.reservationId || slotTechnicalId(slot); }
    catch (err) { return slot && slot.id ? slot.id : ""; }
  }

  function cartContains(slot) {
    const id = cartIdForSlot(slot);
    return Boolean(id && cart.find(item => item.id === id));
  }

  function cartPlacesForSlot(slot) {
    return cartContains(slot) ? cartPlaces : 0;
  }

  function remainingAfterCart(slot) {
    return Math.max(0, Number(slot.publicRemaining ?? slot.capacity ?? 0) - cartPlacesForSlot(slot));
  }

  function cleanCart() {
    const valid = {};
    allSlots.forEach(slot => { valid[cartIdForSlot(slot)] = true; });
    const before = cart.length;
    cart = cart.filter(item => item && item.id && valid[item.id]);
  ...
  function publicFormUrl(slotOrItems) {
    const params = [];
    function add(key, value) {
      if (!key) return;
      params.push(encodeURIComponent(key) + "=" + encodeURIComponent(value || ""));
    }
    const items = Array.isArray(slotOrItems) ? slotOrItems : [slotOrItems];
    const clean = items.filter(Boolean);
    function itemId(item) {
      try { return item.reservationId || item.id || slotTechnicalId(item); }
      catch (err) { return item && item.id ? item.id : ""; }
    }
    add(PUBLIC_FORM_ENTRIES.places, String(cartPlaces || 1));
    add(PUBLIC_FORM_ENTRIES.evenement, clean.map(x => x.title || "").join(" ; "));
    add(PUBLIC_FORM_ENTRIES.date, clean.map(x => formatDate(x.dateIso)).join(" ; "));
    add(PUBLIC_FORM_ENTRIES.horaire, clean.map(x => showTime(x.start) + (x.end ? "-" + showTime(x.end) : "")).join(" ; "));
    add(PUBLIC_FORM_ENTRIES.lieu, clean.map(x => x.roomLabel || "").join(" ; "));
    add(PUBLIC_FORM_ENTRIES.idCreneau, clean.map(itemId).filter(Boolean).join(";"));
  ...
  function reservationLabel(slot) {
    ...
    const remaining = remainingAfterCart(slot);
    const suffix = cartContains(slot) ? " après votre panier" : "";
    return remaining + " place" + (remaining > 1 ? "s" : "") + " restante" + (remaining > 1 ? "s" : "") + suffix;
  }
  ...
  function renderCard(slot) {
    ...
    const inCart = cartContains(slot);
    const available = remainingAfterCart(slot);
    const reserveAction = slot.publicFull || (!inCart && available < cartPlaces)
      ? '<span class="btn2 disabled" aria-label="Complet">Complet</span>'
      : (inCart
        ? '<span class="btn2 disabled">Déjà au panier</span>'
        : '<button type="button" class="btn2" data-add-cart="' + esc(slot.id) + '">Ajouter au panier</button>');
  ...
  function openSlot(id) {
    ...
    $("jpo-public-modal-body").innerHTML =
      ...
      (s.publicFull ? rowHtml("Réservation", "Créneau complet") : '<div class="actions" style="margin-top:16px"><button type="button" class="btn" data-add-cart="' + esc(s.id) + '">' + (cartContains(s) ? 'Déjà dans le panier' : 'Ajouter ce créneau au panier') + '</button></div>');
    $("jpo-public-modal").classList.add("open");
  }

  function currentCartSlots() {
    const byId = {};
    allSlots.forEach(slot => { byId[cartIdForSlot(slot)] = slot; });
    return cart.map(item => byId[item.id] || item).filter(Boolean);
  }

  function maxPlacesForCart() {
    const slots = currentCartSlots();
    if (!slots.length) return 6;
    return Math.max(1, Math.min(6, ...slots.map(slot => {
  ...
  function normalizeCartPlaces() {
    cartPlaces = Math.max(1, Math.min(6, parseInt(cartPlaces || "1", 10) || 1));
    const max = maxPlacesForCart();
    if (cart.length && cartPlaces > max) cartPlaces = max;
    saveCartPlaces();
  }

  function addToCart(slotId) {
    const slot = allSlots.find(x => x.id === slotId || cartIdForSlot(x) === slotId);
    if (!slot || slot.publicFull || cartContains(slot)) return;
    if (remainingAfterCart(slot) < cartPlaces) return;
    cart.push({
      id: cartIdForSlot(slot),
      reservationId: cartIdForSlot(slot),
      slotId: slot.id,
      title: slot.title,
  ...
  function submitCart() {
    ...
    normalizeCartPlaces();
    const invalid = slots.find(slot => Number(slot.publicRemaining ?? slot.capacity ?? 0) < cartPlaces);
    if (invalid) {
      alert("Le nombre de places demandé dépasse les places restantes pour au moins un rendez-vous du panier. Merci de réduire le nombre de places ou de retirer ce rendez-vous.");
      openCartModal();
      return;
    }
    try {
      ...
      console.error("Erreur panier public JPO", err);
      alert("Impossible d'ouvrir le formulaire de réservation. Merci de réessayer ou de contacter le Conservatoire.");
    }
  }
  ...
  function renderCartBar() {
    ...
    if ($("jpo-public-cartmeta")) {
      $("jpo-public-cartmeta").textContent = cart.length
        ? cart.length + " rendez-vous sélectionné(s) · " + cartPlaces + " place(s) par rendez-vous"
        : "";
    }
  }
  ...
  function renderCartModal() {
    ...
    normalizeCartPlaces();
    const maxPlaces = maxPlacesForCart();
    const placeOptions = Array.from({ length: maxPlaces }, (_, i) => i + 1).map(n => '<option value="' + n + '">' + n + ' place' + (n > 1 ? 's' : '') + '</option>').join("");
    body.innerHTML = '<div class="cartplaces"><label for="jpo-public-cart-places"><strong>Nombre de places souhaitées pour chaque rendez-vous</strong></label><br><select id="jpo-public-cart-places" style="margin-top:8px;max-width:180px">' + placeOptions + '</select><div style="margin-top:8px">Ce nombre sera appliqué à tous les rendez-vous du panier. Le menu est limité automatiquement par le rendez-vous le plus contraint.</div></div>' +
      '<div class="cartlist">' + slots.map(slot => '<div class="cartitem"><div><div class="cartitemtitle">' + esc(slot.title) + '</div><div class="cartitemmeta">' + esc(formatDate(slot.dateIso)) + ' · ' + esc(showTime(slot.start)) + (slot.end ? '-' + esc(showTime(slot.end)) : '') + ' · ' + esc(slot.roomLabel) + '<br>' + esc(reservationLabel(slot)) + '</div></div><button type="button" class="btn2" data-remove-cart="' + esc(cartIdForSlot(slot)) + '">Retirer</button></div>').join("") + '</div>' +
      '<div class="actions" style="margin-top:16px"><button type="button" class="btn" id="jpo-public-cart-submit-inside">Finaliser la réservation</button><button type="button" class="btn2" id="jpo-public-cart-clear">Vider le panier</button></div>';
    const select = $("jpo-public-cart-places");
    if (select) {
      select.value = String(cartPlaces);
      select.addEventListener("change", function(){
        cartPlaces = parseInt(select.value || "1", 10) || 1;
        normalizeCartPlaces();
        renderCartBar();
        renderCartModal();
        render();
  ...
  function bind() {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (add && root.contains(add)) { addToCart(add.getAttribute("data-add-cart")); $("jpo-public-modal").classList.remove("open"); return; }
      ...
      const detailModal = $("jpo-public-modal");
      const cartModal = $("jpo-public-cart-modal");
      if (e.target.closest("[data-close-modal]") && detailModal) detailModal.classList.remove("open");
      if (e.target.closest("[data-close-cart]") && cartModal) cartModal.classList.remove("open");
    });
    const detailModal = $("jpo-public-modal");
    const cartModal = $("jpo-public-cart-modal");
