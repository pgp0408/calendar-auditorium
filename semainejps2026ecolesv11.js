document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderChoice(s) {
    ...
    return '<article class="choice ' + (disabled ? 'full' : '') + '" ' + (!disabled ? 'data-open-slot="' + esc(encodedId) + '" tabindex="0" role="button"' : '') + '>' +
      ...
      '<div class="actions">' + (disabled ? '<button type="button" class="btn disabled">Complet / réservé</button>' : '<button type="button" class="btn jps-real-add" data-open-slot="' + esc(encodedId) + '">Ajouter au parcours</button>') + '</div>' +
      '</article>';
  }
