(function () {
  ...
  function getSchoolIdsFromRow(row) {
    let raw = get(row, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux", "ID créneau", "ID creneau"]);
    if (raw) return raw;
    Object.keys(row || {}).some(k => {
      const v = String(row[k] || "");
      if (/2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(v)) { raw = v; return true; }
      return false;
    });
    return raw || "";
  }

  function buildSchoolReservationsSafe(rows) {
    const map = {};
    try {
      (rows || []).forEach(row => {
        const status = schoolReservationStatus(get(row, ["VALIDATION CRD", "Validation CRD", "STATUT", "Statut"]));
        if (status === "cancelled") return;
        const rawIds = getSchoolIdsFromRow(row);
        if (!rawIds) return;
  ...
  function applySchoolReservationsToPublicSlots(slots, schoolReservations) {
    ...
    return (slots || []).map(slot => {
      try {
        ...
        const schoolReserved = Number(s.total || 0);
  ...
  Promise.all([
    ...
    .then(([propText, publicText]) => {
      ...
      fetch(DEMANDES_ECOLES_CSV)
        ...
        .then(schoolText => {
          try {
            const schoolReservationsMap = buildSchoolReservationsSafe(parseCSV(schoolText || ""));
            allSlots = applySchoolReservationsToPublicSlots(allSlots, schoolReservationsMap);
            normalizeCartPlaces();
            renderCartBar();
            render();
          } catch (err) {
        ...
        .catch(err => console.warn("CSV écoles ignoré côté public", err));
    })
