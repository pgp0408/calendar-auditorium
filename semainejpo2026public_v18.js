(function () {
  ...
  function publicFormUrl(slot) {
    ...
    let id = "";
    try { id = slot.reservationId || slotTechnicalId(slot); }
    catch (err) { id = slot && slot.id ? slot.id : ""; }
    add(PUBLIC_FORM_ENTRIES.places, "1");
    add(PUBLIC_FORM_ENTRIES.evenement, slot && slot.title ? slot.title : "");
    add(PUBLIC_FORM_ENTRIES.date, slot ? formatDate(slot.dateIso) : "");
    add(PUBLIC_FORM_ENTRIES.horaire, slot ? showTime(slot.start) + (slot.end ? "-" + showTime(slot.end) : "") : "");
    add(PUBLIC_FORM_ENTRIES.lieu, slot && slot.roomLabel ? slot.roomLabel : "");
    add(PUBLIC_FORM_ENTRIES.idCreneau, id);
