document.addEventListener("DOMContentLoaded", function () {
  ...
  function makeSlot(row, forcedDate, forcedStart, forcedEnd, autoLabel, indexSuffix) {
    ...
    const capacity = numberFrom(get(row, ["CAPACITÉ CRD", "Nombre estimé de participants / spectateurs"])) || (ROOM_DEFAULTS[key] ? ROOM_DEFAULTS[key].capacity : 35);
    const estimated = numberFrom(get(row, ["Nombre estimé de participants / spectateurs"])) || capacity;
  ...
  function isHighlight(slot) {
    const txt = norm([slot.title, slot.type, slot.discipline, slot.pole, slot.roomLabel].join(" "));
    return slot.capacity > 30 || slot.estimated > 30 || slot.roomKey === "auditorium" || ["concert", "spectacle", "orchestre", "choeur", "chœur", "danse"].some(k => txt.includes(k));
  }
  ...
  function renderFocus(slots) {
    ...
    $("jpo-public-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Temps forts de la semaine</div><div class="focus-sub">Les propositions à forte jauge ou particulièrement visibles sont mises en avant automatiquement.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
  ...
  function renderCard(slot) {
    ...
    return '<article class="card' + strong + '"><div class="top"><div class="tagline">' +
      ...
      (slot.capacity ? 'Jauge indicative : ' + esc(slot.capacity) + ' personnes<br>' : '') +
  ...
  function openSlot(id) {
    ...
    $("jpo-public-modal-body").innerHTML =
      ...
      rowHtml("Jauge indicative", s.capacity ? s.capacity + " personnes" : "—") +
