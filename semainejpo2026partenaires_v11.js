document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <div class="intro"><strong>Logique retenue :</strong> auditorium affiché systématiquement ; autres créneaux affichés uniquement lorsque le cumul des réservations confirmées atteint au moins ${AFFLUENCE_THRESHOLD} personnes sur le même horaire exact. Les créneaux successifs ne sont pas additionnés entre eux.</div>
      </header>
      <main class="content">
  ...
  function timeKey(s){return [s.dateIso, fmtTime(s.start)].join("|");}
  function confirmedAtMoment(slot){
    const k=timeKey(slot);
    return allSlots.filter(s=>timeKey(s)===k).reduce((sum,s)=>sum+(s.confirmed||0),0);
  }
  function uniqueAffluenceTimes(){
    const keys=new Set();
    allSlots.forEach(s=>{if(confirmedAtMoment(s)>=AFFLUENCE_THRESHOLD)keys.add(timeKey(s));});
    return keys.size;
  }
