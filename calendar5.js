document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    <style>
      ...
      #auditorium-calendar .ac-head{padding:28px;border-bottom:1px solid #eef0f3;background:#fbfcfd}
      #auditorium-calendar h2{margin:0;font-size:30px}
      #auditorium-calendar .ac-sub{margin:8px 0 0;color:#6b7280}
      #auditorium-calendar .ac-top{display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap}
      #auditorium-calendar .ac-controls,.ac-checks{display:flex;gap:10px;flex-wrap:wrap;align-items:center}
      ...
      #auditorium-calendar .ac-badge{display:inline-block;margin-top:10px;padding:5px 9px;border-radius:999px;font-size:12px;font-weight:800;color:#fff}
      ...
      <div class="ac-head">
        <div class="ac-top">
          <div>
            ...
            <p class="ac-sub">Réservations validées affichées par espace demandé.</p>
          </div>
        ...
        <div class="ac-legend">
          <div><span class="ac-dot auditorium"></span>Auditorium validé</div>
          <div><span class="ac-dot orchestre"></span>Salle d'orchestre validée</div>
          <div><span class="ac-dot autre"></span>Autre demande validée</div>
        </div>
      </div>
...
function isValidated(v) {
  ...
  function openModal(ev) {
    ...
    $("ac-m-status").textContent = "Réservation validée";
  ...
  function renderList() {
    const el = $("ac-list");
    const data = visibleEvents().sort((a,b) => new Date(a.start) - new Date(b.start));
    el.innerHTML = "";
    ...
    data.forEach(ev => {
      ...
      item.className = "ac-item " + ev.space;
      item.innerHTML =
  ...
  fetch(CSV_URL)
    ...
    .then(text => {
      ...
      if ([COL_DEMANDEUR, COL_DATE, COL_HEURE, COL_VALIDATION, COL_CONCERNE].some(i => i === -1)) {
        throw new Error("Une ou plusieurs colonnes obligatoires sont introuvables. Vérifie les intitulés : Demandeur, Date, Heure, Validation, Votre demande concerne.");
      }

      allEvents = [];

      dataRows.forEach(row => {
        if (!isValidated(row[COL_VALIDATION])) return;
      ...
      if (!allEvents.length) {
        $("ac-error").textContent = "Aucune réservation validée trouvée.";
      }
    })
    .catch(err => {
