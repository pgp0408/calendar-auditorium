document.addEventListener("DOMContentLoaded", function () {
  let root = document.getElementById("jpo-partenaires") || document.getElementById("jpopartenaires") || document.getElementById("semainejpo-partenaires") || document.getElementById("jpo-partenaires");
  if (!root) {
  ...
  root.innerHTML = `
    <style>
      ...
      #jpo-partenaires .toolbar{display:grid;grid-template-columns:1.4fr 1fr 1fr;gap:10px;margin-bottom:18px;align-items:end}
    ...
    <div class="wrap">
      <header class="head">
        ...
        <p class="sub">Une page de lecture institutionnelle pour repérer les meilleurs moments de venue : événements à l’auditorium et créneaux où l’affluence confirmée est significative dans le Conservatoire.</p>
        <div class="intro"><strong>Principe.</strong><br>Cette page ne raisonne pas par grande salle : elle cible les événements à l’auditorium et les créneaux horaires où l’affluence confirmée est importante, y compris les groupes scolaires positionnés dans plusieurs salles au même moment.</div>
        <div class="freebox">Page de repérage uniquement : aucune réservation n’est possible depuis cette page.</div>
      </header>
      <main class="content">
        <div id="jpo-partenaires-error" class="error"></div>
        <div class="stats" id="jpo-partenaires-stats"></div>
        <div class="toolbar">
          <div class="tool"><label>Recherche</label><input id="jpo-partenaires-search" type="search" placeholder="Chercher un titre, une salle, un groupe, un établissement..."></div>
          <div class="tool"><label>Jour</label><select id="jpo-partenaires-day"></select></div>
          <div class="tool"><label>Affichage</label><select id="jpo-partenaires-focus"><option value="all">Auditorium + affluence confirmée</option><option value="affluence">Affluence confirmée uniquement</option><option value="auditorium">Auditorium uniquement</option></select></div>
        </div>
  ...
  function slug(v) {
    return norm(v).replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60);
  }
  ...
  function makeSlot(row, forcedDate, forcedStart, forcedEnd, autoLabel, indexSuffix) {
    ...
    return {
      id: "partners-" + row.__line + "-" + (indexSuffix || "0"),
      reservationId: [dateIso, compact(start), compact(end), key, slug(title)].join("|"),
      line: row.__line,
  ...
  function confirmedAtMoment(slot) {
    return allSlots.filter(s => overlapsSlots(slot, s)).reduce((sum, s) => sum + Number(s.confirmed || 0), 0);
  }

  function isHighlight(slot) {
    return slot.roomKey === "auditorium" || confirmedAtMoment(slot) >= AFFLUENCE_THRESHOLD;
  }
  ...
  function baseFiltered() {
    return allSlots.filter(slot => {
      const confirmedMoment = confirmedAtMoment(slot);
      const isAuditorium = slot.roomKey === "auditorium";
      const isAffluence = confirmedMoment >= AFFLUENCE_THRESHOLD;
      if (!isAuditorium && !isAffluence) return false;
      if (activeDay !== "all" && slot.dateIso !== activeDay) return false;
      if (activeFocus === "affluence" && !isAffluence) return false;
      if (activeFocus === "auditorium" && !isAuditorium) return false;
  ...
  function renderStats(slots) {
    const affluence = slots.filter(s => confirmedAtMoment(s) >= AFFLUENCE_THRESHOLD).length;
    const aud = slots.filter(s => s.roomKey === "auditorium").length;
    const maxConfirmed = slots.reduce((m, s) => Math.max(m, confirmedAtMoment(s)), 0);
    $("jpo-partenaires-stats").innerHTML = [
      [slots.length, "repères affichés"],
      [aud, "événements auditorium"],
      [maxConfirmed, "pic confirmé max"]
  ...
  function renderFocus(slots) {
    ...
    $("jpo-partenaires-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Moments à privilégier</div><div class="focus-sub">Événements à l’auditorium et créneaux où les réservations confirmées cumulées atteignent au moins 30 personnes.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
  ...
  function renderCard(slot) {
    ...
    return '<article class="card' + strong + '"><div class="top"><div class="tagline">' +
      ...
      '<span class="chip blue">' + esc(formatDate(slot.dateIso)) + '</span><span class="chip">' + esc(showTime(slot.start)) + '</span>' + (slot.roomKey === "auditorium" ? '<span class="chip gold">Auditorium</span>' : '') + (confirmedAtMoment(slot) >= AFFLUENCE_THRESHOLD ? '<span class="chip green">Affluence confirmée</span>' : '') + '</div>' +
      '<div class="title">' + esc(slot.title) + '</div>' +
      (slot.description ? '<div class="desc">' + esc(slot.description) + '</div>' : '') +
      '</div><div class="body">' +
      '<strong>' + esc(slot.roomLabel) + '</strong><br>' + esc(slot.type || slot.discipline || "Proposition") + '<br>' + (confirmedAtMoment(slot) ? '<strong>Affluence confirmée du créneau : ' + confirmedAtMoment(slot) + ' personne(s)</strong><br>' : '') +
      '<div class="actions"><button type="button" class="btn" data-slot-id="' + esc(slot.id) + '">Voir le détail</button><span class="btn2" aria-label="Information">Repère partenaire</span></div>' +
      '</div></article>';
  }
  ...
  function renderDays(slots) {
    ...
    $("jpo-partenaires-output").innerHTML = days.map(day => {
      ...
      return '<section class="day"><div class="dayhead"><div><div class="daytitle">' + esc(day.label) + '</div><div class="daymeta">Repères auditorium et affluence confirmée.</div></div><div class="badge">' + daySlots.length + ' rendez-vous</div></div><div class="daybody">' +
  ...
  function openSlot(id) {
    ...
    $("jpo-partenaires-modal-body").innerHTML =
      ...
      rowHtml("Repère", (s.roomKey === "auditorium" ? "Événement à l’auditorium" : "") + (confirmedAtMoment(s) >= AFFLUENCE_THRESHOLD ? (s.roomKey === "auditorium" ? " + " : "") + "affluence confirmée : " + confirmedAtMoment(s) + " personnes cumulées" : "")) +
      rowHtml("Type", s.type) +
      rowHtml("Discipline / pôle", s.pole || s.discipline) +
      rowHtml("Affluence confirmée cumulée", confirmedAtMoment(s) ? confirmedAtMoment(s) + " personne(s)" : "Aucune réservation confirmée détectée sur ce créneau") +
      rowHtml("Publics confirmés", (s.reservationDetails || []).map(d => (d.establishment ? d.establishment + " — " : "") + d.group + " : " + d.total + " personne(s)").join("\n")) +
  ...
  function bind() {
    ["jpo-partenaires-search", "jpo-partenaires-day", "jpo-partenaires-focus"].forEach(id => {
  ...
  function buildReservations(rows) {
    const map = {};
    rows.forEach(row => {
      const validation = norm(get(row, ["VALIDATION CRD", "Validation CRD", "Statut CRD", "STATUT CRD"]));
      if (validation !== "oui") return;
      const establishment = get(row, ["Établissement", "Etablissement", "Nom de l’établissement", "Nom de l'etablissement", "Structure", "École", "Ecole", "Organisme"]);
      let ids = get(row, ["IDs créneaux", "IDs creneaux", "IDS créneaux", "IDS creneaux"]);
      if (!ids) {
        Object.keys(row).some(k => {
          if (/2026-06-\d{2}\|\d{4}\|\d{4}\|/.test(String(row[k] || ""))) { ids = row[k]; return true; }
          return false;
        });
      }
      String(ids || "").split(";").map(x => x.trim()).filter(Boolean).forEach(line => {
        ...
        map[id].details.push({ establishment, group, students, adults, total });
      });
    });
    return map;
  }
  ...
  function applyReservations(slots, reservations) {
    return slots.map(slot => {
      let res = reservations[slot.reservationId];
      if (!res) {
        res = { confirmed: 0, details: [] };
        const prefix = [slot.dateIso, compact(slot.start), compact(slot.end), slot.roomKey].join("|") + "|";
        Object.keys(reservations).forEach(id => {
          if (id.indexOf(prefix) === 0) {
            res.confirmed += reservations[id].confirmed || 0;
            (reservations[id].details || []).forEach(d => res.details.push(d));
          }
        });
      }
      const detailText = (res.details || []).map(d => [d.establishment, d.group].filter(Boolean).join(" ")).join(" ");
      return { ...slot, confirmed: res.confirmed || 0, reservationDetails: res.details || [], search: norm(slot.search + " " + detailText) };
    });
  }
  ...
  fetch(PROPOSITIONS_CSV)
    ...
    .then(propText => {
      return fetch(DEMANDES_CSV)
        .then(r => r.ok ? r.text() : "")
        .catch(() => "")
        .then(demText => {
          const slots = buildSlots(parseCSV(propText));
          const reservations = demText ? buildReservations(parseCSV(demText)) : {};
          allSlots = applyReservations(slots, reservations);
          renderControls();
          bind();
          render();
        });
