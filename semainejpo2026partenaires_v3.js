document.addEventListener("DOMContentLoaded", function () {
  ...
  root.innerHTML = `
    ...
    <div class="wrap">
      <header class="head">
        ...
        <p class="sub">Une page de lecture institutionnelle pour identifier les meilleurs moments de venue : événements à l'auditorium et temps où plusieurs publics sont présents dans le Conservatoire, y compris scolaires.</p>
        <div class="intro"><strong>Objectif.</strong><br>Cette page aide les partenaires, élus et invités institutionnels à repérer les temps les plus pertinents pour venir au Conservatoire : événements à l'auditorium, grands formats et moments de forte présence dans l'établissement, y compris sur les temps scolaires.</div>
        <div class="freebox">Page de repérage interne / partenaires : aucune réservation ni inscription depuis cet espace.</div>
      </header>
      <main class="content">
        <div id="jpo-partenaires-error" class="error"></div>
        <div class="stats" id="jpo-partenaires-stats"></div>
        <div class="toolbar">
          <div class="tool"><label>Recherche</label><input id="jpo-partenaires-search" type="search" placeholder="Chercher une discipline, un titre, un lieu..."></div>
          <div class="tool"><label>Jour</label><select id="jpo-partenaires-day"></select></div>
          <div class="tool"><label>Discipline / pôle</label><select id="jpo-partenaires-pole"></select></div>
          <div class="tool"><label>Affichage</label><select id="jpo-partenaires-focus"><option value="all">Tous les repères</option><option value="tempsforts">Pics hors auditorium uniquement</option><option value="auditorium">Auditorium uniquement</option></select></div>
        </div>
  ...
  function isPublicVisible(slot) {
    // Version partenaires : on conserve aussi les temps scolaires.
    return true;
  }

  function slotPeak(slot) {
    return allSlots.filter(other => other.dateIso === slot.dateIso && overlapsWindow(other, minutes(slot.start), minutes(slot.end))).reduce((sum, other) => {
      return sum + Number(other.estimated || other.realCapacity || other.capacity || 0);
    }, 0);
  }

  function isHighlight(slot) {
    if (slot.roomKey === "auditorium") return true;
    return slotPeak(slot) >= 35;
  }
  ...
  function baseFiltered() {
    return allSlots.filter(isPublicVisible).filter(slot => {
      ...
      if (activeFocus === "all" && !isHighlight(slot)) return false;
      if (activeFocus === "tempsforts" && !(isHighlight(slot) && slot.roomKey !== "auditorium")) return false;
  ...
  function renderFocus(slots) {
    ...
    $("jpo-partenaires-focus-section").innerHTML = '<div class="focus-head"><div><div class="focus-title">Moments à privilégier</div><div class="focus-sub">Les repère(s) auditorium ou dans les grandes salles avec une jauge réelle d’au moins 35 personnes sont mis en avant automatiquement.</div></div><div class="badge">' + highlights.length + ' sélection</div></div><div class="focus-body"><div class="cards">' + highlights.map(renderCard).join("") + '</div></div>';
  }
  ...
  function renderCard(slot) {
    ...
    return '<article class="card' + strong + '"><div class="top"><div class="tagline">' +
      (isHighlight(slot) ? '<span class="chip gold">Repère partenaire</span>' : '') +
      '<span class="chip blue">' + esc(formatDate(slot.dateIso)) + '</span><span class="chip">' + esc(showTime(slot.start)) + '</span><span class="chip green">Partenaires</span></div>' +
      '<div class="title">' + esc(slot.title) + '</div>' +
      (slot.description ? '<div class="desc">' + esc(slot.description) + '</div>' : '') +
      '</div><div class="body">' +
      '<strong>' + esc(slot.roomLabel) + '</strong><br>Présence cumulée estimée sur le créneau : ' + esc(slotPeak(slot)) + ' personne(s)<br>' + esc(slot.type || slot.discipline || "Proposition") + '<br>' +
      '<div class="actions"><button type="button" class="btn" data-slot-id="' + esc(slot.id) + '">Voir le détail</button><span class="btn2" aria-label="Information">Repérage</span></div>' +
      '</div></article>';
  }

  function renderDays(slots) {
    if (!slots.length) { $("jpo-partenaires-output").innerHTML = '<div class="empty">Aucun repère à afficher avec ces filtres.</div>'; return; }
    ...
    $("jpo-partenaires-output").innerHTML = days.map(day => {
      ...
      return '<section class="day"><div class="dayhead"><div><div class="daytitle">' + esc(day.label) + '</div><div class="daymeta">Événements auditorium et moments de présence significative, y compris scolaires.</div></div><div class="badge">' + daySlots.length + ' repère(s)</div></div><div class="daybody">' +
        ...
        '</div></section>';
    }).join("") || '<div class="empty">Aucun repère à afficher avec ces filtres.</div>';
  }
  ...
  function openSlot(id) {
    ...
    $("jpo-partenaires-modal-body").innerHTML =
      ...
      rowHtml("Présence cumulée estimée", slotPeak(s) + " personne(s) sur le créneau") +
      rowHtml("Lecture", "Partenaires et gratuite, sans réservation, dans la limite des places disponibles.") +
      rowHtml("Type", s.type) +
